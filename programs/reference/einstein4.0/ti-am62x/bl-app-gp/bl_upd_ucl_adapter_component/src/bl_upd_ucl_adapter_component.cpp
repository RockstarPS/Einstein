...
/// Start of user code : Header user code for file bl_upd_ucl_adapter_component.cpp
...
bool BlUpdUclAdapterComponent::onInit()
{
...
bool BlUpdUclAdapterComponent::onExit()
{
...
bool BlUpdUclAdapterComponent::onStart()
{
	bool ret = Component::onStart();
	uint8 SyncStatus = false;
	uint8 RetVal = E_NOT_OK;

	onStart_status = true;

	RetVal = ObjUpdateHandler.RecoveryPartitionSynchronization(&SyncStatus);

	onStart_status = false;
...
bool BlUpdUclAdapterComponent::onStop()
{
...
void BlUpdUclAdapterComponent::worker()
{
...
void BlUpdUclAdapterComponent::onReceiveUpdateShortServiceReq(UpdateShortServiceReq const  & msg)
{
...
void BlUpdUclAdapterComponent::onReceiveUpdateLongServiceReq(UpdateLongServiceReq const  & msg)
{
...
void BlUpdUclAdapterComponent::periodicTask()
{

/// Start of user code : Footer user code for file upd_ucl_adapter_component.cpp
	UclTransferHandlingTask();
}
void BlUpdUclAdapterComponent::UclTransferHandlingTask(void)
{
	...
	if((UclMsgRxFlag == true)&&(onStart_status == false))
	{
		UclMsgRxFlag = false;
...
void BlUpdUclAdapterComponent::ucl_sendResponse(uint8 Sid, uint8 ErrorCode, uint16 RespLength, uint8* data )
{
...
uint8 BlUpdUclAdapterComponent::arrayTo32BitValue(const uint8_t* arr, uint32 &value)
{
