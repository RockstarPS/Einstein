...
/****************************************************************/
#define DLTEXT_CFG_NVM_BLOCKS_PRESENT             (STD_ON)

#define BURAM_ENABLE                              (STD_OFF)
#define DLTEXT_USE_SNVM                           (STD_OFF)
#define DLTEXT_CFG_DIRECT_UCL_TX_PRESENT          (STD_OFF)
#define DLTEXT_CFG_KPI_PRESENT                    (STD_OFF)
...
#define DLTEXT_CFG_SHMEM_IPC_PRESENT              (STD_ON)

#ifndef DLT_DM_TO_GIP_INTERFACE
#define DLT_DM_TO_GIP_INTERFACE                   (STD_ON)
#endif

#ifndef DLT_HSM_TO_GIP_INTERFACE
#define DLT_HSM_TO_GIP_INTERFACE                  (STD_ON)
#endif

#if (DLTEXT_CFG_SHMEM_IPC_PRESENT == STD_OFF)
  #if ((DLT_DM_TO_GIP_INTERFACE == STD_ON) || \
       (DLT_HSM_TO_GIP_INTERFACE == STD_ON))
    #error "DM/HSM IPC cannot be enabled when DLTEXT_CFG_SHMEM_IPC_PRESENT is STD_OFF"
  #endif
#endif


/* -------- Verbose payload (AUTOSAR-style) encoding for DDR shared logs ----- */
#if (DLT_USEVERBOSEMODE == STD_ON)
/* AUTOSAR verbose encoding:
 * RAWD argument → bit10 = 1 → 0x00000400 (32-bit TypeInfo)
 * Serialised in big-endian just before raw payload.
 */
#define DLTEXT_TYPEINFO_BYTES      ((uint32)0x00000400UL)

#endif /* DLT_USEVERBOSEMODE == STD_ON */

#define  DLT_DEFAULT_SESSION_ID  0U

#define DLTEXT_RESET_APID_CHAR0   ('R')
#define DLTEXT_RESET_APID_CHAR1   ('S')
#define DLTEXT_RESET_APID_CHAR2   ('T')
#define DLTEXT_RESET_APID_CHAR3   ('M')

/* CTID "RHIS" (Reset History) */
#define DLTEXT_RESET_CTID_CHAR0   ('R')
#define DLTEXT_RESET_CTID_CHAR1   ('H')
#define DLTEXT_RESET_CTID_CHAR2   ('I')
#define DLTEXT_RESET_CTID_CHAR3   ('S')

/* Size limit for reset-history DLT frame stored in Reset buffer */
#ifndef DLTEXT_RESET_DLT_MAX_SIZE
#define DLTEXT_RESET_DLT_MAX_SIZE ((uint16)DLTEXT_SIZE_256BYTES)
#endif


#define Rte_Write_pSR_DLTMessageReadRequest_DLTMessageReadRequestData  Rte_Write_DltExt_pSR_DLTMessageReadRequest_DLTMessageReadRequestData
#define DltExt_Ucl_VipToGip Rte_Write_pSR_DLTMessageReadRequest_DLTMessageReadRequestData

#define Rte_Read_rSR_DLTMessageReadAck_DLTMessageReadAckData Rte_Read_DltExt_rSR_DLTMessageReadAck_DLTMessageReadAckData
#define DltExt_Ucl_VipToGip_Ack Rte_Read_rSR_DLTMessageReadAck_DLTMessageReadAckData



#define DLTEXT_UART_PRINT          STD_ON
#define DLT_UART_APPEND_CRLF       STD_OFF
...
#define DLT_UART_BUSY_WAIT_CNT      (3000000U) /* spin-wait timeout before OS dump print */
#endif

#if (DLTEXT_PLATFORM_NOT_FEE_NvM == STD_ON)
#define NVMEXT_INLINE_DELAY_VALUE 240000U
#endif
...
#define DLT_EXT_TIMER_FRQ                       25     /*MHz*/
#define DLTEXT_GET_CURRENT_TIME_IN_US           ((Gpt_GetTimeElapsed (GptConf_GptChannelConfiguration_MCU_TIMER1))/DLT_EXT_TIMER_FRQ)
#define DLTEXT_INLINESPI_TIMEOUT_US              3000000u

// PmicCdd Reset Type Max Value
#define PMICCDD_RESET_MAX_FAULT_IDS          14U
 ...
 *                                 Type Declarations                          *
 *****************************************************************************/
extern Std_ReturnType NvMExt_WriteBlock(NvM_BlockIdType NvmBlockId, uint8* NvM_SrcPtr);
...
typedef struct
{
    uint16 FaultIds[PMICCDD_RESET_MAX_FAULT_IDS];
    uint8  NumFaults;   /* Number of valid entries (0–10) */
} FaultIdListType;
...
extern void DltExt_RstMPushOsArmDump(const uint8* DataPtr, uint8 Length);
...
extern const FaultIdListType PmicCddResetFaultIds;
...
#define DLTEXT_FAULT_STATUS_MASK              (0xF0u)
#define DLTEXT_FAULT_LOGLEVEL_MASK            (0x0Fu)
#define DLTEXT_FAULT_STATUS_SHIFT             (4u)
#define DLTEXT_FAULT_STATUS_ACTIVE_NIBBLE     ((uint8)FLTM_ACTIVE)
#define DLTEXT_FAULT_STATUS_INACTIVE_NIBBLE   ((uint8)FLTM_INACTIVE)
