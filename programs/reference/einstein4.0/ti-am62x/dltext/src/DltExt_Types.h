...
#define DLTEXT_RESET_HISTORY_RESERVED_SIZE \
    ((uint8)((uint16)DLTEXT_MAX_NVM_SIZE - (((uint16)DLTEXT_TOTAL_NUM_OF_RESET_HISTORY * (uint16)DLTEXT_ADDITIONAL_RESET_INFO_SIZE) + 3u)))
#define DLTEXT_RESET_HISTORY_MAX_SIZE \
    ((uint8)((((uint16)DLTEXT_TOTAL_NUM_OF_RESET_HISTORY * (uint16)DLTEXT_ADDITIONAL_RESET_INFO_SIZE) + 3u) + (uint16)DLTEXT_RESET_HISTORY_RESERVED_SIZE))
...
#define DLTEXT_SWHANG_FLTID                     FLTM_E_SW_HANG
#define DLTEXT_ADDITIONAL_APPLOG_INFO_SIZE      (uint8)(12u)
#define DLTEXT_SIZE_128BYTES                    128U
#define DLT_EXT_APPLOG_SHAREMEM_SIZE            2048u
#define DLTEXT_SIZE_256BYTES                    256u
#define DLTEXT_RESET_SHMEM_SIZE                 4096u
#define DLTEXT_APPLOG_SHAREMEM_BUFFER1          1u
#define DLTEXT_APPLOG_SHAREMEM_BUFFER2          2u
#define DLTEXT_RESETLOG_RESETDUMP               \
          (uint16)(DLTEXT_RESET_HISTORY_MAX_SIZE + DLTEXT_OSCPU_DUMP_SIZE) // 256
#define DLTEXT_IPC_MSG_ID                  1U
#define DLT_BUFF_ACK                       1U
#define DLT_BUFF_FULL_FLG                  1U
#define BANK_BUFFE1                        1U

#define UNEXPECTEDRESET_SIZE               3U
#define KPI_SNAPSHOT_SIZE                  5U
#define ADDON_LOGDATA_SIZE                 6U
...
typedef enum
{
    DLTEXT_BUFFER_NONE     = 0x00U,
    DLTEXT_VIP_SHAREMEMBUF1 = 0x01U,
    DLTEXT_VIP_SHAREMEMBUF2 = 0x02U,
    DLTEXT_SHAREMEMRESETBUF = 0x04U,
    DLTEXT_HSM_BUFFER1 = 0x08U,
    DLTEXT_HSM_BUFFER2      = 0x10U,
    DLTEXT_DM_BUFFER1       = 0x20U,
    DLTEXT_DM_BUFFER2       = 0x40U
...
{ // DLT Extension Reset Log Data Buffer for Critical Faults
    ...
    uint8 unexpectedResetCode[UNEXPECTEDRESET_SIZE];               /* Unexpected Reset reason code that is to be stored immeadiately */
...
uint32 Time_Stamp;
uint16 Message_ID;
uint8 Addon_LogData[ADDON_LOGDATA_SIZE];
boolean Valid_Flag;
...
typedef struct 
{
    ...
    uint8 KPI_Varible;
    uint8 Faultstatus;
    uint8 KPI_Snapshot[KPI_SNAPSHOT_SIZE];
