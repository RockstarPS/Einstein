...
#define BASE_TICK_RATE_MS   (2)
  ...
  void main(void)
  {
    ...
    printf("Power           - p\n");
    printf("Other keys 0-9, up/down/left/right, enter\n");
    ...
    //LSH_ADD_STATE(GFX_SCR_OPENING_ANIM, GFX_OPEN_ANIM);
    //LSH_ADD_STATE(GFX_SCR_SLIDEOUT_ANIM, GFX_SLIDE_OUT_ANIM);
    LSH_ADD_STATE(GFX_SCR_IMG, ANIM_MAX_ID);
  ...
  ******************************************************************************/
  ...
  DWORD WINAPI GTThreadFunction(LPVOID lpParam) {
    ...
    Graphics_Task();
    t6 = clock();
    time_taken = (double)(t6 - t5) / CLOCKS_PER_SEC * 1000;
    printf("time taken in msec %.20f\n", time_taken);
  ...
  ******************************************************************************/
  void hmi_gfx_mgr02_driver_vsync_isr(void)
  {
    ...
    for (i = 0; i < GFX_MGR02_NUM_LAYERS; i++) {
      fbp1 = (void*)hmi_gfx_mgr02_context_get_front_fb(i);
