...
void hmi_menu_prn_hndlr(struct menu_t *hmi_menu_presentation, UINT8_T p_lsh_status_U8)
{
  if((p_lsh_status_U8 != LSH_DEACTIVATED_STATUS) && (hmi_menu_presentation !=NULL))
  {
    ...
    (void)hmi_gfx_mgr02_layout_set_sid(GFX_TXT_MENU_HEADER, 
      (UINT16)hmi_menu_const_struct_array_table[hmi_menu_presentation->menu_id].menu_header_label);
    (void)hmi_gfx_mgr02_layout_set_screen(hmi_menu_presentation->prsnt_screen_id, 1);
  }
  else
  {
    (void)hmi_gfx_mgr02_layout_set_screen(hmi_menu_presentation->prsnt_screen_id, 0);
  }
}
...
void hmi_menu_prn_clear_all_lines(struct menu_t * hmi_menu_presentation, UINT16 fl_layout )
{
  ...
  if((hmi_menu_const_struct_array_table[hmi_menu_presentation->menu_id].total_display_lines) <= MENU_WITH_FOUR_ITEMS)
  {
    ...
    { 
      ...
      {
        (void)hmi_gfx_mgr02_layout_select_dimage(four_line_menu_config[fl_layout][gfxAnim].icon, GFX_MENU_NO_ICON);
        (void)hmi_gfx_mgr02_layout_select_dimage(four_line_menu_config[fl_layout][gfxAnim].arrow, GFX_MENU_NO_ICON);
        (void)hmi_gfx_mgr02_layout_set_sid(four_line_menu_config[fl_layout][gfxAnim].text,SID_TxtID_1_Blank);
      }
    }
  }
}
...
void hmi_menu_prn_update_icon(struct menu_t *hmi_menu_presentation, MENU_1LINE_T const * fl_menu_line_config_U8, UINT16_T fl_menu_item_absl_index_U16)
{
  ...
  (void)hmi_gfx_mgr02_layout_select_dimage(fl_menu_line_config_U8->icon, hmi_menu_const_struct_array_table[hmi_menu_presentation->menu_id].entry_array.static_array[fl_menu_item_absl_index_U16].item_symbol);
  //(void)hmi_gfx_mgr02_if_select_dimage(fl_menu_line_config_U8->icon, hmi_menu_const_struct_array_table[hmi_menu_presentation->menu_id].entry_array[fl_menu_item_absl_index_U16].item_symbol);
  if(entryType == MENU_ITEM_ENTRY_TYPE_MENU)
  {
    (void)hmi_gfx_mgr02_layout_select_dimage(fl_menu_line_config_U8->arrow, 0);
  }
  else
  {
    (void)hmi_gfx_mgr02_layout_select_dimage(fl_menu_line_config_U8->arrow, GFX_MENU_NO_ICON);
  }  
}
                              ...
                              UINT16_T fl_menu_item_abls_indx_U16)
{
  ...
  (void)hmi_gfx_mgr02_layout_set_obj_pos_x(fl_menu_line_config_U8->text_anim, fl_menu_line_config_U8->textx);
  (void)hmi_gfx_mgr02_layout_set_obj_pos_y(fl_menu_line_config_U8->text_anim, fl_menu_line_config_U8->texty);
  ...
  {
    (void)hmi_gfx_mgr02_layout_set_sid(fl_menu_line_config_U8->text, (UINT16)hmi_menu_const_struct_array_table[hmi_menu_presentation->menu_id].entry_array.static_array[fl_menu_item_abls_indx_U16].label.str);
  }
  ...
  {
    (void)hmi_gfx_mgr02_layout_set_text(fl_menu_line_config_U8->text, (void*)hmi_menu_const_struct_array_table[hmi_menu_presentation->menu_id].entry_array.static_array[fl_menu_item_abls_indx_U16].label.str);
  }
  else
  {
    (void)hmi_gfx_mgr02_layout_set_sid(fl_menu_line_config_U8->text, SID_TxtID_1_Blank);
  }
}
...
void hmi_menu_prn_update_cursor(UINT8_T fl_cursor_type_U8, UINT16 fl_menu_focus_line_U8)
{
  ...
  {
    (void)hmi_gfx_mgr02_layout_set_obj_pos_x(GFX_IMG_MENU_FOCUS_ANIM, lc_4line_cursor[fl_menu_focus_line_U8].cur_x);
    (void)hmi_gfx_mgr02_layout_set_obj_pos_y(GFX_IMG_MENU_FOCUS_ANIM, lc_4line_cursor[fl_menu_focus_line_U8].cur_y);
  }
}
...
void hmi_menu_prn_time(void)
{
  ...
  (void)HMI_SNPRINTF(current_time, 30, HMI_TEXT("%02lu:%02lu"), \
  (UINT32)timeinfo->tm_hour, (UINT32)timeinfo->tm_min);
  (void)hmi_gfx_mgr02_layout_set_text(GFX_TXT_MENU_TIME, current_time);
}
