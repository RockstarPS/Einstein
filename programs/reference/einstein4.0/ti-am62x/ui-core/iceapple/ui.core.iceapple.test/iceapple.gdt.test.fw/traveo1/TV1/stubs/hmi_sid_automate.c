...
extern const char* GetSIDName();
extern const char* GetFolderName();
extern void ChangeLanguage();
#if 0
#define NUM_SIDS_TO_TEST           NUM_SIDS
#define NUM_LANGUAGES_TO_TEST      NUM_LANGUAGES
#else
#define NUM_SIDS_TO_TEST           NUM_SIDS
#define NUM_LANGUAGES_TO_TEST      2
#endif
...
#endif
  if (langIndex < NUM_LANGUAGES_TO_TEST && langIndex != -1)
  {
    if (sidIndex < NUM_SIDS_TO_TEST)
    {
      (void)hmi_gfx_mgr02_layout_select_gfont_index(GFX_MGR02_FONT_ID_FON_GP_TXT_AUTOMATE, 0, 0);
...
void ChangeLanguage()
{
  ...
  if ((langIndex < NUM_LANGUAGES) && (langIndex < NUM_LANGUAGES_TO_TEST))
  {
...
int IsLastSID()
{
  return (sidIndex >= NUM_SIDS_TO_TEST) ? TRUE : FALSE;
}
...
const char* GetSIDName()
{
  char* sidName = "defaultSID";
  int lastSidIndex = sidIndex - 1;
  if ((lastSidIndex >= 0) && (lastSidIndex < NUM_SIDS))
  {
    sidName = sid_names[lastSidIndex];
  }
  return sidName;
}
const char* GetFolderName()
{
  ...
  if (langIndex < NUM_LANGUAGES_TO_TEST)
  {
