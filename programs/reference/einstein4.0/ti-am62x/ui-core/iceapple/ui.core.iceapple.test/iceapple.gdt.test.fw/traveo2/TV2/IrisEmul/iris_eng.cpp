...
{
  GLenum fl_error = glGetError();
  if (fl_error != GL_NO_ERROR)
  {
    ...
    fl_error = GL_NO_ERROR;
  }
}
#define CHECK_GL_ERROR CheckOpenGLError();
#else
...
static std::map<void*, IRIS2_Palette*> mapPal;
...
static GLuint brontesFB_HUD;
static GLuint brontesTex_HUD;
static GLState brontesState_HUD;
static bool   IsBrontesInit_HUD = false;
static bool   IsBrontesReleased_HUD = false;
static int currentActiveLayer = 0;
...
{
  currentActiveLayer = p_layer_index;
}
void IrisEng_DispCtrl_Draw_Layer(int p_layer_index)
{
  ...
  {   
    ...
    if (!pWindow)
      continue;
    currentLayer = (pWindow->layer == 4) ? 0 : 1;
    if (currentLayer == p_layer_index)
    {
      ...
      if (p_layer_index == 0)
      {
        glClearColor(0.0, 0.0, 1.0, 1.0); CHECK_GL_ERROR;
      }
      else
      {
...
{
  IRIS2_Palette* result;
  std::map<void*, IRIS2_Palette*>::iterator cit = mapPal.find(Key);

  if (cit == mapPal.end())
  {
    result = new IRIS2_Palette();
    mapPal[Key] = result;
  }
  else
  {
...
void IrisEng_SetBrontesCtxHUD()
{
  ...
  bool beenInitialized = IsBrontesInit_HUD;
  if (!IsBrontesInit_HUD)
  {
    glEnable(GL_TEXTURE_2D);
    glGenTextures(1, &brontesTex_HUD); CHECK_GL_ERROR;
    glBindTexture(GL_TEXTURE_2D, brontesTex_HUD); CHECK_GL_ERROR;
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE); CHECK_GL_ERROR;
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE); CHECK_GL_ERROR;
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST); CHECK_GL_ERROR;
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST); CHECK_GL_ERROR;
    ...
    glPixelStorei(GL_PACK_ALIGNMENT, 1); CHECK_GL_ERROR;
    glPixelStorei(GL_UNPACK_ALIGNMENT, 1); CHECK_GL_ERROR;
    // glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 336, 168, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL); CHECK_GL_ERROR;
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA, 800, 480, 0, GL_RGBA, GL_UNSIGNED_BYTE, NULL); CHECK_GL_ERROR;
    glGenFramebuffers(1, &brontesFB_HUD); CHECK_GL_ERROR;

    glBindFramebuffer(GL_FRAMEBUFFER, brontesFB_HUD); CHECK_GL_ERROR;
    glFramebufferTexture2D(GL_DRAW_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, brontesTex_HUD, 0); CHECK_GL_ERROR;
    status = glCheckFramebufferStatus(GL_FRAMEBUFFER);
    assert(status == GL_FRAMEBUFFER_COMPLETE);

    IsBrontesInit_HUD = true;
  }
  ...
  if (beenInitialized)
  {
    glViewport(brontesState_HUD.viewport[0], brontesState_HUD.viewport[1], brontesState_HUD.viewport[2], brontesState_HUD.viewport[3]); CHECK_GL_ERROR;

    glFrontFace(brontesState_HUD.frontFace);
    ...
    if (brontesState_HUD.scicorsTest) glEnable(GL_SCISSOR_TEST); else glDisable(GL_SCISSOR_TEST); CHECK_GL_ERROR;
    if (brontesState_HUD.depthTest) glEnable(GL_DEPTH_TEST); else glDisable(GL_DEPTH_TEST); CHECK_GL_ERROR;
    if (brontesState_HUD.cullFaces) glEnable(GL_CULL_FACE); else glDisable(GL_CULL_FACE); CHECK_GL_ERROR;
  ...
  IsBrontesReleased_HUD = false;
}
void IrisEng_ReleaseBrontesCtxHUD()
{
  if (IsBrontesInit_HUD && !IsBrontesReleased_HUD)
  {
    glGetIntegerv(GL_VIEWPORT, brontesState_HUD.viewport); CHECK_GL_ERROR;
    glGetBooleanv(GL_SCISSOR_TEST, &brontesState_HUD.scicorsTest); CHECK_GL_ERROR;
    glGetBooleanv(GL_DEPTH_TEST, &brontesState_HUD.depthTest); CHECK_GL_ERROR;
    glGetBooleanv(GL_CULL_FACE, &brontesState_HUD.cullFaces); CHECK_GL_ERROR;

    glGetIntegerv(GL_FRONT_FACE, &brontesState_HUD.frontFace); CHECK_GL_ERROR;
...
void MakeActivePECtx(IRIS2_PECtx* ctx)
{
  ...
  if (ctx->storeRectArea.isAcive)
  {
    ...
    planeBottom[3] = -1. * ((float)ctx->pStore->height - ((float)ctx->storeRectArea.y + ctx->storeRectArea.h));
...
static void BitBlt(IRIS2_Surface* pSurf, float x, float y, unsigned int color)
{
  ...
  else /* The surface is usual buffer */
  {
    ...
    if (pSurf->imgformat == CYGFX_SM_FORMAT_A8)
    {
      /* Special processing for CYGFX_SM_FORMAT_A8 */ 
