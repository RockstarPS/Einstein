...
static UINT8 hmi_popup_core_close_popup(POPUP_ID p_popup_id, POPUP_EVENTS p_event, POPUP_ANIMATION_ID p_anim_id);
static UINT8 hmi_popup_core_deactivate(POPUP_ID_T p_popup_id, POPUP_EVENTS p_event, POPUP_ANIMATION_ID p_anim_id);
static UINT8 hmi_popup_core_activate(POPUP_LAYER_TYPE const *p_layer, POPUP_ANIMATION_ID p_anim_id);
...
static void hmi_popup_core_execute_presentation(POPUP_ID p_popup_id, LSH_STATE_ID_T p_logic_state_U8, UINT8 p_status_U8);
...
**==========================================================================*/
static POPUP_ID l_prev_active_popup_id[POPUP_MAX_LAYERS];
...
void hmi_popup_core_initialize(void)
{
  ...
  for(fl_lay_index = 0; fl_lay_index < POPUP_MAX_LAYERS ; fl_lay_index++)
  {
    l_prev_active_popup_id[fl_lay_index]=L_POPUP_MAX_COUNT;
    l_active_popup_id[fl_lay_index]=L_POPUP_MAX_COUNT;
    fl_layer = l_layer_table[fl_lay_index];
    if(fl_layer->l_group_priority_table_ptr!=(LAYER_GROUP_PRIORITY_TYPE **) NULL)
    {
      for(fl_group_index = 0; fl_group_index <fl_layer->l_total_grp_priority_count ; fl_group_index++)
      {
        for(fl_queue_index=0;fl_queue_index<(fl_layer->l_group_priority_table_ptr[fl_group_index]->queue_size);fl_queue_index++)
        {
          fl_layer->l_group_priority_table_ptr[fl_group_index]->queue[fl_queue_index]=L_POPUP_MAX_COUNT;
        }
      }
    }
  }
  l_keypad_entry_table_ptr=l_total_pp_type.l_keypad_entry_pp_table_ptr;
  for(fl_index = 0; fl_index <l_total_pp_type.keypad_count_U8 ; fl_index++)
  {
  ...
  for(fl_index = 0; fl_index <l_total_pp_type.progress_bar_count_U8 ; fl_index++)
  {
    *(l_progress_bar_table_ptr[fl_index].l_progress_counter_U8)=0;
  }

  for(fl_index = 0; fl_index < ((L_POPUP_MAX_COUNT+7)/8); fl_index++)
  {
    l_pp_id_sts_bit_U8[fl_index]=0;
    l_pp_id_tmr_bit_U8[fl_index]=0;
    l_pp_id_lsh_sts_bit_U8[fl_index]=0;
    l_min_tmr_bit_U8_ptr[fl_index]=0;
  }
}
...
void hmi_popup_core_get_popup_info(POPUP_INFO_DATA_TYPE * const p_info_data, POPUP_LAYER_ID p_layer_id)
{
  ...
  POPUP_ID fl_start_index;
  POPUP_ID fl_end_index;

  if(p_layer_id < POPUP_MAX_LAYERS)
  {
    fl_layer = l_layer_table[p_layer_id];
    fl_start_index=POPUP_LAYER_START_INDEX(fl_layer);
    fl_end_index=MAX_LAYER_POPUP_ID(fl_layer);
    r_active=hmi_popup_core_find_active_popup_and_count(fl_start_index,fl_end_index);
  ...
  {
    fl_start_index=(POPUP_ID)0;
    fl_end_index=L_POPUP_MAX_COUNT;
...
{
  UINT8 fl_return_BOOL = FALSE;
  if(L_POPUP_MAX_COUNT > p_popup_id)
  {
    if(POPUP_CONTEXT_INPUT==p_context_type)
    {
...
void hmi_popup_core_close_all_popup(POPUP_LAYER_ID p_layer_id)
{
  ...
  if(POPUP_MAX_LAYERS > p_layer_id)
  {
    fl_layer = l_layer_table[p_layer_id];
  ...
  fl_active = hmi_popup_core_find_active_popup_and_count(fl_index,fl_end_index);
  fl_index = fl_active.popup_id;

  while((fl_index < fl_end_index)&&(fl_active.total_popup_count!=0))
  {
    hmi_popup_core_deactivate_popup(fl_index, POPUP_ANIM_MAX_ID);
    /* Take care the minimum timer */
    fl_active = hmi_popup_core_find_active_popup_and_count(fl_index+1,fl_end_index);
    fl_index = fl_active.popup_id;
  }
}
...
** Updated          : 20 JUN, 2013 nkrishn9
**==========================================================================*/
UINT8 hmi_popup_core_activate_popup(POPUP_ID p_popup_id, POPUP_ANIMATION_ID p_anim_id)
{
  ...
  if (L_POPUP_MAX_COUNT > p_popup_id)
  {
      ...
      if ((fl_active_pp >= (fl_max_layer_popup_id)) || (fl_active_pp >= p_popup_id)\
          ||((fl_popupid_grp_priorty== GROUP_PRIORITY(fl_active_pp))\
              &&(fl_popupid_grp_priorty!=NO_GROUP_PRIORITY)))
      {
        /****************************************************************************
        ...
        *****************************************************************************/
        if(fl_popupid_grp_priorty!=NO_GROUP_PRIORITY)
        {
          ...
          else
          {
            ...
            if(temp_self_deactivate_pp!=L_POPUP_MAX_COUNT)
            {
              ...
              {
                (void)hmi_popup_core_deactivate((POPUP_ID_T)temp_self_deactivate_pp, POPUP_CLOSED_ON_DEMAND_EVT, POPUP_ANIM_MAX_ID);
              }
            }
          ...
          fl_temp_flag=0;
          temp_low_prty_pp_start_index = (POPUP_ID)(p_popup_id+1);
          for(fl_index=(UINT8)(p_popup_id+1);((fl_index < (POPUP_ID_T)(fl_max_layer_popup_id))\
                &&(fl_temp_flag==0));fl_index++)
          {
        ...
        for(fl_index=temp_low_prty_pp_start_index;(fl_index < (fl_max_layer_popup_id));fl_index++)
        {
          ...
          if((LOW_PRIORITY_PP_SELF_DEACTIVTION(fl_index))&&(POPUP_ACTIVE(fl_index))&&(p_popup_id!=fl_index))
          {
            ...
            if(GROUP_PRIORITY(fl_index)!=NO_GROUP_PRIORITY)
            {
              (void)hmi_popup_core_remove_from_queue((POPUP_ID)fl_index, fl_layer, GROUP_PRIORITY(fl_index));
            }
            (void)hmi_popup_core_deactivate((POPUP_ID_T)fl_index, POPUP_CLOSED_ON_DEMAND_EVT, POPUP_ANIM_MAX_ID);
          }
        }

        if (fl_active_pp != fl_max_layer_popup_id)
        {
          ...
          {
            if(GROUP_PRIORITY(fl_active_pp)!=NO_GROUP_PRIORITY)
            {
              ...
              {
                (void)hmi_popup_core_deactivate((POPUP_ID_T)fl_active_pp, POPUP_CLOSED_ON_DEMAND_EVT, HMI_POPUP_REMOVE_LOWPRIO_ACTIVE_ANIM);
              }
            }
            else
            {
...
#endif
        {
          ...
          p_anim_remove_id=HMI_POPUP_REMOVE_LOWPRIO_ACTIVE_ANIM;
          if(LOGIC_STATE_ID(fl_active_pp)==LOGIC_STATE_ID(p_popup_id))
          {
            /* The animation needs to block if already active and new popup has same screen id */
            p_anim_id=POPUP_ANIM_MAX_ID;
            p_anim_remove_id=POPUP_ANIM_MAX_ID;
          }
          else
          {
            p_anim_remove_id=HMI_POPUP_REMOVE_LOWPRIO_ACTIVE_ANIM;
          ...
          if((fl_active_pp<(fl_max_layer_popup_id))&&((fl_active_pp!=p_popup_id)))
          {
            (void)LSH_REM_STATE_EXT(LOGIC_STATE_ID(fl_active_pp), p_anim_remove_id, (UINT8)fl_active_pp);
          }
          fl_return_U8 = LSH_ADD_STATE_EXT(LOGIC_STATE_ID(p_popup_id), p_anim_id, (UINT8)p_popup_id);

          if(fl_active_pp!=p_popup_id)
          {
            if(POPUP_TYPE(p_popup_id) == POPUP_PROGRESS)
            {
        ...
        if((lc_popup_config_table[p_popup_id].layer_timeout != 0)||((0!=KEYPAD_PP_MULTIPRESS_TIMEOUT(p_popup_id))&&(POPUP_TYPE(p_popup_id)==POPUP_KEYPAD)))
        {
          if(lc_popup_config_table[p_popup_id].timerid<POPUP_MAX_TIMER)
          {
            ...
            if(lc_popup_config_table[p_popup_id].timer==ALLOW_TO_EXPIRE)
            {
      ...
      else
      {
        if(!LOW_PRIORITY_PP_SELF_DEACTIVTION(p_popup_id))
        {
          ...
          if(lc_popup_config_table[p_popup_id].layer_timeout != 0)
          {
            if(lc_popup_config_table[p_popup_id].timerid<POPUP_MAX_TIMER)
            {
              ENABLE_TIMER_STATUS(p_popup_id);
...
** Updated          : 20 JUN, 2013 nkrishn9
**==========================================================================*/
UINT8 hmi_popup_core_deactivate_popup(POPUP_ID p_popup_id, POPUP_ANIMATION_ID p_anim_id)
{
  UINT8 fl_return_U8,temp_timer_status;
  POPUP_LAYER_TYPE const * fl_layer;
  ...
  if(L_POPUP_MAX_COUNT > p_popup_id)
  {
    ...
    {
      fl_layer = hmi_popup_core_get_popup_layer(p_popup_id);
      if((p_popup_id != l_active_popup_id[fl_layer->l_layer_index])||(lc_popup_config_table[p_popup_id].min_timeout == 0))
      {
        /* Minimum Timeout timer is expired , so deactivate the Popup */
        fl_return_U8 = hmi_popup_core_close_popup(p_popup_id, POPUP_CLOSED_ON_DEMAND_EVT, p_anim_id);
      }
      else
      {
...
** Updated          : 20 JUN, 2013 nkrishn9
**==========================================================================*/
UINT8 hmi_popup_core_forced_deactivate_popup(POPUP_ID p_popup_id, POPUP_ANIMATION_ID p_anim_id)
{
  UINT8 fl_return_U8;
...
UINT8 hmi_popup_core_rx_button_handler(POPUP_LAYER_ID p_layer_id, LSH_EVENT_HANDLER_STATUS_T *p_event_hndl_status_SP)
{
  ...
  fl_button_command_U16 = p_event_hndl_status_SP->button_command & (~(LSH_ANIM_MASK));
    ...
    case POPUP_ACK:
          ...
          fl_count_index++)
      {
        ...
        if(((fl_ackevent_ptr[fl_count_index].l_btn_event_U16)==fl_button_command_U16))
        {
          ...
          (void)hmi_popup_core_close_popup(fl_active_pp, POPUP_CLOSED_ON_ACK_EVT, HMI_POPUP_REMOVE_ACK_ANIM);
        }
      }
     ...
     ||((fl_return_U8 & (POPUP_BTN_ACTION)) ==(POPUP_BTN_ACTION)))
  {
    ...
    if(lc_popup_config_table[fl_active_pp].layer_timeout != 0)
    {
      ...
      * timer. Wait for the start of the timer until notified by the app */
      if(IS_TIMER_STATUS_ENABLE(fl_active_pp))
      {
        ...
        if(POPUP_TYPE(fl_active_pp)==POPUP_KEYPAD)
        {
        ...
        else if(POPUP_TYPE(fl_active_pp)==POPUP_PROGRESS)
        {
  ...
  else if((fl_return_U8 & (POPUP_BTN_BLOCK)) ==(POPUP_BTN_BLOCK))
  {
...
void hmi_popup_core_focus_handler(POPUP_LAYER_ID p_layer, LSH_FOCUS_HANDLER_STATUS_T  *p_focus_hndl_status_SP)
{
  ...
  fl_active_pp = hmi_popup_core_get_active_popup(fl_layer);
  if((POPUP_FOCUS_STATUS_MASK & p_focus_hndl_status_SP->focus_status)!=0)
  {
    /* The popup core (whole) state as ACTIVE or INACTIVE is reported here */
    if(l_total_pp_type.pp_state_handler_ptr!=POPUP_NOTIFY_CB_NULL_HANDLER_T)
    {
      POPUP_ACTIVE_DATA_TYPE fl_active;
      fl_active = hmi_popup_core_find_active_popup_and_count((POPUP_ID)0, L_POPUP_MAX_COUNT);
	  /* The flag status helps to report only on state change as either active to inactive or vice-versa */
      if((l_popup_status==TRUE)&&(0==fl_active.total_popup_count))
      {
        /*no more popup is active whole popup core */
        l_total_pp_type.pp_state_handler_ptr(POPUP_STATE_INACTIVE);
        l_popup_status=FALSE;
      }
      else if((l_popup_status==FALSE)&&(1<=fl_active.total_popup_count))
      {
    ...
    if(0==(LSH_DEACTIVATED_STATUS & p_focus_hndl_status_SP->focus_status))
    {
      ...
      if((IS_TIMER_STATUS_ENABLE((POPUP_ID)p_focus_hndl_status_SP->client_id))&&(l_active_popup_id[fl_layer->l_layer_index]!=(POPUP_ID)p_focus_hndl_status_SP->client_id))
      {
        if(lc_popup_config_table[p_focus_hndl_status_SP->client_id].timer!=ALLOW_TO_EXPIRE)
        {
    ...
    else
    {
      ...
      {
        l_active_popup_id[fl_layer->l_layer_index]=L_POPUP_MAX_COUNT;
      }
      DEACTIVATE_LSH_POPUP(p_focus_hndl_status_SP->client_id);
      hmi_popup_core_clear_min_deact_pndng(p_focus_hndl_status_SP->client_id);
    }
  }
  ...
  ** ANIMATION STATUS is not modified. it is routed as it is from LSH      */
  ...
  if(fl_active_pp!=p_focus_hndl_status_SP->client_id)
  {
...
void hmi_popup_core_presentation_handler(POPUP_LAYER_ID p_layer, LSH_PRESN_HANDLER_STATUS_T const * const p_presn_hndl_status_SP)
{
  ...
  fl_active_pp = l_active_popup_id[p_layer];
  switch(p_presn_hndl_status_SP->presentation_status)
  {
  case LSH_ACTIVATED_STATUS:
    if(fl_active_pp < (MAX_LAYER_POPUP_ID(fl_layer)))
    {
      /* New Popup is requested for activation, so deactivate the previous popup 
       * and activate the new popup*/
      if(fl_active_pp != l_prev_active_popup_id[p_layer])
      {
        if(l_prev_active_popup_id[p_layer] != L_POPUP_MAX_COUNT)
        {
          hmi_popup_core_execute_presentation( l_prev_active_popup_id[p_layer], p_presn_hndl_status_SP->logic_state_id, LSH_DEACTIVATED_STATUS);
        }
        l_prev_active_popup_id[p_layer] = fl_active_pp;
      }
      /* There is popup active in this layer */
      hmi_popup_core_execute_presentation( fl_active_pp, p_presn_hndl_status_SP->logic_state_id, LSH_ACTIVATED_STATUS);
    }
    break;
  ...
  case LSH_DEACTIVATED_STATUS:
    {
      hmi_popup_core_execute_presentation( l_prev_active_popup_id[p_layer], p_presn_hndl_status_SP->logic_state_id, LSH_DEACTIVATED_STATUS);
      l_prev_active_popup_id[p_layer] = L_POPUP_MAX_COUNT;
    }
    break;
  ...
  case LSH_REFRESH_STATUS:
    ...
    if(fl_active_pp < (MAX_LAYER_POPUP_ID(fl_layer)))
    {
      ...
      {
        hmi_popup_core_execute_presentation( l_prev_active_popup_id[p_layer], p_presn_hndl_status_SP->logic_state_id, LSH_REFRESH_STATUS);
      }
    }
    break;
...
void hmi_popup_core_remap_button_events(UINT16 p_button_id_U16, POPUP_ID p_popup_id )
{
  if (L_POPUP_MAX_COUNT > p_popup_id)
  {
    hmi_lsh_remap_button_events(p_button_id_U16, LOGIC_STATE_ID(p_popup_id));
  }
}
/*============================================================================
...
void hmi_popup_core_timeout_callback(UINT16 p_timer_id, UINT16 p_event)
{
  ...
  fl_popup_id = (POPUP_ID)(p_event & 0x00FF);
  ...
  {
    switch(fl_tmr_type_U8)
    {
    case HMI_TMR_TIME_OUT_TYPE:
      if(POPUP_PROGRESS==fl_popup_id_type)
      {
        if(FALSE==hmi_popup_core_increment_progressbar_counter(fl_popup_id))
        {
          (void)hmi_popup_core_close_popup(fl_popup_id, POPUP_CLOSED_ON_TIMEOUT_EVT, HMI_POPUP_REMOVE_PROG_TIMEOUT_ANIM);
        }
      }
      else if(POPUP_KEYPAD==fl_popup_id_type)
      {
        if(FALSE==hmi_popup_core_inc_keypadmultitimer_counter(fl_popup_id))
        {
          (void)hmi_popup_core_close_popup(fl_popup_id, POPUP_CLOSED_ON_TIMEOUT_EVT, HMI_POPUP_REMOVE_TIMEOUT_ANIM);
        }
      }
      else
      {
      ...
      if (fl_layer->l_min_tmr_id != HMI_NO_OF_TIMERS)
      {
        if (POPUP_MINIMUM_TIME(fl_popup_id) != 0)
        {
          ...
          if(MIN_TIMER_DEACTIVATE_PENDING(fl_popup_id))
          {
            ...
            (void)hmi_popup_core_close_popup(fl_popup_id, POPUP_CLOSED_ON_DEMAND_EVT, HMI_POPUP_REMOVE_MINTIMEOUT_ANIM);
          }
        }
      }
      break;
...
UINT8 hmi_popup_core_get_keypad_content(POPUP_KEYPAD_CONTENT_TYPE *const p_keypad_content)
{
  ...
  BOOLEAN fl_flag_BOOL = FALSE;

  /* Is valid popup */
  if (L_POPUP_MAX_COUNT > p_keypad_content->popup_id)
  {
    if (POPUP_KEYPAD == POPUP_TYPE(p_keypad_content->popup_id))
    {
...
void hmi_popup_core_set_keypad_entry(POPUP_KEYPAD_SET_TYPE const * const p_keypad_set)
{
  ...
  /* Is valid popup */
  if (L_POPUP_MAX_COUNT > p_keypad_set->popup_id)
  {
    if (POPUP_KEYPAD == POPUP_TYPE(p_keypad_set->popup_id))
    {
...
BOOLEAN hmi_popup_core_get_progress_bar_counter(POPUP_PROGRESS_COUNT_TYPE *p_progress)
{
  ...
  if((POPUP_ACTIVE(p_progress->popup_id))&&(POPUP_TYPE(p_progress->popup_id)==POPUP_PROGRESS))
  {
...
void hmi_popup_core_task(void)
{
  ...
  for(fl_index = 0; fl_index < ((L_POPUP_MAX_COUNT+7)/8); fl_index++)
  {
    ...
    {
      for(fl_index1 = 0; fl_index1 < 8; fl_index1++)
      {
        fl_layer = hmi_popup_core_get_popup_layer((POPUP_ID)((fl_index*8)+fl_index1));
        if((POPUP_ACTIVE(((fl_index*8)+fl_index1))==0)&&(POPUP_ACTIVE_LSH(((fl_index*8)+fl_index1))!=0)\
            &&(l_active_popup_id[fl_layer->l_layer_index]!=(((fl_index*8)+fl_index1))))
        {
          /* For all the popup which are deactivated by popup core selfly, the deactivated status is 
          updated to application */
          fl_focus_hndl_status_S.logic_state_id = LOGIC_STATE_ID((fl_index*8)+fl_index1);
          fl_focus_hndl_status_S.focus_status   = (LSH_DEACTIVATED_STATUS|LSH_LOST_FOCUS_STATUS);
          fl_focus_hndl_status_S.focus_id  = LOGIC_STATE_ID((fl_index*8)+fl_index1);
          POUP_FOCUS_HANDLER((fl_index*8)+fl_index1)( &fl_focus_hndl_status_S);
        }
...
static UINT8 hmi_popup_core_close_popup(POPUP_ID p_popup_id, POPUP_EVENTS p_event, POPUP_ANIMATION_ID p_anim_id)
{
  ...
  if (POPUP_ACTIVE(p_popup_id))
  {
    ...
    {
      if(GROUP_PRIORITY(p_popup_id)!=NO_GROUP_PRIORITY)
      {
        (void)hmi_popup_core_remove_from_queue(p_popup_id, fl_layer, GROUP_PRIORITY(p_popup_id));
      }
    }
    fl_return_U8=hmi_popup_core_deactivate((POPUP_ID_T)p_popup_id, p_event, p_anim_id);
    hmi_popup_core_disable_timer_status((POPUP_ID_T)p_popup_id);
    ...
    {
      if((p_anim_id==HMI_POPUP_REMOVE_PROG_TIMEOUT_ANIM)||(p_anim_id==HMI_POPUP_REMOVE_ACK_ANIM))
      {
        p_anim_id=HMI_DEFAULT_POPUP_ADD_ANIM;
      }
      else if(p_anim_id==HMI_POPUP_REMOVE_TIMEOUT_ANIM)
      {
        p_anim_id=HMI_POPUP_ADD_TIMEOUT_ANIM;
      }
      else if(p_anim_id==HMI_POPUP_REMOVE_MINTIMEOUT_ANIM)
      {
        p_anim_id=HMI_POPUP_ADD_MINTIME_ANIM;
      }
      else
      {
        p_anim_id=HMI_DEFAULT_POPUP_ADD_ANIM;
      }
      (void)hmi_popup_core_activate(fl_layer, p_anim_id);
    }
  }
  return(fl_return_U8);
}
...
static UINT8 hmi_popup_core_deactivate(POPUP_ID_T p_popup_id, POPUP_EVENTS p_event, POPUP_ANIMATION_ID p_anim_id)
{
  ...
  fl_return_U8 = LSH_REM_STATE_EXT(LOGIC_STATE_ID(p_popup_id), p_anim_id, (UINT8)p_popup_id);

  /* Clear the activation status bit of the corresponding popup if the popup is de-activated */
  DEACTIVATE_POPUP(p_popup_id);
  if(POPUP_TYPE(p_popup_id) == POPUP_KEYPAD)
  {
...
static UINT8 hmi_popup_core_activate(POPUP_LAYER_TYPE const *p_layer, POPUP_ANIMATION_ID p_anim_id)
{
  ...
  if (fl_active_pp != (MAX_LAYER_POPUP_ID(p_layer)))
  {
    ...
    *****************************************************************************/
    ...
    {
     /* coverity issue */
      fl_return_U8=LSH_ADD_STATE_EXT(LOGIC_STATE_ID(fl_active_pp), p_anim_id, (UINT8)fl_active_pp);
    }
...
/*============================================================================
** Function Name    : hmi_popup_core_execute_presentation
** Visibility       : Local
** Description      : This function should be called to execute the
**                    Presentation handlers
** Invocation       : Invoked by the View state manager
** Inputs/Outputs   : logic state
**                    popup ID
**                    status
** Critical Section : No
** Created          : 15 OCT, 2012 NKRISHN9
...
static void hmi_popup_core_execute_presentation(POPUP_ID p_popup_id, LSH_STATE_ID_T p_logic_state_U8, UINT8 p_status_U8)
{
  LSH_PRESN_HANDLER_STATUS_T fl_presentation;
  fl_presentation.logic_state_id=p_logic_state_U8;
  fl_presentation.presentation_status=p_status_U8;

  if (L_POPUP_MAX_COUNT > p_popup_id)
  {
    if(POUP_PRESENT_HANDLER(p_popup_id) != LSH_NULL_PRESN_HANDLER)
    {
      POUP_PRESENT_HANDLER(p_popup_id)(&fl_presentation);
    }
  }
}
...
static POPUP_LAYER_TYPE  const * hmi_popup_core_get_popup_layer(POPUP_ID p_popup_id)
{
  ...
  UINT8 fl_layer_index_U8 = 0;
  UINT8 fl_srch_flag      = 0;
  POPUP_LAYER_TYPE  const * fl_layer = NULL;
  UINT16 fl_pp_count_U16  = 0;

  for(fl_layer_index_U8 = 0; (fl_layer_index_U8 < POPUP_MAX_LAYERS) && (fl_srch_flag == 0); fl_layer_index_U8++ )
  {
...
static POPUP_ID hmi_popup_core_get_active_popup(POPUP_LAYER_TYPE const *p_layer)
{
  ...
  for (fl_index = POPUP_LAYER_START_INDEX(p_layer); 
    (fl_index <  (MAX_LAYER_POPUP_ID(p_layer)))&&(fl_stop_U8 == 0);  fl_index++)
  {
    if (POPUP_ACTIVE(fl_index))
    {
      fl_return = (POPUP_ID)fl_index;
      if(GROUP_PRIORITY(fl_return)!=NO_GROUP_PRIORITY)
      {
...
static void hmi_popup_core_start_popup_timer(POPUP_ID p_popup_id)
{
  ...
  {
    if(POPUP_KEYPAD!=POPUP_TYPE(p_popup_id))
    {
      if(lc_popup_config_table[p_popup_id].layer_timeout != 0)
      {
        if(lc_popup_config_table[p_popup_id].timerid<POPUP_MAX_TIMER)
        {
          HMI_POPUP_START_TIMER((TIMER_ID_TYPE)lc_popup_config_table[p_popup_id].timerid,lc_popup_config_table[p_popup_id].layer_timeout,((UINT16)(HMI_TMR_TIME_OUT_TYPE << 8) | (UINT16)p_popup_id));
        }
      }
    }
    else
...
static void hmi_popup_core_start_popup_min_timer(POPUP_ID p_popup_id)
{
  ...
  if(p_popup_id< L_POPUP_MAX_COUNT)
  {
    ...
    if(lc_popup_config_table[p_popup_id].min_timeout != 0)
    {
      POPUP_LAYER_TYPE const * fl_layer;
      fl_layer = hmi_popup_core_get_popup_layer(p_popup_id);
      if((fl_layer->l_min_tmr_id)<POPUP_MAX_TIMER)
      {
        ...
        (UINT16)((UINT16)(HMI_TMR_MINIMUM_TIMEOUT_TYPE << 8)| (UINT16)p_popup_id));
      }
    }
  }
}
...
static BOOLEAN hmi_popup_core_add_to_queue(POPUP_ID p_popup_id, POPUP_LAYER_TYPE const *p_layer, UINT8 p_group_priority)
{
  ...
  {
    p_group_priority=p_group_priority-1;
    /* move the popup from end of the queue to top to create a space to add a popup */
    for(fl_index=((p_layer->l_group_priority_table_ptr[p_group_priority]->queue_size)-1);fl_index>0;fl_index--)
    {
      p_layer->l_group_priority_table_ptr[p_group_priority]->queue[fl_index]=p_layer->l_group_priority_table_ptr[p_group_priority]->queue[fl_index-1];
    }
    /* Add the popup to start of the array */
    p_layer->l_group_priority_table_ptr[p_group_priority]->queue[fl_index]=(POPUP_ID_T)p_popup_id;
    fl_flag=TRUE;
  }
...
static BOOLEAN hmi_popup_core_remove_from_queue(POPUP_ID p_popup_id, POPUP_LAYER_TYPE const *p_layer, UINT8 p_group_priority)
{
  ...
  {
    p_group_priority=p_group_priority-1;
    temp_grp_pp_id_ptr=p_layer->l_group_priority_table_ptr[p_group_priority]->queue;
    temp_queue_size=p_layer->l_group_priority_table_ptr[p_group_priority]->queue_size;

    for(fl_index=0;((fl_index<(temp_queue_size-1))\
                      &&(temp_grp_pp_id_ptr[fl_index]!=L_POPUP_MAX_COUNT));fl_index++)
    {
      if((temp_grp_pp_id_ptr[fl_index]==p_popup_id)||(fl_flag!=FALSE))
      {
        temp_grp_pp_id_ptr[fl_index]=temp_grp_pp_id_ptr[fl_index+1];
        fl_flag=TRUE;
      }
    }
    ...
    if(fl_flag!=FALSE)
    {
      temp_grp_pp_id_ptr[temp_queue_size-1]=L_POPUP_MAX_COUNT;
    }
  }
  return(fl_flag);
}
...
static POPUP_ID hmi_popup_core_get_lastin_pp_in_queue(POPUP_LAYER_TYPE const *p_layer, UINT8 p_group_priority)
{
  ...
  p_group_priority = p_group_priority-1;
  if(p_group_priority < p_layer->l_total_grp_priority_count)
  {
    return_popup_id = (POPUP_ID)p_layer->l_group_priority_table_ptr[p_group_priority]->queue[0];
  }
...
static POPUP_ID hmi_popup_core_search_self_deactivate_pp_in_queue(POPUP_LAYER_TYPE const *p_layer, UINT8 p_group_priority)
{
  ...
  temp_self_deact_pp_id= L_POPUP_MAX_COUNT;

  if(p_group_priority<=LAST_GROUP_PRIORITY)
  {
    p_group_priority=p_group_priority-1;

    for(fl_index=0;((fl_index<p_layer->l_group_priority_table_ptr[p_group_priority]->queue_size)&&(fl_flag==FALSE));fl_index++)
    {
      temp_self_deact_pp_id=p_layer->l_group_priority_table_ptr[p_group_priority]->queue[fl_index];
    ...
    (fl_index < p_end_index); fl_index++)
  {
    if ((POPUP_ACTIVE(fl_index))&&(!MIN_TIMER_DEACTIVATE_PENDING(fl_index)))
    {
      if(fl_stop_U8 ==0)
      {
        ...
        if(GROUP_PRIORITY(r_active.popup_id)!=NO_GROUP_PRIORITY)
        {
...
static BOOLEAN hmi_popup_core_increment_progressbar_counter(POPUP_ID p_popup_id)
{
  ...
  if(!PROGRESSBAR_PP_CNTR_REACH_MAXCNT(p_popup_id))
  {
...
static UINT8 hmi_popup_keypad_btn_handler(POPUP_KEYPADENTRY_TYPE  const *p_keypad, UINT16 p_btn_id, POPUP_ID p_popup_id)
{
    ...
    (LSH_GET_STATUS(p_btn_id)!=LSH_BUTTON_INACTIVE_STATE))
  {
    ...
    {
      (void)hmi_popup_core_close_popup(p_popup_id, POPUP_CLOSED_ON_OK_EVT, HMI_DEFAULT_POPUP_CLOSE_ANIM);
    ...
    else if(FALSE!=hmi_popup_core_search_button_event(p_keypad->l_cancel_event, p_btn_id))
    {
      ...
      (void)hmi_popup_core_close_popup(p_popup_id, POPUP_CLOSED_ON_CANCEL_EVT, HMI_DEFAULT_POPUP_CLOSE_ANIM);
      fl_return = LSH_STATUS_ABSORBED;
      p_btn_id=0;
    }
    else
