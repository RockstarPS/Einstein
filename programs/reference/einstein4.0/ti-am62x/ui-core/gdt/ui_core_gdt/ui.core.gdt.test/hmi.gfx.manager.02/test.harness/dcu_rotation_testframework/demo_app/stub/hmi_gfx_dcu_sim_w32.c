...
static void hmi_sim_Blend_ARGB(unsigned p_layer, int x, int y, unsigned p_alpha, unsigned p_red, unsigned p_green, unsigned p_blue)
{
   ...
   if((x >= 0) && (y >= 0)&&(x < (DCU.DISP_SIZE.B.DELTA_X * 16)) && (y < (DCU.DISP_SIZE.B.DELTA_Y)))
   {
...
static char hmi_sim_dcu_copy_indexed(int p_layer, unsigned p_mask)
{
   ...
   unsigned * fl_data   = DCU.LAYER[p_layer].CTRLDESCL3.R;
   signed     s_x       = l_cur_x;
   ...
   if(width)
   {
      ...
      if(DCU.LAYER[p_layer].CTRLDESCL4.B.TILE_EN != 0)
      {
         ...
         if(DCU.LAYER[p_layer].CTRLDESCL4.B.DATA_SEL != 0)
         {
            ...
            ** it must be in RGB888 format.
            */
            fl_data = &DCU.CLUT[DCU.LAYER[p_layer].CTRLDESCL4.B.LUOFFS];
         }
         while(tile_height)
         {
            if(l_cur_y >= l_max_y)
...
static void hmi_sim_dcu_process(void)
{
      ...
      (DCU.DCU_MODE.B.DCU_MODE  ==  1)     )
   {
      ...
      {
         DCU_POS fl_pos;
         fl_pos.fl_x = DCU.LAYER[i].CTRLDESCL2.B.POSX;
         fl_pos.fl_y = DCU.LAYER[i].CTRLDESCL2.B.POSY;
    ...
    if(EDMA.CR.B.EMLM == 1)
    {
        ...
        for (i = 0u; i < 16; i++ )
        {
            while((EDMA.SERQ.R == i) && (DMA_Handler_GetChannelStatus(i) != DMA_CH_IDLE))
            {
                ...
                addressSrc = EDMA.TCD[ i ].SADDR;
                addressDest = EDMA.TCD[ i ].DADDR;
...
static void DrawPixels(HWND hWnd, HDC hdcwnd)
{
   ...
   else
   {
    ...
    BitBlt(hdcwnd,
           ...
           0, 0,
           SRCCOPY);
	if ((count > 0) /*&& (getch()) && (count < 6)*/)
	{
		pbi = CreateBitmapInfoStruct(hWnd, hbitmap);
				 
		num = swprintf (pszFile,30, L"%s", L"c:\\temp\\screen_");
		num += swprintf (pszFile + num ,(30 - num), L"%i", count);
		num += swprintf (pszFile + num ,(30 - num), L"%s", L".bmp");
		
		CreateBMPFile(hWnd, pszFile, pbi, 
				   hbitmap , hdc);
	}
...
void hmi_gfx02_dcu_sim_init(int zoom)
{
   ...
   *l_Update_P = 2;
   ...
   lcd_size.lcd_size_height = DCU.DISP_SIZE.B.DELTA_Y;
   lcd_size.zoom            = zoom;
   l_fb_p                   = calloc(lcd_size.lcd_size_width*lcd_size.lcd_size_height, sizeof(unsigned));
...
** Visibility:          :global
** Description:         :Initializes members within a BITMAPINFOHEADER structure
                         using BITMAPINFO structure
...
{ 
    ...
    if (!GetObject(hBmp, sizeof(BITMAP), (LPSTR)&bmp)) 
        MessageBox(hwnd,L"GetObject", NULL,MB_OK); 
...
** Visibility:          :global
** Description:         :Retrieves the array of palette indices, 
                         opens a BITMAP file, copies the data, and closes the file.
...
void CreateBMPFile(HWND hwnd, LPTSTR pszFile, PBITMAPINFO pbi, 
 ...
 { 
    ...
    if (!lpBits) 
		...
		MessageBox(hwnd,L"GlobalAlloc", NULL,MB_OK); 
        ...
        DIB_RGB_COLORS)) 
    {
		...
		MessageBox(hwnd,L"GetDIBits", NULL,MB_OK); 
    }
    ...
    if (hf == INVALID_HANDLE_VALUE) 
       MessageBox(hwnd,L"CreateFile", NULL,MB_OK); 
    ...
    if (!WriteFile(hf, (LPVOID) &hdr, sizeof(BITMAPFILEHEADER), 
        (LPDWORD) &dwTmp,  NULL)) 
    {
		MessageBox(hwnd,L"WriteFile", NULL,MB_OK); 
    ...
    // Close the .BMP file.  
     if (!CloseHandle(hf)) 
           MessageBox(hwnd,L"CloseHandle", NULL,MB_OK); 
