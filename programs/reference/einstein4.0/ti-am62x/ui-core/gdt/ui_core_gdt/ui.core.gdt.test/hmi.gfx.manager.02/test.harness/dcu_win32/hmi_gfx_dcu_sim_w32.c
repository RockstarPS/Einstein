...
void        hmi_dcu_sim_update_fb(void);
...
int                   lGraphicsPending;
int                   lInvalidateCalled;
int                   l_cur_x;
int                   l_cur_y;
signed                l_max_x;
signed                l_max_y;
...
void InvalidateWindow(void)
{
  MSG msg;
  lInvalidateCalled = 1;
  InvalidateRect((HWND)ghWnd, 0, 0);

  while (PeekMessage(&msg, (HWND)ghWnd, 0, 0, PM_NOREMOVE))
  {
...
void hmi_gfx_mgr02_driver_vsync_isr(void)
{
  ...
  for (i = 0; i < GFX_MGR02_NUM_LAYERS; i++) 
  {
    ...
    switch (lc_layer_def_S[i].pix_format) 
    {
      ...
      case GFX_MGR02_PIX_FORMAT_1BPP:DCU.LAYER[i].CTRLDESCL4.B.BPP      = GFX_MGR02_1BPP; break;
