...
int main(void)
{
	...
	LSH_ADD_STATE(GFX_SCR_SAMPLE,255);
#else
    APP_InitInputBuffer();
    APP_InitPxp();
    APP_InitLcdif();
...
void hmi_scr_sample_present_handler(LSH_PRESN_HANDLER_STATUS_T *p_presen_hndl_status_SP)
{
	...
	{
		//if(l_update_alpha == TRUE)
		{
		//	l_update_alpha = FALSE;
			hmi_gfx_mgr02_layout_set_obj_alpha(GFX_IMG_CIRCLE_ANIM, 255);
		}
		//else
		{
			hmi_gfx_mgr02_layout_set_obj_alpha(GFX_IMG_CIRCLE_ANIM, 254);
		//	l_update_alpha = TRUE;
		}
	}
}
