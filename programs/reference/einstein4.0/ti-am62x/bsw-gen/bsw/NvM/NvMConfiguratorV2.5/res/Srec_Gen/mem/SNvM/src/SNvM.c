...
{
	// printf("Hello_SNVM_writereq\n");
  Std_ReturnType fl_RetVal=0;
...
static void SNvM_Write_bkup1_Data(void)
{
	...
	if(SNvM_BlockConfig[SNvM_data_value.SNvM_Id].Sector_Offset!=0)
	{
...
static void SNvM_Write_Header1(void)
{
	...
	for(byte_cnt=4;byte_cnt<Page_SIZE;byte_cnt++)// padding zeros in header
	{
		SNvM_HeadBuff[byte_cnt]=0x00;
	}
  ...
  if(fl_FlsJobStatus != MEMIF_JOB_PENDING)
	  {
		if(fl_FlsJobStatus == MEMIF_JOB_OK)
		{
			...
			for(byte_cnt=0;byte_cnt<Page_SIZE;byte_cnt++)
			{
				...
				if(SNvM_Read_HeadBuff[byte_cnt]!=(SNvM_Read_HeadBuff[byte_cnt+Page_SIZE]^0xff))
				{
					Cmp_result=FALSE;
				}
			}
