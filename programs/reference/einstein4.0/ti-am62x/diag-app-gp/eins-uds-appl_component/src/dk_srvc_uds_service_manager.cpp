...
)
{
    ...
    mCacheClientAddr=0;
#ifdef UDSCFG_LOG_ENABLED
    LOGD(&gUdsBlServicesLogContext," new response received for  ",
         clientAddress," responseData size ",responseData.size(),
         " response type ",static_cast<uint32_t>(responseType));
#endif
    if (isRequestInProgress(clientAddress) != false)
    {
        ...
        if (pDiagnosticDataBufferControl.mDiagServiceManagerState == EDiagServiceManagerState_WaitingForResponse)
        {
                    ...
                    (responseType == CUdsServiceManagerIf::EResponseType_t::EResponse_Suppress))
            {
                ...
                {
                    pDiagnosticDataBufferControl.mData.insert(pDiagnosticDataBufferControl.mData.begin()+1,
            ...
            else
            {
                ...
                sendNegativeResponse(clientAddress, \
                                     static_cast<CUdsServiceHandlerIf::EResponseCode_t>(responseData[0]));
            }
        }
        else
        {
...
#ifdef UDSCFG_LOG_ENABLED
            LOGD(&gUdsBlServicesLogContext," request processing Invalid state ",
                 pDiagnosticDataBufferControl.mDiagServiceManagerState);
#endif
        }
    }
    else
...
void CUdsServiceManager::setCurrentSession(const CUdsServiceHandlerIf::ESessionType_t session)
{
    ...
    {
        if (session == CUdsServiceHandlerIf::ESessionType_t::ESessionType_DefaultSession)
        {
            /// \verbatim
            /// -# If requested session is default session do the following,
            ///     Reset communication and control DTC setting
        ...
        DiagRequest.serviceId = 0x10 ;
        DiagRequest.didId = 0xFFFF ;
        DiagRequest.controlOpLen = 0 ;
        DiagRequest.data[0]= session;
        DiagRequest.dataLen = 1;

        DK_RTE_Send_GipDiagRequest(DiagRequest);

        LOGD(&gUdsBlServicesLogContext,"Send current session level - Idx ", session  );
...
void CUdsServiceManager::setCurrentSecurityLevel(const CUdsServiceHandlerIf::ESecurityLevel_t securityLevel)
{
    ...
    {
#ifdef UDSCFG_LOG_ENABLED
        LOGD(&gUdsBlServicesLogContext,"Invalid security Level request",static_cast<uint32_t>(securityLevel));
#endif
    }
}
        ...
        if (pDiagnosticDataBufferControl.mDiagServiceManagerState == EDiagServiceManagerState_ProcessingReceivedRequest)
        {
                    ...
                    mServiceHandlerConfiguration[mCurrentServiceTableId].end())
            {
                        ...
                        (pDiagnosticDataBufferControl.mAddressType == 0x00U))
                {
                    ...
                    else if ((static_cast<uint32_t>(static_cast<uint32_t>(1U) << static_cast<uint32_t>(currSecurityLevel)) & \
                              (pTempServiceConfig->mSupportedSecurityMask)) == 0U)
                    {
...
void CUdsServiceManager::RoutineControlResponse(GipDiagResponse & msg)
{
    ////< Send the current session
    GipDiagRequest DiagRequest ;
    DiagRequest.serviceId = 0x10 ;
    DiagRequest.didId = 0xFFFF ;
    DiagRequest.controlOpLen = 0 ;
    DiagRequest.data[0]= mCurrentSession;
    DiagRequest.dataLen = 1;

    DK_RTE_Send_GipDiagRequest(DiagRequest);

    LOGD(&gUdsBlServicesLogContext,"Send current session level - Idx ", +mCurrentSession  );
