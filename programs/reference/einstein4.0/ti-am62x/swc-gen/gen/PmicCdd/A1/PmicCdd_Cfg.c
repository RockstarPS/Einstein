...
const PmicCdd_WdgConfigType PmicCdd_WdgConf =
{
    ...
    PMICCDD_LONG_WINDOW_1000MS, /* Long window timeout value in mS */
    PMICCDD_WINDOW_1_2_47850US, /*window1 timeout value*/
    PMICCDD_WINDOW_1_2_47850US, /*window2 timeout value*/
...
{
    if(PMICCDD_DEV_ERROR_DETECT == STD_ON)
    {
        volatile boolean Haltsystem = TRUE;
        const uint16 Pmic_ModuleId = 65224U;
        const uint8 Pmic_InstanceId = 0x01U;

        (void)Det_ReportError(Pmic_ModuleId,Pmic_InstanceId, ApiId, ErrorId);
        while(Haltsystem == TRUE)
        {
            //do nothing
        }
    }
...
{
    eFltM_FaultIdType PmicCdd_FaultId = 0U;
    uint8 logData[5]={0xAA, 0xAA, 0xAA, 0xAA, 0xAA};
    eFltM_FaultStatusType FaultStatus = FLTM_ACTIVE;

    logData[0] = ErrorId;
    logData[1] = ErrorCode;
    if(ErrorId == PMICCDD_ERROR_WDG_FAIL_INT_REST)
    {
        PmicCdd_FaultId = EXT_WDG_RESET;
    }
    else if((ErrorId == PMICCDD_ERROR_I2C_RECEIVE_CRC_FAILED) || (ErrorId == PMICCDD_ERROR_I2C_TRANSMIT_REG_UNLOCK_CRC_FAILED))
    {
        PmicCdd_FaultId = EXT_WDG_COMM_ERROR;
    }
    else if(ErrorId == PMICCDD_ERROR_ABIST_TEST_FAIL)
    {
        PmicCdd_FaultId = PMIC_ABIST_FAIL;
    }
    else if(ErrorId == PMICCDD_BUCK1_OVUV_FAIL)
    {
        PmicCdd_FaultId = PMICCDD_FLTM_BUCK1_OVUV;
    }
    else if(ErrorId == PMICCDD_BUCK2_OVUV_FAIL)
    {
        PmicCdd_FaultId = PMICCDD_FLTM_BUCK2_OVUV;
    }
    else if(ErrorId == PMICCDD_BUCK3_OVUV_FAIL)
    {
        PmicCdd_FaultId = PMICCDD_FLTM_BUCK3_OVUV;
    }
    else if(ErrorId == PMICCDD_BUCK4_OVUV_FAIL)
    {
        PmicCdd_FaultId = PMICCDD_FLTM_BUCK4_OVUV;
    }
    else if(ErrorId == PMICCDD_LDO1_OVUV_FAIL)
    {
        PmicCdd_FaultId = PMICCDD_FLTM_LDO1_OVUV;
    }
    else if(ErrorId == PMICCDD_LDO2_OVUV_FAIL)
    {
        PmicCdd_FaultId = PMICCDD_FLTM_LDO2_OVUV;
    }
    else if(ErrorId == PMICCDD_LDO3_OVUV_FAIL)
    {
        PmicCdd_FaultId = PMICCDD_FLTM_LDO3_OVUV;
    }
    else if(ErrorId == PMICCDD_VCCA_OVUV_FAIL)
    {
        PmicCdd_FaultId = PMICCDD_FLTM_VVCA_OVUV;
    }
    else if(ErrorId == PMICCDD_VMON1_OVUV_FAIL)
    {
        PmicCdd_FaultId = PMICCDD_FLTM_VMON1_OVUV;
    }
    else if (ErrorId == PMICCDD_ERROR_TWARN_FAIL)
    {
        /* The Twarn_int is reported before once it reached the TWARN_LEVEL, So 
         reporting it to Fltm */
        PmicCdd_FaultId = PMICCDD_SAFERECOVERY_POWER_FAULT;
        FaultStatus = FLTM_ACTIVE;
    }
    else if (ErrorId == PMICCDD_ERROR_NVM_CODE_MISMATCH)
    {
        // reporting the NVM code error as EXT_WDG_COMM_ERROR
        PmicCdd_FaultId = EXT_WDG_COMM_ERROR;
    }
    else
    {
        //Do nothing, as of now other errors are not reported to FltM
    }
    (void)FltM_ReportFault((FltM_FaultIdType)PmicCdd_FaultId, FaultStatus, &logData, 5u);
}

/**
 * @brief Wrapper function to handle SW Hang logging via DLT
 */
void PmicCdd_Callout_WdgEarlyWarning(const uint8 *DataPtr, uint8 Length)
{
    ...
    if (DataPtr != NULL_PTR)
    {
        (void)DltExt_PmicCddWdgEarlyWarning(DataPtr, Length);
    }
}
