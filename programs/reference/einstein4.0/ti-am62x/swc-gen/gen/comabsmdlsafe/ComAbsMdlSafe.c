...
*    Auto generated code in ABASAVAR at 2025-11-17 13:38:22.525827
********************************************************************************************************************/
/********************************************************************************************************************
*  HEADER INCLUDES                                                                                                  *
********************************************************************************************************************/
...
void CComAbsMdlSafeTxCan_GetCanSignalTxConfirmation(tSafeTxSignalId SignalId,tTxSigConfirmStatus *pStatus)
{
    ...
    if (SignalId < eEndOfSafeTxSignal)
    {
        ...
        if((COMABSMDLSAFE_TXCAN_IS_CONFIRMATION_SUPPORTED(pTxSignalConfig->TxSigalProps)) &&
            (BytePosition < COMABSMDLSAFE_TXCAN_NUM_OF_CONF_STATUS_BYTES))
        {
            if (TRUE == COMABSMDLSAFE_IS_BIT_SET(ComAbsMdlSafeTxCanSignalConfirmStatus[BytePosition],(SignalId & 0x07U)))
            {
...
void CComAbsMdlSafe_Impl_RxCanMdlSafeMainFunction(void)
{
    ...
    ComAbsMdlSafe_RxCanGetFrom_SG_ABS_Core();
}
...
void CComAbsMdlSafeRxCan_SignalGetJustRcvdStatus(tRxSafeSignalId SignalId, tRxSignalJustRcvdStatus *pJustRcvdStatus)
{
    ...
    if (SignalId < eEndOfSafeRxSignal)
    {
        ...
        if(BytePosition < COMABSMDLSAFE_RXCAN_NUM_OF_JUST_RCVD_STATUS_BYTES)
        {
            if (TRUE == COMABSMDLSAFE_IS_BIT_SET(ComAbsMdlSafe_RxCanJustRcvdStatus[BytePosition],(uint8)((uint32)SignalId & 0x07U)))
            {
...
Std_ReturnType CComAbsMdlSafe_RxCan_GetSignalValue(tRxSafeSignalId SignalId, uint8 *pSignalValue, tRxSafeSignalStatus *pSignalStatus)
{
        ...
        (pSignalStatus != NULL_PTR))
    {
        ...
        if (COMABSMDLSAFE_IS_BIT_SET(ComAbsMdlSafe_RxCanSignalRepetationCounterStatus[ByteIndex], (SignalId & 0x07U)) == TRUE)
        {
            SigStatus |= eSIGNAL_REPEATED;
        }
        if(COMABSMDLSAFE_IS_BIT_SET(ComAbsMdlSafe_RxCanSignalInvalidCRCStatus[ByteIndex],(SignalId & 0x07U)) == TRUE)
        {
            SigStatus |= eSIGNAL_INVALID_CRC;
        }
         if(COMABSMDLSAFE_IS_BIT_SET(ComAbsMdlSafe_RxCanSignalInvalidAliveCounterStatus[ByteIndex],(SignalId & 0x07U)) == TRUE)
        {
            SigStatus |= eSIGNAL_INVALID_ALIVE_COUNTER;
        }

        /*check if the signal value has to be fetched from COM buffer or retention buffer */
        if (COMABSMDLSAFE_IS_BIT_SET(ComAbsMdlSafe_RxCanSignal_currentStatus[ByteIndex], (uint8)((uint32)SignalId & 0x07U)))
        {
                                                    ...
                                                    tRxSafeSignalStatus *pSignalStatus, uint32 *pSignalLength)
{
        ...
        (pSignalStatus != NULL_PTR))
    {
        ...
        if (pSignalConfig->SignalInvalidIdx < COMABSMDLSAFE_RXCAN_NUM_OF_INVALID_SIGNALS)
        {
            ...
            if (byteIndex < COMABSMDLSAFE_RXCAN_NUM_OF_INVALID_SIGNALS)
            {
                if (COMABSMDLSAFE_IS_BIT_SET(ComAbsMdlSafe_RxCanSignalInvalidStatus[byteIndex],
        ...
        if (COMABSMDLSAFE_IS_BIT_SET(ComAbsMdlSafe_RxCanSignalNRStatus[ByteIndex], (uint8)((uint32)SignalId & 0x07U)))
        {
            SigStatus |= eSIGNAL_NEVER_RECEIVED;
        }
        if (COMABSMDLSAFE_IS_BIT_SET(ComAbsMdlSafe_RxCanSignalMissingStatus[ByteIndex], (uint8)((uint32)SignalId & 0x07U)))
        {
            SigStatus |= eSIGNAL_MISSING;
        }
        ...
        if (GetLocalBuffer != FALSE)
        {
            ...
            if (FALSE != COMABSMDLSAFE_IS_BIT_SET(pSignalConfig->RxSigMonitoringType, COMABSMDLSAFE_RXCAN_SIG_RETENTION_BIT_POS))
            {
                if (pSignalConfig->SignalGetFunctIdx < COMABSMDLSAFE_RXCAN_NUM_OF_U8ARR_SIG_CFG)
                {
                    ...
                    {
                        *pSignalLength = (uint32)ComAbsMdlSafe_RxCanRetentionSigBufferU8ArrLength[LengthBufferIdx];
                    }
                    else
                    {
                        *pSignalLength = 0U;
                    ...
                    for (ByteIndex = 0U;
                         ...
                         ((((uint32)ByteIndex + (uint32)pSignalConfig->SignalLengthInByte) - 1U) < COMABSMDLSAFE_RXCAN_NUM_OF_U8ARR_RETENTION_BUFFERS);
                         ByteIndex++)
                    {
                        pSignalValue[ByteIndex] = ComAbsMdlSafe_RxCanRetentionSigBufferU8Arr[(uint32)ByteIndex + (uint32)pSignalConfig->SignalBufferStartIdx];
                    }
                }
            }
            else
            {
                if (pSignalConfig->SignalGetFunctIdx < COMABSMDLSAFE_RXCAN_NUM_OF_U8ARR_SIG_CFG)
                {
                    ...
                    {
                        *pSignalLength = (uint32)ComAbsMdlSafe_RxCanSigBufferU8ArrLength[LengthBufferIdx];
                    }
                    else
                    {
                        *pSignalLength = 0U;
                    ...
                    for (ByteIndex = 0U;
                         (ByteIndex < (*pSignalLength)) && 
                         ((((uint32)ByteIndex + (uint32)pSignalConfig->SignalLengthInByte) - 1U) < COMABSMDLSAFE_RXCAN_NUM_OF_U8ARR_BUFFERS);
                         ByteIndex++)
                    {
                        pSignalValue[ByteIndex] = ComAbsMdlSafe_RxCanSigBufferU8NArr[(uint32)ByteIndex + (uint32)pSignalConfig->SignalBufferStartIdx];
                    }
                }
            }
        }
...
void CComAbsMdlSafeRxCan_SignalMissingProcessingControl(tRxSafeSignalId SignalId,boolean Status)
{
    ...
    if (SignalId < eEndOfSafeRxSignal)
    {
            ...
            (ByteIndex <  COMABSMDLSAFE_RXCAN_NUM_OF_MISSING_STATUS_BYTES))
        {
            if (Status == TRUE)
            {
                COMABSMDLSAFE_SET_BIT(ComAbsMdlSafeRxCanIsSignalMissingEnabled[ByteIndex],(SignalId & 0x07U));
                ComAbsMdlSafeRxCanSignalMissingCounter[SignalId] = pSignalConfig->SignalMissingCounterValue;
            }
            else
            {
                COMABSMDLSAFE_CLEAR_BIT(ComAbsMdlSafeRxCanIsSignalMissingEnabled[ByteIndex],(SignalId & 0x07U));
    ...
    if (SignalId < eEndOfSafeRxSignal)
    {
           ...
           (pSignalConfig->SignalInvalidIdx <  COMABSMDLSAFE_RXCAN_NUM_OF_INVALID_SIGNALS))
        {
				...
				(pSignalConfig->SignalInvalidIdx < COMABSMDLSAFE_RXCAN_NUM_OF_INVALID_SIGNALS))
            {
            	...
            	if (Status == TRUE)
				{
					COMABSMDLSAFE_SET_BIT(ComAbsMdlSafeRxCanIsSignalInvalidEnabled[ByteIndex],(pSignalConfig->SignalInvalidIdx & 0x07U));
				}
				else
				{
					COMABSMDLSAFE_CLEAR_BIT(ComAbsMdlSafeRxCanIsSignalInvalidEnabled[ByteIndex],(pSignalConfig->SignalInvalidIdx & 0x07U));
					/* clear the timer */
    ...
    if (SignalId < eEndOfSafeRxSignal)
    {
        ...
        if (ByteIndex <  COMABSMDLSAFE_RXCAN_NUM_OF_MISSING_STATUS_BYTES)
        {
            if (Status == TRUE)
            {
                ...
                COMABSMDLSAFE_SET_BIT(ComAbsMdlSafeRxCanIsSignalNREnabled[SignalId>>3U],(SignalId & 0x07U));
                /* Set the NR flag to 1 */
                COMABSMDLSAFE_SET_BIT(ComAbsMdlSafe_RxCanSignalNRStatus[SignalId>>3U],(SignalId & 0x07U));
            }
            else
            {
            	/* clear the NR monitoring status enable flag  */
                COMABSMDLSAFE_CLEAR_BIT(ComAbsMdlSafeRxCanIsSignalNREnabled[SignalId>>3U],(SignalId & 0x07U));
                /* Set the NR flag to 0 */
                COMABSMDLSAFE_CLEAR_BIT(ComAbsMdlSafe_RxCanSignalNRStatus[SignalId>>3U],(SignalId & 0x07U));
            }
        }
    }
}
...
void ComAbsMdlSafeReloadMissingTimer(tRxSafeSignalId SignalId)
{
    ...
    if (SignalId < eEndOfSafeRxSignal)
	{
        ...
        ComAbsMdlSafeRxCanSignalMissingCounter[SignalId] = pSignalConfig->SignalMissingCounterValue;
        COMABSMDLSAFE_CLEAR_BIT(ComAbsMdlSafe_RxCanSignalMissingStatus[SignalId>>3U],(SignalId & 0x07U));
    }
...
static void ComAbsMdlSafeRxCanSignalMissingProcessing(void)
{
	...
	for (SignalId = 0;SignalId < eEndOfSafeRxSignal;SignalId++)
	{
		...
		if ((TRUE == COMABSMDLSAFE_IS_BIT_SET(ComAbsMdlSafeRxCanIsSignalMissingEnabled[ByteIndex],(SignalId & 0x07U))))

		{
			...
			if (ComAbsMdlSafeRxCanSignalMissingCounter[SignalId] >0U)
			{
				...
				if (ComAbsMdlSafeRxCanSignalMissingCounter[SignalId] == 0U)
				{
					COMABSMDLSAFE_SET_BIT(ComAbsMdlSafe_RxCanSignalMissingStatus[ByteIndex],(SignalId & 0x07U));
	...
	for (SignalInvalidIdx = 0U; SignalInvalidIdx < COMABSMDLSAFE_RXCAN_NUM_OF_INVALID_SIGNALS ;SignalInvalidIdx++)
	{
		...
		if (SignalId < eEndOfSafeRxSignal)
		{
				...
				((ComAbsMdlSafeRxCanIsSignalNR(SignalId) != TRUE)))
			{
				...
				{
                    if ((FALSE == COMABSMDLSAFE_IS_BIT_SET(ComAbsMdlSafeRxCanSignalInvalidStatus[ByteIndex],(SignalInvalidIdx & 0x07U))) &&
                    ...
                    if (ComAbsMdlSafeRxCanSignalInvalidCounter[SignalInvalidIdx] > 0U)
                    {
                        ...
                        if (ComAbsMdlSafeRxCanSignalInvalidCounter[SignalInvalidIdx] == 0U)
                        {
                            COMABSMDLSAFE_SET_BIT(ComAbsMdlSafeRxCanSignalInvalidStatus[ByteIndex],(SignalInvalidIdx & 0x07U));
                        }
                    }
				}
				else
				{
					ComAbsMdlSafeRxCanSignalInvalidCounter[SignalInvalidIdx] = 0U;
					/* Clear status */
					COMABSMDLSAFE_CLEAR_BIT(ComAbsMdlSafeRxCanSignalInvalidStatus[ByteIndex],(SignalInvalidIdx & 0x07U));
				}
			}
		}
	}
...
static boolean ComAbsMdlSafeRxCanIsSignalMissing(tRxSafeSignalId SignalId)
{
    ...
    if (ByteIndex <  COMABSMDLSAFE_RXCAN_NUM_OF_MISSING_STATUS_BYTES)
    {
        status = COMABSMDLSAFE_IS_BIT_SET(ComAbsMdlSafe_RxCanSignalMissingStatus[ByteIndex],(SignalId & 0x07U));
    }
    return status;
}
...
static boolean ComAbsMdlSafeRxCanIsSignalNR(tRxSafeSignalId SignalId)
{
    ...
    {
        status = COMABSMDLSAFE_IS_BIT_SET(ComAbsMdlSafe_RxCanSignalNRStatus[ByteIndex],(SignalId & 0x07U));
    }
    return status;
}
