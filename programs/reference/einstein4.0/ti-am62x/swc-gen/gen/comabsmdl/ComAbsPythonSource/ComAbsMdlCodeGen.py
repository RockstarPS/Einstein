...
def generate_code_comabsmdl(user_name,gen_time):
    '''
    This method is used to generate the ComAbsMdl.c and ComAbsMdl.h file.
    :param user_name: file generation time
    :param gen_time: user name to be displayed for file generation
    :return: None
    '''

    # ComAbsMdl.h
    file_cfg_h = open("ComAbsMdl.h", "w")
...
********************************************************************************************************************/
Std_ReturnType ComAbsMdl_RxCanSetSignalValueU8Arr(tRxSignalId SignalId,uint8 * pSignalValue,uint32 *pSignalLength);
...
Std_ReturnType ComAbsMdl_RxCanSetSignalValue(tRxSignalId SignalId,uint8 * pSignalValue);
...
Std_ReturnType CComAbsMdl_TxCan_SetSignalFromRetentionBuffer(tTxSignalId SignalId)
{
    ...
    if (SignalId < eEndOfTxSignal)
    {
        ...
        if(COMABSMDL_TXCAN_IS_KEEP_ALIVE_SUPPORTED(pTxSignalConfig->TxSigalProps))
        {
            ...
            {
            	if ((pTxSignalConfig->SignalBufferStartIdx < COMABSMDL_TXCAN_NUM_OF_U8ARR_BUFFERS) &&
...
void CComAbsMdl_TxCan_GetCanSignalTxConfirmation(tTxSignalId SignalId,tTxSigConfirmStatus *pStatus)
{
    ...
    if (SignalId < eEndOfTxSignal)
    {
        ...
        if((COMABSMDL_TXCAN_IS_CONFIRMATION_SUPPORTED(pTxSignalConfig->TxSigalProps)) &&
            (BytePosition < COMABSMDL_TXCAN_NUM_OF_CONF_STATUS_BYTES))
        {
            if (TRUE == COMABSMDL_IS_BIT_SET(ComAbsMdl_TxCanSignalConfirmStatus[BytePosition],(SignalId & 0x07)))
            {
...
void CComAbsMdl_RxCan_SignalGetJustRcvdStatus(tRxSignalId SignalId, tRxSignalJustRcvdStatus *pJustRcvdStatus)
{
    ...
    if (SignalId < eEndOfRxSignal)
    {
        ...
        if(BytePosition < COMABSMDL_RXCAN_NUM_OF_JUST_RCVD_STATUS_BYTES)
        {
            if (TRUE == COMABSMDL_IS_BIT_SET(ComAbsMdl_RxCanJustRcvdStatus[BytePosition],(SignalId & 0x07)))
            {
...
Std_ReturnType CComAbsMdl_RxCan_GetSignalValue(tRxSignalId SignalId,uint8 * pSignalValue,tRxSignalStatus *pSignalStatus)
{
        ...
        (pSignalStatus != NULL_PTR))
    {
		...
		if ( TRUE == COMABSMDL_IS_BIT_SET(ComAbsMdl_RxCanSignal_currentStatus[ByteIndex],(SignalId & 0x07)))
		{
        ...
        if (GetLocalBuffer != FALSE)
        {
            ...
            if ( FALSE != COMABSMDL_IS_BIT_SET(pSignalConfig->RxSigMonitoringType,COMABSMDL_RXCAN_SIG_RETENTION_BIT_POS))
            {
                if (pSignalConfig->SignalGetFunctIdx < COMABSMDL_RXCAN_NUM_OF_U8ARR_SIG_CFG)
                {
                    ...
                    {
                        *pSignalLength = ComAbsMdl_RxCanRetentionSigBufferU8ArrLength[LengthBufferIdx];
                    }
                    else
                    {
                        *pSignalLength = 0;
                    ...
                    {
                        pSignalValue[ByteIndex] = ComAbsMdl_RxCanRetentionSigBufferU8Arr[ByteIndex+pSignalConfig->SignalBufferStartIdx];
                    }
                }
            }
            else
            {
                if (pSignalConfig->SignalGetFunctIdx < COMABSMDL_RXCAN_NUM_OF_U8ARR_SIG_CFG)
                {
                    ...
                    {
                        *pSignalLength = ComAbsMdl_RxCanSigBufferU8ArrLength[LengthBufferIdx];
                    }
                    else
                    {
                        *pSignalLength = 0;
                    ...
                    {
                        pSignalValue[ByteIndex] = ComAbsMdl_RxCanSigBufferU8NArr[ByteIndex+pSignalConfig->SignalBufferStartIdx];
                    }
                }
            }
        }
...
void CComAbsMdl_RxCan_SignalMissingProcessingControl(tRxSignalId SignalId,boolean Status)
{
    ...
    if (SignalId < eEndOfRxSignal)
    {
            ...
            (ByteIndex <  COMABSMDL_RXCAN_NUM_OF_MISSING_STATUS_BYTES))
        {
            if (Status == TRUE)
            {
                COMABSMDL_SET_BIT(ComAbsMdl_RxCanIsSignalMissingEnabled[ByteIndex],(SignalId & 0x07));

            }
            else
            {
                COMABSMDL_CLEAR_BIT(ComAbsMdl_RxCanIsSignalMissingEnabled[ByteIndex],(SignalId & 0x07));
                /*! Set Timer to Zero */
    ...
    if (SignalId < eEndOfRxSignal)
    {
           ...
           (pSignalConfig->SignalInvalidIdx <  COMABSMDL_RXCAN_NUM_OF_INVALID_SIGNALS))
        {
				...
				(pSignalConfig->SignalInvalidIdx < COMABSMDL_RXCAN_NUM_OF_INVALID_SIGNALS))
            {
            	...
            	if (Status == TRUE)
				{
					COMABSMDL_SET_BIT(ComAbsMdl_RxCanIsSignalInvalidEnabled[ByteIndex],(pSignalConfig->SignalInvalidIdx & 0x07));
				}
				else
				{
					COMABSMDL_CLEAR_BIT(ComAbsMdl_RxCanIsSignalInvalidEnabled[ByteIndex],(pSignalConfig->SignalInvalidIdx & 0x07));
					/* clear the timer */
    ...
    if (SignalId < eEndOfRxSignal)
    {
        ...
        if (ByteIndex <  COMABSMDL_RXCAN_NUM_OF_MISSING_STATUS_BYTES)
        {
            if (Status == TRUE)
            {
                ...
                COMABSMDL_SET_BIT(ComAbsMdl_RxCanIsSignalNREnabled[SignalId>>3U],(SignalId & 0x07));
                /* Set the NR flag to 1 */
                COMABSMDL_SET_BIT(ComAbsMdl_RxCanSignalNRStatus[SignalId>>3U],(SignalId & 0x07));
            }
            else
            {
            	/* clear the NR monitoring status enable flag  */
                COMABSMDL_CLEAR_BIT(ComAbsMdl_RxCanIsSignalNREnabled[SignalId>>3U],(SignalId & 0x07));
                /* Set the NR flag to 0 */
                COMABSMDL_CLEAR_BIT(ComAbsMdl_RxCanSignalNRStatus[SignalId>>3U],(SignalId & 0x07));
            }
        }
    }
}
...
void ComAbsMdl_ReloadMissingTimer(tRxSignalId SignalId)
{
    ...
    if (SignalId < eEndOfRxSignal)
	{
        ...
        ComAbsMdl_RxCanSignalMissingCounter[SignalId] = pSignalConfig->SignalMissingCounterValue;
        COMABSMDL_CLEAR_BIT(ComAbsMdl_RxCanSignalMissingStatus[SignalId>>3U],(SignalId & 0x07));
    }
...
void ComAbsMdl_RxCanVarInit(void)
{
	...
	ComAbsMdl_UtilMemSet(&ComAbsMdl_RxCanSignal_currentStatus[0],0xFFU,COMABSMDL_RXCAN_NUM_OF_NR_STATUS_BYTES);
...
static void ComAbsMdl_RxCanSignalMissingProcessing(void)
{
	...
	for (SignalId = 0;SignalId < eEndOfRxSignal;SignalId++)
	{
		...
		if ((TRUE == COMABSMDL_IS_BIT_SET(ComAbsMdl_RxCanIsSignalMissingEnabled[ByteIndex],(SignalId & 0x07))))

		{
			...
			if (ComAbsMdl_RxCanSignalMissingCounter[SignalId] >0U)
			{
				...
				if (ComAbsMdl_RxCanSignalMissingCounter[SignalId] == 0U)
				{
					COMABSMDL_SET_BIT(ComAbsMdl_RxCanSignalMissingStatus[ByteIndex],(SignalId & 0x07));
	...
	for (SignalInvalidIdx = 0U; SignalInvalidIdx < COMABSMDL_RXCAN_NUM_OF_INVALID_SIGNALS ;SignalInvalidIdx++)
	{
		...
		if (SignalId < eEndOfRxSignal)
		{
				...
				((ComAbsMdl_RxCanIsSignalNR(SignalId) != TRUE)))
			{
				...
				{
                    if ((FALSE == COMABSMDL_IS_BIT_SET(ComAbsMdl_RxCanSignalInvalidStatus[ByteIndex],(SignalInvalidIdx & 0x07))) &&
                    ...
                    if (ComAbsMdl_RxCanSignalInvalidCounter[SignalInvalidIdx] > 0U)
                    {
                        ...
                        if (ComAbsMdl_RxCanSignalInvalidCounter[SignalInvalidIdx] == 0U)
                        {
                            COMABSMDL_SET_BIT(ComAbsMdl_RxCanSignalInvalidStatus[ByteIndex],(SignalInvalidIdx & 0x07));
                        }
                    }
				}
				else
				{
					ComAbsMdl_RxCanSignalInvalidCounter[SignalInvalidIdx] = 0U;
					/* Clear status */
					COMABSMDL_CLEAR_BIT(ComAbsMdl_RxCanSignalInvalidStatus[ByteIndex],(SignalInvalidIdx & 0x07));
				}
			}
		}
	}
...
static boolean ComAbsMdl_RxCanIsSignalMissing(tRxSignalId SignalId)
{
    ...
    if (ByteIndex <  COMABSMDL_RXCAN_NUM_OF_MISSING_STATUS_BYTES)
    {
        status = COMABSMDL_IS_BIT_SET(ComAbsMdl_RxCanSignalMissingStatus[ByteIndex],(SignalId & 0x07));
    }
    return status;
}
...
static boolean ComAbsMdl_RxCanIsSignalNR(tRxSignalId SignalId)
{
    ...
    {
        status = COMABSMDL_IS_BIT_SET(ComAbsMdl_RxCanSignalNRStatus[ByteIndex],(SignalId & 0x07));
    }
    return status;
}
...
if __name__ == '__main__':
    ...
    generate_code_comabsmdl(username,current_time)