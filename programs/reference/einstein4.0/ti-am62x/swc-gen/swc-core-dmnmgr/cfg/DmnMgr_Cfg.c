...
extern SDmnMgrUsrCfg * DmnMgr_UserConfigurations(void) 
{
	...
	static const SDmnMgrUsrCfgList DmnMgrUsrConfig[MANAGED_DOMAINS_CNT] = 
	{
		{
			1,		/* GIP Domain ID */
			500,	/* The Periodicity at which HB Msgs from Domain is expected.*/
			3,		/*Acceptable HB Miss Count before DM initiate GIP Reset, FUSA req */
			35000,	/*Maximum time to wait for first HB from Domain */
			...
			10000,	/*Maximum time to wait for UCL Link Up */
...
void DmnMgr_ReportGipHealthError(uint16 FaultID)
{
	uint8 logData[5] = {0xAA, 0xAA, 0xAA, 0xAA, 0xAA};
	logData[0] = EDomainId_Cluster;
	
	OsTrustedCall_FltM_ReportFault(FaultID, FLTM_ACTIVE, logData, 5);
}
/* 
#-------------------------------------------------------------------------------------------------------------------
