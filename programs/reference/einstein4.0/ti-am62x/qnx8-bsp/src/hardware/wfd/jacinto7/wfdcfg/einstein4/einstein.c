...
#define DSS0_VP1_OLDI_CFG                   0x3020A160
#define DSS0_VP1_OLDI_CFG_VAL               0x0000190d
#define DSS_OLDI_CFG_REG_SIZE               4
...
#if 1
/* In QNX71, OLDI CFG register is set by screen driver. */
static void oldi_cfg_set_value(void)
{
	uint32_t *oldi_cfg_regptr = NULL;
	if ((oldi_cfg_regptr = mmap_device_memory(0, DSS_OLDI_CFG_REG_SIZE,
			PROT_READ|PROT_WRITE|PROT_NOCACHE, MAP_SHARED, DSS0_VP1_OLDI_CFG)) == 0) {
		SLOG_ERROR("failed to map DSS0_VP1_OLDI_CFG registers");
		return;
	}
	SLOG_INFO("%s: Reg 0x%08x, READ val 0x%08x", __func__, DSS0_VP1_OLDI_CFG, *oldi_cfg_regptr);
	*oldi_cfg_regptr = 0x0000190d;
	SLOG_INFO("%s: Reg 0x%08x, WRITE val 0x%08x", __func__, DSS0_VP1_OLDI_CFG, *oldi_cfg_regptr);
	if (oldi_cfg_regptr) {
		munmap_device_memory(oldi_cfg_regptr, DSS_OLDI_CFG_REG_SIZE);
	}
	return;
}
#endif
