...
/* SDIO 1.8V enable pin for SDMMC peripheral */
#define EVM_SDIO_1V8_EN_PIN_NUM        (31U)  /* GPIO0_31 */  
/* CPSW RGMII interrupt pin */
#define EVM_CPSW_RGMII_INT_PIN_NUM     (89U)  /* GPIO0_89*/  
...
static const uint16_t hdmi_pins[] = {
    ...
    PIN_MMC1_DAT0,      //offset 0x0230,
    PIN_MMC1_SDCD,      //offset 0x0240,
};

static const uint16_t oldi_pins[] = {
    PIN_VOUT0_DATA7,    //offset 0x00d4, DISP_RESET
    PIN_MMC1_DAT0,      //offset 0x0230, DISP_STBY
    //PIN_MMC1_SDCD,      //offset 0x0240, ERR_DET
    PIN_MCASP0_AXR1,    //offset 0x019C, PWM_DIMMING
    //PIN_WKUP_CLKOUT0,   //offset 0x0084, TFT_BL_EN
 };
...
static int init_hdmi(void)
{
	...
	for (pin = 0; pin < sizeof(hdmi_pins) / sizeof(uint16_t); pin++) {
		...
		kprintf("%s: reg=0x%x val=0x%x\n", __func__, (uint32_t)AM62P_CTRLMMR_PADCONFIG + hdmi_pins[pin], hdmi_pincfg);
...
static int init_oldi(void)
{
    uint8_t     c_state;
	uint32_t    oldi_pixel_freq = 89456000;
	uint32_t    oldi_pincfg = PIN_OUTPUT | PIN_MODE(7);
	uint8_t     pin;

	// Pinmux OLDI
	kprintf("%s: Start to set pin-mux for OLDI0\n", __func__);

	for (pin = 0; pin < sizeof(oldi_pins) / sizeof(uint16_t); pin++) {
		kprintf("%s: reg=0x%x val=0x%x\n", __func__, (uint32_t)AM62P_CTRLMMR_PADCONFIG + oldi_pins[pin], oldi_pincfg);
		out32(((uint32_t)AM62P_CTRLMMR_PADCONFIG + oldi_pins[pin]), oldi_pincfg);
	}
	
	if (ti_sci_cmd_clk_set_freq(TISCI_DEV_DSS0, TISCI_DEV_DSS0_DPI_0_IN_CLK, 
			oldi_pixel_freq * 7, oldi_pixel_freq * 7, oldi_pixel_freq * 7) == -1)
	{
		  kprintf("%s: OLDI: ti_sci_cmd_clk_set_freq() failed!\n", __func__);
		  return -1;
	}

	if (ti_sci_set_clock_state(TISCI_DEV_DSS0, TISCI_DEV_DSS0_DPI_0_IN_CLK,
			0, MSG_CLOCK_SW_STATE_REQ) == -1)
	{
		  kprintf("%s: OLDI: ti_sci_set_clock_state() failed!\n", __func__);
		  return -1;
	}
	kprintf("%s: Start to set pin-mux for OLDI0 completed\n", __func__);

    
    // Enable TISCI_DEV_OLDI0_VD VD power
    if (ti_sci_get_device_state(TISCI_DEV_OLDI0_VD, NULL, NULL, NULL, &c_state) == -1) {
        kprintf("%s: get_device_state failed!\n", __func__);
        return -1;
    }

    if (c_state == MSG_DEVICE_HW_STATE_OFF) {
        kprintf("no TISCI_DEV_OLDI0_VD power: trying to turn it on\n");
        if (ti_sci_set_device_state(TISCI_DEV_OLDI0_VD, 0, MSG_DEVICE_SW_STATE_ON) == -1) {
            kprintf("%s: ti_sci_set_device_state failed!\n", __func__);
            return -1;
        }
    }

    // Enable TISCI_DEV_OLDI1_VD VD power
    if (ti_sci_get_device_state(TISCI_DEV_OLDI1_VD, NULL, NULL, NULL, &c_state) == -1) {
        kprintf("%s: get_device_state failed!\n", __func__);
        return -1;
    }

    if (c_state == MSG_DEVICE_HW_STATE_OFF) {
        kprintf("no TISCI_DEV_OLDI1_VD power: trying to turn it on\n");
        if (ti_sci_set_device_state(TISCI_DEV_OLDI1_VD, 0, MSG_DEVICE_SW_STATE_ON) == -1) {
            kprintf("%s: ti_sci_set_device_state failed!\n", __func__);
            return -1;
        }
    }
...
/* Board specific pinmux configuration function*/  
int ti_init_am62p_evm_pads(void)
{
    uint8_t     c_state;
    uint32_t    oldi_pixel_freq = 89456000;  //oldi
	uint32_t    oldi_pincfg = PIN_OUTPUT | PIN_MODE(7); //oldi
	uint8_t     pin; 


/*Initializing and configuring peripherals*/
...
/* cpswg initialization */
    ...
    // TODO: Clear up if this pin is used by VP or GP
    // out32(AM62P_CTRLMMR_PADCONFIG + PIN_RGMII2_TXC, ((PIN_PULL_DISABLE | PIN_INPUT_ENABLE) & (~PIN_PULL_DIRECTION)) | PIN_MODE(0)); /* ENET_RESET_MON */
    
    
    out32(AM62P_CTRLMMR_PADCONFIG + PIN_RGMII2_TD2, PIN_PULL_DISABLE | PIN_MODE(7));  /* DO_ENET_EN */
    out32(AM62P_GPIO0_BASE + AM62P_GPIO_DIR,
        in32(AM62P_GPIO0_BASE + AM62P_GPIO_DIR) | ~(AM62P_GPIO_BIT(91U)));
    // Default signal output low
    out32(AM62P_GPIO0_BASE + AM62P_GPIO_SET_DATA, ~((unsigned int)(AM62P_GPIO_BIT(91U))) );
    

    out32(AM62P_CTRLMMR_PADCONFIG + PIN_RGMII2_TX_CTL, PIN_PULL_DISABLE | PIN_MODE(7)); /* DO_ENET_RSTB */
    out32(AM62P_GPIO0_BASE + AM62P_GPIO_DIR,
        in32(AM62P_GPIO0_BASE + AM62P_GPIO_DIR) | ~(AM62P_GPIO_BIT(87U)));
    // Default signal output high
    out32(AM62P_GPIO0_BASE + AM62P_GPIO_SET_DATA, (unsigned int)(AM62P_GPIO_BIT(87U)) );
    

    /* RGMII2_TD0 */    
    out32(AM62P_CTRLMMR_PADCONFIG + PIN_RGMII2_TD0, PIN_PULL_DISABLE | PIN_MODE(7)); /*DI_ENET_MII_INT*/     
    // GPIO0_89 as input-> the interrupt will have to be enabled for this pad
    out32(AM62P_GPIO0_BASE + AM62P_GPIO_DIR,
        in32(AM62P_GPIO0_BASE + AM62P_GPIO_DIR) | (AM62P_GPIO_BIT(EVM_CPSW_RGMII_INT_PIN_NUM)));
    // Default signal output high
    out32(AM62P_GPIO0_BASE + AM62P_GPIO_SET_DATA, (unsigned int)(AM62P_GPIO_BIT(EVM_CPSW_RGMII_INT_PIN_NUM)));
...
hw_init(void)
{
    ...
    /* Configure Pins for AM62P soc */
    #if TI_AM62P_E4_ENABLED
        ti_init_am62p_evm_pads();
		init_usb_host(0);
    #else
	init_i2c();
	init_gpio_am62p();
