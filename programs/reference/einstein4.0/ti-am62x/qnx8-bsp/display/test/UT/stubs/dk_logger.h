...
void log ( DltContext *const context, const DltLogLevelType loglevel, const T value, const TArgs... args )
{
    ...
    const bool init_status = inst->init ( loglevel, context );

    if((true == inst->getEnabled()) && (true == init_status))
    {
        ss << getpid();
	    st << gettid();
        const std::string pidInfo = "(" + ss.str() + "|"  + st.str() + ") ";
        inst->append ( pidInfo );
...
template <typename T, typename... TArgs>
void logDebug ( DltContext *const pContext, const T value, const TArgs... args )
{
    log ( pContext, DLT_LOG_DEBUG, value, args... );
}

template <typename T, typename... TArgs>
void logInfo ( DltContext *const pContext, const T value, const TArgs... args )
{
    log ( pContext, DLT_LOG_INFO, value, args... );
}

template <typename T, typename... TArgs>
void logError ( DltContext *const pContext, const T value, const TArgs... args )
{
    log ( pContext, DLT_LOG_ERROR, value, args... );
}

template <typename T, typename... TArgs>
void logWarning ( DltContext *const pContext, const T value, const TArgs... args )
{
    log ( pContext, DLT_LOG_WARN, value, args... );
}

template <typename T, typename... TArgs>
void logVerbose ( DltContext *const pContext, const T value, const TArgs... args )
{
...
#define LOG_REGISTER_APP_FILE(NAME, DESC, FILE) dk_Logger_createInstance(NAME, DESC, TRUE, eLogDestination_File, FALSE, FILE, FALSE)  
