...
typedef struct
{
    ...
    NvM_BlockTestResultType     NvMBlockTestResult[NVM_NUM_OF_BLOCKS];
...
volatile uint16 t_blockId_u16 = 0xFFFF;
...
void NvM_TestFunction(void)
{
    ...
    switch(NvM_TestStateMachine)
    {
        ...
        case NVM_TEST_INITIATE:
            NvM_TestDataStruct.CurrentTestBlockId = 2;
            MemLib_MemSet((uint8*)(&(NvM_TestWriteBuffer[0])), 0x28, NVM_WRITE_VERIFICATION_DATASIZE);
        ...
        case NVM_TEST_COMPLETE:
            ...
            if(NvM_TestDataStruct.NvMTestFailiureCount > 0)
            {
