...
Coverity_Summary_vip_html = local_paths['Coverity_Summary_vip_html']
Coverity_Summary_vip=local_paths['Coverity_Summary_vip']
Coverity_high=local_paths['Coverity_high']
Coverity_med=local_paths['Coverity_med']
Coverity_low=local_paths['Coverity_low']
Coverity_tech=local_paths['Coverity_all']
...
def create_html_table(csvfilepath,core):
    ...
    with open(csvfilepath, 'r',encoding="UTF-8") as fp:
        ...
        if core == 'VIP':
            ...
            html_table.append("<th>VIP</th>")
            html_table.append("<th colspan=\"3\">STATIC</th>")
            html_table.append("<th colspan=\"3\">MISRA C 2012</th>")
            html_table.append("<th colspan=\"3\">CERT C</th>")
...
def convert_to_html(core):
    html_table = create_html_table(Coverity_Summary_vip,core)
    with open(Coverity_Summary_vip_html, 'w') as html_obj:
        html_obj.write(create_style_sheet())
        if core == 'VIP':
            html_obj.write("<h1>Coverity Summary Report</h1>")
            html_obj.write("<hr>")
            html_obj.write("<h2>VIP Impact Summary Report</h1>")
            html_obj.write("<hr>")
        if core == 'GIP':
...
def parse_coverity_data(mode):
    ...
    for key in coverity_key_dict.keys():
            ...
            for comp in comp_list:
                ...
                high = len(specific_comp[specific_comp['Impact'] == "High"]) 
                medium = len(specific_comp[specific_comp['Impact'] == "Medium"]) 
                low = len(specific_comp[specific_comp['Impact'] == "Low"]) 
                misra_high = len(specific_comp[specific_comp[misra_category_str] == "Mandatory"]) 
                misra_medium = len(specific_comp[specific_comp[misra_category_str] == "Required"]) 
                misra_low = len(specific_comp[specific_comp[misra_category_str] == "Advisory"]) 
                #cert_high = len(specific_comp[specific_comp[cert_category_str] == "L1"]) 
                #cert_medium = len(specific_comp[specific_comp[cert_category_str] == "L2"]) 
                #cert_low = len(specific_comp[specific_comp[cert_category_str] == "L3"])
                if mode!="HSM":            
                ...
                coverity_data_row.append(high)
                coverity_data_row.append(medium)
                coverity_data_row.append(low)
                coverity_data_row.append(misra_high)
                coverity_data_row.append(misra_medium)
                coverity_data_row.append(misra_low)
                #coverity_data_row.append(cert_high)
                #coverity_data_row.append(cert_medium)
                #coverity_data_row.append(cert_low)
                #if mode!="HSM":
    ...
    coverity_df = pd.DataFrame(coverity_data_list, columns = ['Coverity Rule', 'Component','High', 'Medium', 'Low','Mandatory','Required','Advisory'])
    # Filtering VIP STATIC
    vip_static = coverity_df.loc[(coverity_df['Coverity Rule'] == 'STATIC')]
    # Removing Coverity Rule column
    vip_static = vip_static[['Component', 'High','Medium','Low']]
    vip_static = vip_static.rename(columns={'High':'SHigh','Medium':'SMedium','Low':'SLow'})

    # Filtering VIP/GIP MISRA C 2012 and GIP MISRA CPP 2008
    vip_misra = coverity_df.loc[(coverity_df['Coverity Rule'] == 'MISRA C 2012') | (coverity_df['Coverity Rule'] == 'MISRA CPP 2008')]
    # Removing Coverity Rule column
    vip_misra = vip_misra[['Component','Mandatory','Required','Advisory']]
    vip_misra = vip_misra.rename(columns={'Mandatory':'MHigh','Required':'MMedium','Advisory':'MLow'})

    # Filtering VIP CERT C
    #vip_cert = coverity_df.loc[(coverity_df['Coverity Rule'] == 'CERT C') | (coverity_df['Coverity Rule'] == 'CERT CPP')]
    # Removing Coverity Rule column
    #vip_cert = vip_cert[['Component','L1','L2','L3']]
    #vip_cert = vip_cert.rename(columns={'L1':'CHigh','L2':'CMedium','L3':'CLow'})
    ...
    vip_frames = [vip_static, vip_misra]
    #vip_frames = [vip_static, vip_misra, vip_his]
    vip_df = pd.concat(vip_frames, keys=["STATIC", "MISRA"])
    vip_df = vip_df.groupby(['Component'],as_index = False).agg({'SHigh': 'sum', 'SMedium': 'sum','SLow': 'sum','MHigh': 'sum', 'MMedium': 'sum','MLow': 'sum'})
    vip_df = vip_df.sort_values(by = ['Component'],ascending=True) 
    vip_df[['SHigh','SMedium','SLow','MHigh','MMedium','MLow']] = vip_df[['SHigh','SMedium','SLow','MHigh','MMedium','MLow']].astype(int)
    
    Third_party=[]
    Tech_team=[]

    for ind in vip_df.index:
        if vip_df['Component'][ind] in hmi:
            Tech_team.append("HMI")
        elif vip_df['Component'][ind] in di:
            Tech_team.append("DI-APPS")
        elif vip_df['Component'][ind] in infra:
            Tech_team.append("INFRA")
        else:
            Tech_team.append("AUTOSAR")
    
    #vip_df['Third-Party'] = Third_party
    vip_df['Tech Team'] = Tech_team
    return vip_df
    
x=[]
build=local_paths["coverity_stream"]
if "VIP" in build:
    df1=parse_coverity_data("VIP")
    x.append(df1)	
...
for i in range(len(f_df)):
    if f_df.iloc[i,7]=="AUTOSAR":
        a[0]+=f_df.iloc[i,1]+f_df.iloc[i,4]
        a[1]+=f_df.iloc[i,2]+f_df.iloc[i,5]
        a[2]+=f_df.iloc[i,3]+f_df.iloc[i,6]
    
    elif f_df.iloc[i,7]=="HMI":
        h[0]+=f_df.iloc[i,1]+f_df.iloc[i,4]
        h[1]+=f_df.iloc[i,2]+f_df.iloc[i,5]
        h[2]+=f_df.iloc[i,3]+f_df.iloc[i,6]
        
    elif f_df.iloc[i,7]=="DI-APPS":
        d[0]+=f_df.iloc[i,1]+f_df.iloc[i,4]
        d[1]+=f_df.iloc[i,2]+f_df.iloc[i,5]
        d[2]+=f_df.iloc[i,3]+f_df.iloc[i,6]
    
    elif f_df.iloc[i,7]=="INFRA":
        I[0]+=f_df.iloc[i,1]+f_df.iloc[i,4]
        I[1]+=f_df.iloc[i,2]+f_df.iloc[i,5]
        I[2]+=f_df.iloc[i,3]+f_df.iloc[i,6]
    """
    #if f_df.iloc[i,7]=="AUTOSAR":
...
f_df.to_csv(Coverity_Summary_vip,index=False)
