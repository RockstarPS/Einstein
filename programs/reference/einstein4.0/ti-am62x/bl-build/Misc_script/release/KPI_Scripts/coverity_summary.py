...
def parse_coverity_data(mode):
    ...
    vip_frames = [vip_static, vip_misra, vip_cert]
    #vip_frames = [vip_static, vip_misra, vip_his]
    vip_df = pd.concat(vip_frames, keys=["STATIC", "MISRA","CERT C"])
    vip_df = vip_df.groupby(['Component'],as_index = False).agg({'SHigh': 'sum', 'SMedium': 'sum','SLow': 'sum','MHigh': 'sum', 'MMedium': 'sum','MLow': 'sum', 'CHigh': 'sum', 'CMedium': 'sum','CLow': 'sum'})
    vip_df = vip_df.sort_values(by = ['Component'],ascending=True) 
    vip_df[['SHigh','SMedium','SLow','MHigh','MMedium','MLow','CHigh','CMedium','CLow']] = vip_df[['SHigh','SMedium','SLow','MHigh','MMedium','MLow','CHigh','CMedium','CLow']].astype(int)
...
if "VIP" in build:
    ...
    x.append(df1)
if "BL" in build:
	df2=parse_coverity_data("BL")
	x.append(df2)
if "HSM" in build:
	df3=parse_coverity_data("HSM")
	x.append(df3)
if "GIP" in build:
	df4=parse_coverity_data("GIP")
	x.append(df4)
if "VPCM4" in build:
	df5=parse_coverity_data("VPCM4")
	x.append(df5)		
