...
TRACY_API SymbolData DecodeSymbolAddress( uint64_t ptr );
TRACY_API SymbolData DecodeCodeAddress( uint64_t ptr );
TRACY_API const char* DecodeCallstackPtrFast( uint64_t ptr );
TRACY_API CallstackEntryData DecodeCallstackPtr( uint64_t ptr );
TRACY_API void InitCallstack();
