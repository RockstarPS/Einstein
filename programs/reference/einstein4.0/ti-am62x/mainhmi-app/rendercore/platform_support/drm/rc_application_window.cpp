		...
		drm_fb* get_fb_from_bo(struct gbm_bo* bo)
		{
			...
			if (fl_fb == nullptr)
			{
					...
					gbm_bo_get_stride_for_plane && gbm_bo_get_offset)
				{
					...
					if (num_planes > 4)
					{
						printf("gbm_bo_get_plane_count %d > 4\n", num_planes);
						fflush(stdout);
						num_planes = 4;
					}
...
namespace rendercore_engine
{
    ...
    bool rc_application_window::createNativeWindow(void)
	{
		...
		if (drm.fd >= 0)
		{
			...
			if (fl_resources != nullptr)
			{
				...
				for (int i = 0; i < fl_resources->count_connectors; i++)
				{
					...
					if (fl_connector->connection == DRM_MODE_CONNECTED)
					{
				...
				if (fl_connector != nullptr)
				{
					...
					if (drm.mode != nullptr)
					{
						...
						if (drm.crtc_id != 0)
						{
							...
							printf("DRM - native visual id / format = %d\n", GBM_FORMAT_XRGB8888);
							fflush(stdout);
							...
							if (drm.surface != nullptr)
							{
								printf("DRM - createNativeWindow Success\n");
								fflush(stdout);
								fl_sts = true;
							}
							else
							{
								printf("DRM - failed to create gbm surface\n");
								fflush(stdout);
								fl_sts = false;
							}
						}
					...
					{
						printf("DRM - could not find mode!\n");
						fflush(stdout);
						fl_sts = false;
					}
				}
				else
				{
					...
					// a connector..
					//printf("DRM - no connected connector!(device=%s)\n", device);
					printf("DRM - no connected connector!\n");
					fflush(stdout);
					fl_sts = false;
				}
			}
		...
		{
			printf("DRM - could not open drm device \n");
			fflush(stdout);
			fl_sts = false;
		}
		return(fl_sts);
			...
			else
			{
				if(firstframe)
				{
					...
					pthread_mutex_lock(mutex);
						...
						ret = drmModeSetPlane(drm.fd, 32, drm.crtc_id, drm.fb[current_buffer]->fb_id, 0, 0, 0, width, height, 0, 0, width << 16, height << 16);
						if (ret)
						{
							printf("Failed to set plane 32\n");
							fflush(stdout);
						}
						else
						{
...
Date              : 01-APRIL-2025
CDSID             : tgopala2
Traceability      : TFDCX32348-69218: To support print logs with DLT or syslog and remove unnecessary prints in logs
Change Description: Removed unwanted printfs
----------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------
Date              : 22-OCT-2025
CDSID             : braj
