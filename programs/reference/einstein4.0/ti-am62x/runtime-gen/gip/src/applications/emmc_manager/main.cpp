...
extern std::map<mid_t, func_t> oEmmcMgrComponentMessageMap;

//Instantiate Software Components
EmmcMgrComponent oEmmcMgrComponent(&oEmmcMgrComponentMessageMap, COMPID_OEMMCMGRCOMPONENT_STR);

//Define Messages
static const uint16_t oEmmcMgrComponentMessageArraySize = 0U;
const MsgCmn oEmmcMgrComponentMessageArray[oEmmcMgrComponentMessageArraySize] =
{

};

//Define Message Map for Components
std::map<mid_t, func_t> oEmmcMgrComponentMessageMap = createMap(oEmmcMgrComponentMessageArraySize, oEmmcMgrComponentMessageArray);

//Instantiate Containter
Container EmmcMgrCon(&oEmmcMgrComponent);

//Instantiate Application
EmmcManager gApp("EmmcManager", &EmmcMgrCon);

//Instantiate Threads
#define PTH_BODY
#include "emmc_manager_threads.h"
#undef PTH_BODY
...
int main()
{
    ...
    try
    {
        ...
        #ifdef __QNX__
         LOG_REGISTER_APP_SLOG2("EMGR", "EMMC Manager");
         LOG_REGISTER_CONTEXT ( gRTELogContext, "RTE", "Runtime Context", DLT_LOG_INFO );
        #else
         LOG_REGISTER_APP_DLT("EMGR", "EMMC Manager");
         LOG_REGISTER_CONTEXT ( gRTELogContext, "RTE", "Runtime Context", DLT_LOG_INFO );
        #endif
        /// End of user code
