...
void CUdsServiceEcuResetHandler::serviceInitialize(void)
{
    ...
    mEcuResetServiceConfig.mSupportedSessionMask = ESessionMask_t::ESessionMask_FotaSession;
    ...
    mEcuResetServiceConfig.mPostResponseHandlerCbkFn = [=](const uint16_t clientAddress) {ecuResetPostresponseHandler(clientAddress);} ;
    mEcuResetServiceConfig.mResetServiceHandlerCbkFn = [=](const uint16_t clientAddress) {ecuResetResetHandler(clientAddress);} ;
...
void CUdsServiceEcuResetHandler::ecuResetRequestHandler(const uint16_t clientAddress, const uint8_t * const pRequestData,uint16_t requestLength)
{
    ...
    else
    {
        ...
        mpServiceManagerInstance.serviceResponseUpdate(clientAddress,	responseType,
				subfunction);
    }
}
