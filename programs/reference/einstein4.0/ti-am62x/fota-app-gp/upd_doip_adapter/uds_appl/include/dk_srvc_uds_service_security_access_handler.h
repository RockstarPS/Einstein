...
///
#ifndef DK_SRVC_UDS_SERVICE_SECURITY_ACCESS_HANDLER_H
#define DK_SRVC_UDS_SERVICE_SECURITY_ACCESS_HANDLER_H
...
public:
    ...
    ///
    explicit CUdsServiceSecurityAccessHandler(CUdsServiceManagerIf &serviceManagerInstance) : mpServiceManagerInstance(serviceManagerInstance)
	{
	}
    ...
    static void diagHardeningdiagTunnelCallback(void* instancePointer, uint8_t result);
    

    void diagHardeningOnStart(void);

private:

    void diagHardeningInitializeVirginFlag(void);
    void diagHardeningInitializeSerialNumber(void);
    ...
    typedef enum
    {
        ESecurityAccessState_WaitForSeed = 0u,	///< It defines the wait for seed state. In this state the request seed sub function is processed.
        ESecurityAccessState_WaitForKey,		///< It defines the wait for key state. In this state the verify key sub function is processed.
    ...
    typedef struct
    {
		...
		CUdsServiceHandlerIf::ESecurityMask_t mSupportedSecurityMask;			///< Supported security mask of security level.
    ...
    ///
    /// @brief Buffer for the encrypted seed.
    ///
    uint8_t mEncryptedSeed[256];
    ...
    ///
    /// @brief Generated random seed
    ///
    uint8_t mRandomSeed[86];
    ...
    ///
    CUdsServiceHandlerIf::EResponseCode_t processSeedRequest (const uint16_t clientAddress, CUdsServiceHandlerIf::ESecurityLevel_t newSecurityLevel, uint8_t securitySubFunction );

    uint8_t DoIpSec_GenerateSecretSeed(uint8_t *MsgBuff);

    void DoIpSec_ScrambleRandom(uint32_t *pScramble);

    uint8_t DoIpSec_ReadSecretCounter(uint8_t *MsgBuff, uint16_t BuffSize);

    uint8_t DoIpSec_IsMaxValueNotReached(const uint8_t *MsgBuff, uint16_t CounterSize);

    uint8_t DoIpSec_WriteSecretCounter(const uint8_t *MsgBuff, uint16_t BuffSize);
