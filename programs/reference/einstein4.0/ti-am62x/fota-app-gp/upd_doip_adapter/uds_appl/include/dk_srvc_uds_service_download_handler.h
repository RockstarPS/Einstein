...
public:
	...
	const std::string gipFilePath = "/fota_cache/gip.iso";

	const std::string gipDeltaFilePath = "/fota_cache/gip_delta";
	...
	int32_t extractGipFile(std::string file, std::string cache);

	gipVersionInfo extractVersionFromFilename(const std::string& filename);
	...
	/// @brief This method is used to set the temporary file path used for storing the request file.
	///
	/// @param[in] status - Installation status
	///
	void installerFlashStatus(EInstallationStatus_t_t status);
...
private:
	...
	/* 1160 mb is the size of the fota_cache partition */
	const uint32_t  mUdsDownloadMaxFileSize = 1160*1024*1024U;
	///
	...
	const uint32_t mUdsDownloadBlockSize = 4*1024 + 2U;
	...
	///
	/// @brief It defines the temporary file handle for the currently requested file.
	///
	std::FILE *mDownloadFile;
	...
	///
	/// @brief It defines the RequestDownload (0x34) service configurations values.This value should be configured
	///         at serviceInitialize and should be passed to CUdsServiceManager to register the configuration.
	///
	...
	/// @param[in] requestLength  size of the requested data.
	///
	void requestDownloadServiceRequestHandler(const uint16_t clientAddress,
			const uint8_t * const pRequestData, const uint16_t requestLength);
	...
	/// @param[in] clientAddress requested client address.
	///
	void requestDownloadServicePostresponseHandler(
