...
/**************************************************************************
Function name       : void Ea_Init(const Ea_ConfigType* ConfigPtr)
...
{
    /* As per AR, EA shall perform init only when NULL pointer is passed */
    if(ConfigPtr == NULL)
    {
        uint8 Count;

        MemLib_MemSet(&Ea_Descriptor, 0, sizeof(Ea_Descriptor_Type));
        /* Set the state to Internal Busy to indicate the start of Init */
        Ea_Descriptor.state = MEMIF_BUSY_INTERNAL;
        ...
        /* Init is completed, set the state to IDLE */
        Ea_Descriptor.state = MEMIF_IDLE;
    }
}
...
Std_ReturnType Ea_Read(uint16 BlockNumber, uint16 BlockOffset, uint8* DataBufferPtr, uint16 Length)
{
    ...
    retVal = E_NOT_OK ;
    /* Ensure that Init is completed */
    if(Ea_Descriptor.state != MEMIF_UNINIT)
    {
        /* JobActiveId is set to IDLE after the Init is started
        * this is equivalant to check of state MEMIF_IDLE or MEMIF_BUSY_INTERNAL */
        if (Ea_Descriptor.JobActiveId == EA_JOB_IDLE)
        {
            ...
            /* Job is accepted, set the state to Busy */
            Ea_Descriptor.state         = MEMIF_BUSY;
            retVal = E_OK;
        }
    }
...
{
    /* Return the state, state is properly updated in all places */
   return (Ea_Descriptor.state);
}
...
void Ea_JobErrorNotification(void)
{
    ...
    if (Ea_JobEndNote_Flg==1)
    {
        ...
        Ea_Descriptor.Eep_JobResult = MEMIF_JOB_FAILED;
...
/*By                : kapalaniv                                             */
/*Traceability      :                                                       */
/*Change Description: EA 4.3 MIGRATION                                      */
/*--------------------------------------------------------------------------*/
