...
* purpose, critical section, unit, and resolution                            *
******************************************************************************/
#if (CRC_CALCULATECRC8_ENABLE == CRC_ENABLE)
...
#endif
#if (CRC_CALCULATECRC16_ENABLE == CRC_ENABLE)
...
#endif
#if (CRC_CALCULATECRC32_ENABLE == CRC_ENABLE)
...
#endif
#if (CRC_CALCULATECRC8H2F_ENABLE == CRC_ENABLE)
...
#endif
#if (CRC_CALCULATECRC32P4_ENABLE == CRC_ENABLE)
...
uint32 Crc_CalculateCRC32(const uint8 *Crc_DataPtr, uint32 Crc_Length, uint32 Crc_StartValue32, boolean Crc_IsFirstCall)
{
    ...
    if (NULL_PTR != Crc_DataPtr)
	{
		while ( fl_CrcLength_u32 > 0u )
		{
			Crc_Value32 ^= 0xFFuL & (uint32) Crc_DataPtr[DataPtrAddr_u16];
			Crc_Value32 = ( Crc_Value32 >> 8 ) ^ ( Crc_Table32bit [ (uint8)(Crc_Value32 )]);
			fl_CrcLength_u32--;
			DataPtrAddr_u16++;
		}
	}
    return CRC_FINAL_XOR_CRC32 ^ Crc_Value32;
}
#endif
