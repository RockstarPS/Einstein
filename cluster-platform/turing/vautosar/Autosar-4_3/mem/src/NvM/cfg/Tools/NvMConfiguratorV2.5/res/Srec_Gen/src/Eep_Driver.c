...
void Eep24xx_Init(const EepDrv_ConfigType* ConfigPtr)
{
    uint32 idx;
    for(idx = 0; idx < MAXIMUM_DFLASH_SIZE; idx++)
    {
...
Std_ReturnType Eep24xx_Read(EepDrv_AddressType EepromAddress, uint8* DataBufferPtr,EepDrv_LengthType Length)
{
    uint8  *targetAdd = (uint8 *)DataBufferPtr;
    uint32 idx;
    if(targetAdd != NULL)
...
Std_ReturnType Eep24xx_Write(EepDrv_AddressType EepromAddress,const uint8* DataBufferPtr,EepDrv_LengthType Length)
{
    uint32  idx;
   boolean found;
...
Std_ReturnType Eep24xx_Erase(EepDrv_AddressType EepromAddress, EepDrv_LengthType Length)
{
    uint32 idx;
   for(idx = EepromAddress; idx < (EepromAddress + Length); idx++)
   {
...
Std_ReturnType Eep24xx_Compare(EepDrv_AddressType EepromAddress,const uint8* DataBufferPtr,EepDrv_LengthType Length)
{
    Std_ReturnType LenReturnValue = E_OK;
   uint8  *targetAdd =DataBufferPtr;
   uint32 idx;
