...
******************************************************************************/
#define NV_WRITE_BLOCK                      0x07u
#define NV_READ_BLOCK                       0x06u
#define NV_INVALIDATE_BLOCK                 0x0Bu
#define NV_CANCEL_BLOCK                     0x0Au
#define NV_RESTORE_BLOCK_DEFAULTS           0x08u

#define NV_ERASE_BLOCK                      0x09u

#define NV_WRITE_PRAM_BLOCK                 0x17u
#define NV_READ_PRAM_BLOCK                  0x16u
#define NV_RESTORE_PRAM_BLOCK_DEFAULTS      0x18u

#define NV_READALL                          0x0Cu
#define NV_WRITEALL                         0x0Du
#define NV_VALIDATEALL                      0x19u
#define NV_FIRSTINITALL                     0x14u
...
typedef enum
{
    ...
    NVM_JOB_SUBSTATE_PROCESS_FIRSTINIT_ALL,
...
static void NvM_ProcessFirstInitAll(void);
...
static boolean NvM_CheckQueueIsFull(uint16 BlockId);
...
#endif
static Std_ReturnType NvM_CheckForHighPrioRead(uint8 fl_NextPrio);
...
/**************************************************************************
Function name   : void NvM_Init(const NvM_ConfigType* ConfigPtr)
Arguments       : void
Return type     : void
...
{
    /* As per AR, NvM shall Init only when NULL pointer is passed */
    if(ConfigPtr == NULL)
    {
        uint16 fl_iteration;
        ...
        for(fl_iteration=0U; fl_iteration< (NVM_NUM_OF_BLOCKS+1U); fl_iteration++)
        {
            NvM_AdminBlock[fl_iteration].BlockModifiedStatus = (boolean)0U;
            NvM_AdminBlock[fl_iteration].BlockProcessStatus = 0U;
            NvM_AdminBlock[fl_iteration].BlockValidStatus = (boolean)0U;
...
Std_ReturnType NvM_SetRamBlockStatus( NvM_BlockIdType BlockId, boolean BlockChanged)
{
    ...
    if(BlockId <= NVM_NUM_OF_BLOCKS)
    {
        ...
        (
            ...
            (FALSE == (uint8)(NvM_BlockConfig[BlockId].BlockUseSetRamBlockStatus)) ||\
            (NVM_REQ_PENDING == NvM_AdminBlock[BlockId].BlockProcessStatus) 
        )
        {
            fl_RetVal = E_NOT_OK;
...
Std_ReturnType NvM_RestoreBlockDefaults( NvM_BlockIdType BlockId, void* NvM_DestPtr)
{
    ...
    if(BlockId <= NVM_NUM_OF_BLOCKS)
    {
        if(NvM_AdminBlock[BlockId].BlockProcessStatus != NVM_REQ_PENDING)
        {
            ...
            (
                (FALSE != NvM_AdminBlock[BlockId].BlockWriteProt)    || \
                (FALSE != ((uint8)NvM_AdminBlock[BlockId].BlockLockedStatus))  || \
                (NvM_CheckQueueIsFull(BlockId)) || \
...
Std_ReturnType NvM_InvalidateNvBlock(NvM_BlockIdType BlockId)
{
    ...
    if(BlockId <= NVM_NUM_OF_BLOCKS)
    {
        ...
        (
            (FALSE != NvM_AdminBlock[BlockId].BlockWriteProt)    || \
            (FALSE != ((boolean)NvM_AdminBlock[BlockId].BlockLockedStatus))  || \
            (NvM_CheckQueueIsFull(BlockId)) || \
            ((NVM_DATASET_BLOCK == NvM_BlockConfig[BlockId].BlockManagementType) && (NvM_AdminBlock[BlockId].DataSetIndex >= NvM_BlockConfig[BlockId].NvBlockNum))
        )
        {
...
Std_ReturnType NvM_ReadPRAMBlock(NvM_BlockIdType BlockId)
{
    ...
    if(BlockId <= NVM_NUM_OF_BLOCKS)
    {
        ...
        else
        {
            ...
            {
                NvM_JobQueueEntity.BlockId = BlockId;
                /* update the request type with service ID */
                NvM_JobQueueEntity.RequestType = NV_READ_PRAM_BLOCK;
                NvM_JobQueueEntity.DataPointer = NvM_BlockConfig[BlockId].RamBlockDataAddress;
                NvM_JobQueueEntity.ReadLength = NvM_BlockConfig[BlockId].NvBlockLength;
                NvM_RetVal = E_OK;
                NvM_EnqueueRequestedJob();
    ...
    if(BlockId <= NVM_NUM_OF_BLOCKS)
    {
        ...
        else
        {
            ...
            else
            {
                NvM_JobQueueEntity.BlockId = BlockId;
                /* update the request type with service ID */
                NvM_JobQueueEntity.RequestType = NV_WRITE_PRAM_BLOCK;
                NvM_JobQueueEntity.DataPointer = NvM_BlockConfig[BlockId].RamBlockDataAddress;
                NvM_JobQueueEntity.ReadLength = NvM_BlockConfig[BlockId].NvBlockLength;
                NvM_RetVal = E_OK;
                NvM_EnqueueRequestedJob();
...
Std_ReturnType NvM_RestorePRAMBlockDefaults(NvM_BlockIdType BlockId)
{
    ...
    if(BlockId <= NVM_NUM_OF_BLOCKS)
    {
        if(NvM_AdminBlock[BlockId].BlockProcessStatus != NVM_REQ_PENDING)
        {
            if
            (
                (FALSE != (uint8)(NvM_AdminBlock[BlockId].BlockWriteProt))    || \
                (FALSE != NvM_AdminBlock[BlockId].BlockLockedStatus)  || \
                (NvM_CheckQueueIsFull(BlockId)) || \
...
/**************************************************************************
Function name   : Std_ReturnType NvM_GetActiveService( uint8* ServiceIdPtr )
Arguments       : ServiceIdPtr : Currently active service of the NvM module.
Return type     : Std_ReturnType
                   E_OK: Successfully retrieved the currently active service 
                   E_NOT_OK: Retrieval of currently active service failed
Description     : Function used to retrieve the currently active service of the NvM module.
***********************************************************************/
Std_ReturnType NvM_GetActiveService(uint8* ServiceIdPtr)
{
    Std_ReturnType NvM_RetVal = E_NOT_OK;
    /* ServiceIdPtr is Valid */
    if(ServiceIdPtr != NULL)
    {
        NvM_RetVal = E_OK;
        switch(NvM_DataStruct.NvMState)
        {
            /* NvM is not yet initialized */
            case NVM_STATE_UNINIT:
                *ServiceIdPtr = NVM_NO_SERVICE;
                NvM_RetVal = E_NOT_OK;
                /*Report to DET*/
            break;
            
            /* Write action is in progress */
            case NVM_STATE_WRITE_BLOCK:
                if(NvM_CurrentJob.RequestType == NV_WRITE_BLOCK)
                {
                    *ServiceIdPtr = NVM_WRITE_BLOCK;
                }
                else
                {
                    *ServiceIdPtr = NVM_WRITE_PRAM_BLOCK;
                }                   
            break;
            
            /* Read action is in progress */
            case NVM_STATE_READ_BLOCK:
                if(NvM_CurrentJob.RequestType == NV_READ_BLOCK)
                {
                    *ServiceIdPtr = NVM_READ_BLOCK;
                }
                else
                {
                    *ServiceIdPtr = NVM_READ_PRAM_BLOCK;
                }
            break;
            
            /* Invalidation action is in progress */
            case NVM_STATE_INVALIDATE:
                *ServiceIdPtr = NVM_INVALIDATE_NV_BLOCK;
            break;
            
            /* Restore block action is in progress */
            case NVM_STATE_RESTORE_BLOCK_DEFAULTS:
                if(NvM_CurrentJob.RequestType == NV_RESTORE_BLOCK_DEFAULTS)
                {
                    *ServiceIdPtr = NVM_RESTORE_BLOCK_DEFAULTS;
                }
                else
                {
                    *ServiceIdPtr = NVM_RESTORE_PRAM_BLOCK_DEFAULTS;
                }
            break;
            ...
            /* Write all (multi block)action is in progress */
            case NVM_STATE_WRITE_ALL:
                *ServiceIdPtr = NVM_WRITE_ALL;
            break;
            
            /* read all (multi block)action is in progress */
            case NVM_STATE_READ_ALL:
                *ServiceIdPtr = NVM_READ_ALL;
            break;
            
            /* FirstInitAll all (multi block)action is in progress */
            case NVM_STATE_FIRSTINT_ALL:
                *ServiceIdPtr = NVM_FIRSTINIT_ALL;
            break;
...
static void NvM_CheckNewRequest(void)
{
    ...
    if((MEMIF_IDLE == MemIf_GetStatus(MEMIF_FEE)) && (MEMIF_IDLE == MemIf_GetStatus(MEMIF_EA)) && (MEMIF_IDLE == MemIf_GetStatus(MEMIF_SNVM))) 
    {
        if(NvM_DataStruct.NvMState == NVM_STATE_IDLE)
        {
                    ...
                    switch(NvM_CurrentJob.RequestType)
                    {
                        ...
                        case NV_WRITE_PRAM_BLOCK:
                            ...
                            if(NVM_REDUNDANT_BLOCK == NvM_BlockConfig[NvM_CurrentJob.BlockId].BlockManagementType)
                            {
                                ...
                                if(FALSE == ((boolean)NvM_AdminBlock[NvM_CurrentJob.BlockId].ValidNvIndex))
                                {
                        ...
                        case NV_RESTORE_PRAM_BLOCK_DEFAULTS:
                            NvM_DataStruct.NvMState = NVM_STATE_RESTORE_BLOCK_DEFAULTS;
                            NvM_CurrentJob.JobState = NVM_JOB_RESTORE_BLOCK_DEFAULTS;
                            NvM_CurrentJob.JobSubState = NVM_JOB_SUBSTATE_RESTORE_BLOCK_DEFAULTS;
                            NvM_CurrentJob.ProcessState = TRUE;
            ...
            {                
                if(NvM_DataStruct.FirstInitAllRequest == TRUE)
                {
                    NvM_CurrentJob.BlockId = 2U;
                    NvM_DataStruct.NvMState = NVM_STATE_FIRSTINT_ALL;
                    NvM_CurrentJob.JobSubState = NVM_JOB_SUBSTATE_PROCESS_FIRSTINIT_ALL;
                    NvM_CurrentJob.JobState = NVM_JOB_STATE_FIRSTINIT;
                    NvM_CurrentJob.ProcessState = TRUE;
                #ifdef MEM_TESTING_ENABLED
                    NVM_JOB_START_HOOK(NVM_FIRSTINIT_ALL,NvM_CurrentJob.BlockId);
                #endif
                }
                else if(NvM_DataStruct.WriteAllRequest == TRUE)
                {
                    NvM_CurrentJob.BlockId = 2U;
...
static void NvM_ProcessJobSubState(void)
{
    switch(NvM_CurrentJob.JobSubState)
    {
        ...
        case NVM_JOB_SUBSTATE_PROCESS_FIRSTINIT_ALL:
        NvM_ProcessFirstInitAll();
        break;
...
static void NvM_WriteCrcCalculate(void)
{
    uint32 CrcLength;
    uint16  Length;
    uint8 PreemptedJobStatus = E_NOT_OK;
#if (NVM_JOB_PRIORITIZATION == STD_ON)
    if((0u == NvM_GetNextHighPriority()) && (0u != NvM_BlockConfig[NvM_CurrentJob.BlockId].BlockJobPriority))
    {
        if ((FALSE == NvM_DataStruct.ReadAllRequest) && \
                (FALSE == NvM_DataStruct.WriteAllRequest))
        {
            PreemptedJobStatus = NvM_PreemptCurrentJob();
            if (E_OK == PreemptedJobStatus)
            {
                NvM_CurrentJob.JobSubState = NVM_JOB_SUBSTATE_IDLE;
                NvM_DataStruct.NvMState = NVM_STATE_IDLE;
                NvM_CurrentJob.JobState = NVM_JOB_STATE_IDLE;
...
#endif
    if(E_NOT_OK == PreemptedJobStatus)
    {
...
static void NvM_InitWrite(void)
{
    ...
    else
    {
        ...
        if(NvM_CurrentJob.JobState == NVM_JOB_STATE_FIRSTINIT)
        {
            JobId = NVM_FIRSTINIT_ALL;
        }
        ...
        {
            NvM_CurrentJob.DirtyNvIndex &= (uint16)~(0x01U << NvM_CurrentJob.CurrentNvIndex);
            /* Check for block type redundant */
            if((NVM_REDUNDANT_BLOCK == NvM_BlockConfig[NvM_CurrentJob.BlockId].BlockManagementType)
                &&(FALSE != (boolean)NvM_CurrentJob.DirtyNvIndex))
            {
                NvM_CurrentJob.JobSubState = NVM_JOB_SUBSTATE_WRITE;
                NvM_CurrentJob.ProcessState = TRUE;
                if(0x01U == NvM_CurrentJob.DirtyNvIndex)
                {
            ...
            {
                if(0xFFFFU != NvM_AdminBlock[NvM_CurrentJob.BlockId].ValidNvIndex)
                {
                ...
                else if (NvM_DataStruct.NvMState == NVM_STATE_FIRSTINT_ALL)
                {
                    if (NVM_MEMIF_SUSPEND_INTERNAL_JOB == NvM_DataStruct.MemIf_InternalJobState)
                    {
                        MemIf_ResumeInternalJob();
...
static void NvM_WriteCbkWait(void)
{
    ...
    else
    {
        ...
        NvM_CurrentJob.ProcessState = FALSE;
#if (NVM_JOB_PRIORITIZATION == STD_ON)
        if((NVM_NUM_OF_BLOCKS+1) > NvM_CurrentJob.BlockId)
        {
            if((0u == NvM_GetNextHighPriority()) && (0u != NvM_BlockConfig[NvM_CurrentJob.BlockId].BlockJobPriority))
            {
                if ((FALSE == NvM_DataStruct.ReadAllRequest) && \
...
static void NvM_WriteVerify(void)
{
    ...
    else
    {
        ...
        if(NvM_CurrentJob.ReadRetryCount > 0U)
        {
        ...
        if(E_NOT_OK == PreemptedJobStatus)
        {
            ...
            else
            {
                ...
                if(FALSE != Result)
                {
                    ...
                    /* Check for block type redundant */
                    if ((NVM_REDUNDANT_BLOCK == NvM_BlockConfig[NvM_CurrentJob.BlockId].BlockManagementType) &&
                        (((uint8)FALSE) != NvM_CurrentJob.DirtyNvIndex))
                    {
                        NvM_CurrentJob.JobSubState = NVM_JOB_SUBSTATE_WRITE;
                        NvM_CurrentJob.ProcessState = TRUE;
                        if(0x01U == NvM_CurrentJob.DirtyNvIndex)
...
/**************************************************************************
Function name   : void NvM_FirstInitAll(void)
Arguments       : void
Return type     : void
...
{
    NvM_DataStruct.FirstInitAllRequest = TRUE;
}

/**************************************************************************
Function name   : void NvM_ProcessFirstInitAll(void)
Arguments       : void
Return type     : void
Description     : Processes the Initialization
**************************************************************************/
static void NvM_ProcessFirstInitAll(void)
{
    /* Ensure that Frist Init all is requested */
    if(NvM_DataStruct.FirstInitAllRequest == TRUE)
    {
        /* Ensure valid block is selected */
        if(NvM_CurrentJob.BlockId < NVM_NUM_OF_BLOCKS)
        {
            /* Either ROM block is available or Init call back is configured */
            if(((NULL_PTR != NvM_BlockConfig[NvM_CurrentJob.BlockId].RomBlockDataAddress) || (NULL_PTR != NvM_BlockConfig[NvM_CurrentJob.BlockId].InitCallback)) 
                && (NULL_PTR != NvM_BlockConfig[NvM_CurrentJob.BlockId].RamBlockDataAddress))
            {
                ...
                NvM_CurrentJob.RamBuffer = NvM_BlockConfig[NvM_CurrentJob.BlockId].RamBlockDataAddress;
                NvM_CurrentJob.ReadLength = NvM_BlockConfig[NvM_CurrentJob.BlockId].NvBlockLength;
                NvM_CurrentJob.RequestType = NVM_FIRSTINIT_ALL;
                NvM_CurrentJob.WriteRetryCount = NvM_BlockConfig[NvM_CurrentJob.BlockId].MaxNumWriteRetries + (uint8)1u;
                NvM_CurrentJob.ReadRetryCount = NvM_BlockConfig[NvM_CurrentJob.BlockId].MaxNumReadRetries + (uint8)1u;
                NvM_CurrentJob.JobState = NVM_JOB_STATE_FIRSTINIT;
                NvM_CrcStruct.IsFirstCall = TRUE;
                ...
                switch(NvM_BlockConfig[NvM_CurrentJob.BlockId].BlockManagementType)
                {
                    case NVM_NATIVE_BLOCK:
                        /* ROM block is available, copy the date from ROM to PRAM and trigger write */
                        if(NULL_PTR != NvM_BlockConfig[NvM_CurrentJob.BlockId].RomBlockDataAddress)
                        {
                            MemLib_MemCpy(NvM_BlockConfig[NvM_CurrentJob.BlockId].RamBlockDataAddress, NvM_BlockConfig[NvM_CurrentJob.BlockId].RomBlockDataAddress, \
                                NvM_BlockConfig[NvM_CurrentJob.BlockId].NvBlockLength);                             
                        }
                        /* ROM block is not available but init callback is available */
                        /* SWC should copy the default values to PRAM and then NvM will trigger write */
                        else
                        {
                        ...
                        NvM_CurrentJob.CurrentNvIndex = 0U;
                    break;
                    /* Block is of type Redundant, restore the default value to PRAM and trigger write*/
                    /* Both the copies must be written and that is ensured using Dirty flag */
                    /* CAUTION: The scenario of 1st copy write fail and 0th copy success to be reworked */
                    case NVM_REDUNDANT_BLOCK:
                        /* ROM block is available, copy the date from ROM to PRAM and trigger write */
                        if(NULL_PTR != NvM_BlockConfig[NvM_CurrentJob.BlockId].RomBlockDataAddress)
                        {
                            MemLib_MemCpy(NvM_BlockConfig[NvM_CurrentJob.BlockId].RamBlockDataAddress, NvM_BlockConfig[NvM_CurrentJob.BlockId].RomBlockDataAddress, \
                    ...
                    /* Block is of type DataSet which is not supported for FirstInit all */
                    case NVM_DATASET_BLOCK:                 
                        NvM_CurrentJob.BlockId = NvM_CurrentJob.BlockId+1U;
                        NvM_CurrentJob.JobSubState = NVM_JOB_SUBSTATE_PROCESS_FIRSTINIT_ALL;
                        NvM_CurrentJob.ProcessState = TRUE;                     
                    break;
            ...
            /* No default data available for this block, move to next one */
            else
            {
                NvM_CurrentJob.BlockId = NvM_CurrentJob.BlockId+1U;
                NvM_CurrentJob.JobSubState = NVM_JOB_SUBSTATE_PROCESS_FIRSTINIT_ALL;
        ...
        /* All blocks processed, Reset the state meachine */
        else
        {
            NvM_DataStruct.FirstInitAllRequest = FALSE;
            NvM_DataStruct.NvMState = NVM_STATE_IDLE;
            NvM_CurrentJob.JobState = NVM_JOB_STATE_IDLE;
            NvM_CurrentJob.JobSubState = NVM_JOB_SUBSTATE_IDLE;
            NvM_CurrentJob.ProcessState = FALSE;
            NvM_MultiBlockCallbackFunction(NVM_FIRSTINIT_ALL,NVM_REQ_OK);
            if (NVM_MEMIF_SUSPEND_INTERNAL_JOB == NvM_DataStruct.MemIf_InternalJobState)
            {
                MemIf_ResumeInternalJob();
            }
        #ifdef MEM_TESTING_ENABLED
            NVM_JOB_END_HOOK(NVM_FIRSTINIT_ALL,NvM_CurrentJob.BlockId);
        #endif
        }
    }
    /* FirstInitAll is not active, this condition should not happen but state meachine is reset for fail safe */
    else
    {
        NvM_DataStruct.NvMState = NVM_STATE_IDLE;
        NvM_CurrentJob.JobState = NVM_JOB_STATE_IDLE;
        NvM_CurrentJob.JobSubState = NVM_JOB_SUBSTATE_IDLE;
        NvM_CurrentJob.ProcessState = FALSE;
        NvM_MultiBlockCallbackFunction(NVM_FIRSTINIT_ALL,NVM_REQ_NOT_OK);
        if (NVM_MEMIF_SUSPEND_INTERNAL_JOB == NvM_DataStruct.MemIf_InternalJobState)
        {
            MemIf_ResumeInternalJob();
        }
    ...
    else
    {
        ...
        if(FALSE != fl_RequestReadBlock)
        {
            if(NvM_CurrentJob.ReadRetryCount > 0U)
            {
                ...
                Length = NvM_CurrentJob.ReadLength + 4U;
            ...
            else
            {
                ...
                && (NvM_CurrentJob.CurrentNvIndex == 0U))
                {
                    NvM_CurrentJob.CurrentNvIndex = NvM_CurrentJob.CurrentNvIndex +1U;
                    NvM_CurrentJob.JobSubState    = NVM_JOB_SUBSTATE_READ;
                    NvM_CurrentJob.ProcessState   = TRUE;
                ...
                else
                {
                    ...
                    /* Actual job result info is updated if there is no default data available */
                    if((NvM_BlockConfig[NvM_CurrentJob.BlockId].RomBlockDataAddress == NULL_PTR) && (NvM_BlockConfig[NvM_CurrentJob.BlockId].InitCallback == NULL_PTR))
                    {
                        jobResult = NvM_CurrentJob.JobStatus;
                    }
                    ...
                    if (NVM_DATASET_BLOCK == NvM_BlockConfig[NvM_CurrentJob.BlockId].BlockManagementType)
                    {
                        if(NULL != NvM_BlockConfig[NvM_CurrentJob.BlockId].SingleBlockCallback)
                        {
                            (void) NvM_BlockConfig[NvM_CurrentJob.BlockId].SingleBlockCallback(JobId,jobResult);
                        }
                    }
                    else
                    {
                        ...
                        {                            
                            ...
                            /* Default data is available, update result to restore from ROM */
                    ...
                    NvM_AdminBlock[NvM_CurrentJob.BlockId].BlockProcessStatus = jobResult;
...
static void NvM_ReadCrcValidate(void)
{
    ...
    {
        /* when CRC does not match, then it is intigrity failed */
        NvM_CurrentJob.JobStatus = NVM_REQ_INTEGRITY_FAILED;
        NvM_CurrentJob.ReadRetryCount = 0U;
        NvM_CurrentJob.JobSubState    = NVM_JOB_SUBSTATE_READ;
        NvM_CurrentJob.ProcessState   = TRUE;
    }
...
{
    uint16 fl_temp;
    uint8 *fl_SrcPtr = SrcPtr;
    uint8 *fl_DestPtr = DestPtr;
    if((fl_SrcPtr != NULL) && (fl_DestPtr != NULL))
    {
        for(fl_temp = 0; fl_temp< length; fl_temp++)
        {
            *fl_DestPtr = *fl_SrcPtr;
            fl_DestPtr++;
            fl_SrcPtr++;
        }
    }
...
/**************************************************************************
Function name   : boolean NvM_CheckQueueIsFull(uint16 BlockId)
Arguments       : BlockId
Return type     : boolean
...
static void NvM_EnqueueRequestedJob(void)
{
    ...
    if( NvM_JobQueueParameter[fl_Index].CurrentQueueSize < 0xFFU )
    {
    ...
    NvM_DataStruct.JobPriorityBits |= ((uint32)(1U<<fl_JobPriority));
}
...
static Std_ReturnType NvM_DequeueNextJob(void)
{
    ...
    {
#endif
        if ((((uint8)FALSE) != NvM_DataStruct.JobPriorityBits) && (fl_JobPriority < NVM_MAX_JOB_PRIORITY))
        {
            fl_Index = NvM_JobQueueManagementTable[fl_JobPriority].Index;
            fl_JobQueueAddress = NvM_JobQueueManagementTable[fl_JobPriority].JobQueueAddress;
...
static uint8 NvM_GetNextHighPriority (void)
{
    uint8 fl_JobPriority = 0U;
    uint32 fl_PriorityBits = NvM_DataStruct.JobPriorityBits;
    if ((fl_PriorityBits & 0xffffU) == 0U)
    {
        fl_JobPriority += 16U;
        fl_PriorityBits >>= 16U;
    }
    if ((fl_PriorityBits & 0xffU) == 0U)
    {
        fl_JobPriority += 8U;
        fl_PriorityBits >>= 8U;
    }
    if ((fl_PriorityBits & 0xfU) == 0U)
    {
        fl_JobPriority += 4U;
        fl_PriorityBits >>= 4U;
    }
    if ((fl_PriorityBits & 0x3U) == 0U)
    {
        fl_JobPriority += 2U;
        fl_PriorityBits >>= 2U;
    }
    if ((fl_PriorityBits & 0x1U) == 0U)
    {   
        fl_JobPriority += 1U;
    }
...
{
    Std_ReturnType fl_return = E_NOT_OK;
    uint8 fl_reqType = 0xFFU;
    uint8 fl_Index = NVM_NUM_OF_PRIORITY;
    NvM_QueueType* fl_JobQueueAddress = 0U;

    if (FALSE == NvM_DataStruct.PreemptedJobFlag)
    {
        if(NVM_MAX_JOB_PRIORITY > fl_NextPrio)
        {
        ...
        if(NVM_NUM_OF_PRIORITY > fl_Index)
        {
            if((NvM_JobQueueParameter[fl_Index].CurrentQueueSize > 0U) && (fl_JobQueueAddress != NULL))
            {
...
/*Change Description: coverity high and medium warning fix                  */
/*--------------------------------------------------------------------------*/
/**-------------------------------------------------------------------------*/
/*Date              : 19-07-2023                                            */
/*Version           :                                                       */
...
/*By                : sdv                                                   */
/*Traceability      :                                                       */
/*Change Description: NvM 4.3 MIGRATION - Implementaion of NvM_FirstInitAll 
                     and NvM_GetActiveService                                */
/*--------------------------------------------------------------------------*/
/**-------------------------------------------------------------------------*/
/*Date              : 02-04-2024                                            */
/*Version           :                                                       */
/*By                : kapalaniv                                             */
/*Traceability      :                                                       */
/*Change Description: NvM 4.3 MIGRATION                                     */
/*--------------------------------------------------------------------------*/
/**-------------------------------------------------------------------------*/
/*Date              : 29-05-2024                                            */
/*Version           :                                                       */
