...
{
    un_CPUSS_CM4_SYSTEM_INT_CTL_t unIntCtl0 = { 0UL };
    if(CPUSS->unIDENTITY.stcField.u4MS == CPUSS_MS_ID_CM4)
    {
        unIntCtl0.stcField.u3CPU_INT_IDX = (uint8_t)pThisIrqParam->SysIntIx;
        unIntCtl0.stcField.u1CPU_INT_VALID = 1U;
        CPUSS->unCM4_SYSTEM_INT_CTL[pThisIrqParam->osIrqNum].u32Register = unIntCtl0.u32Register;
    }
}
