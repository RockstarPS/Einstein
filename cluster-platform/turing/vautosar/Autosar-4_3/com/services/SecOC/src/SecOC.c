...
**                       Version  Check                                       **
*******************************************************************************/
#if (SECOC_CFG_AR_MAJOR_VERSION != SECOC_H_AR_MAJOR_VERSION)
#error "SecOC.c : Mismatch in Specification Major Version"
#endif
#if (SECOC_CFG_AR_MINOR_VERSION != SECOC_H_AR_MINOR_VERSION)
#error "SecOC.c : Mismatch in Specification _MINOR Version"
#endif
#if (SECOC_CFG_AR_PATCH_VERSION != SECOC_H_AR_PATCH_VERSION)
#error "SecOC.c : Mismatch in Specification PATCH Version"
#endif
#if (SECOC_CFG_SW_MAJOR_VERSION != SECOC_H_SW_MAJOR_VERSION)
#error "SecOC.c : Mismatch in Software Major Version"
#endif
#if (SECOC_CFG_SW_MINOR_VERSION != SECOC_H_SW_MINOR_VERSION)
...
)
{
...
#endif
    {
        ...
        versioninfo->sw_major_version = SECOC_H_SW_MAJOR_VERSION;
        versioninfo->sw_minor_version = SECOC_H_SW_MINOR_VERSION;
        versioninfo->sw_patch_version = SECOC_H_SW_PATCH_VERSION;
    }
}
...
SecOC_TpTransmit(PduIdType TxPduId, P2CONST(PduInfoType, AUTOMATIC, SECOC_APPL_CONST) PduInfoPtr)
{
    ...
    /*TxPduId error*/
    else if (!((TxPduId >= SECOC_TX_IF_PDU_NUM) && (TxPduId < SECOC_TX_PDU_NUM)))
    {
...
)
{
...
#endif /* STD_ON == SECOC_DEV_ERROR_DETECT */
    {
        ...
        if ((index < SECOC_TX_IF_PDU_NUM) && (TRUE == txRtPtr->txPduReady))
        {
            ...
            authInfoTxLength = txPduProcessingPtr->SecOCAuthInfoTxLength;
            fvTruncLenInBits = txPduProcessingPtr->SecOCFreshnessValueTxLength;
            ...
            else if (TRUE == txCryptographicPduFlag)
            {
 ...
 *   @param[out]     bufferSizePtr: Available receive buffer after data has
 *                                    been copied.
 *   @param[in/out]  None
 ...
 *   @Sync/Async          Synchronous
 *   @Reentrancy          Non-reentrant
 *   @param[in]      idx:Index for which the pointer to the transmit PDU Processing is requested.
 *   @param[out]     None
 *   @param[in/out]  None
 ...
 *   @Sync/Async          Synchronous
 *   @Reentrancy          Non-reentrant
 *   @param[in]      idx:Index for which the pointer to the transmit Authentic PDU Layer is requested.
 *   @param[out]     None
 *   @param[in/out]  None
 ...
 *   @Sync/Async          Synchronous
 *   @Reentrancy          Non-reentrant
 *   @param[in]      idx: Index representing the Secured PDU collection.
 *   @param[out]     None
 *   @param[in/out]  None
...
static FUNC(Std_ReturnType, SECOC_CODE) SecOC_TxConstructDataToAuthenticator(uint16 index)
{
    ...
    P2CONST(SecOC_TxPduSecuredAreaType, TYPEDEF, SECOC_CONST)
    SecOCTxPduSecuredArea = txPduProcessingPtr->SecOCTxPduSecuredArea;
    SecOC_FreshnessArrayType truncatedFreshnessValue = 0U;
    ...
    freshnessValueTruncLen = txPduProcessingPtr->SecOCFreshnessValueTxLength;
    ...
    if (E_OK == ret)
    {
        ...
        (void)memcpy(
            ...
            (const void *)txRtPtr->txFreshnessValue,
            freshnessValueLen);
    }
    SchM_Exit_SecOC_Authentication();
...
static FUNC(void, SECOC_CODE) SecOC_TxConstructSecuredIPDU(uint16 index)
{
     ...
     * [SecOCFreshnessValueTxLength] (optional) | Authenticator
    ...
    /* Freshness value */
    /* @req SWS_SecOC_00094 */
    if (txPduProcessingPtr->SecOCFreshnessValueTxLength > 0u)
    {
        ...
        SecOC_ArrayShiftLeft((uint8*)txRtPtr->txFreshnessValue, fvLenInBytes, fvOffsetBits);
        (void)memcpy(&destPtr[offset], (const void *)txRtPtr->txFreshnessValue, fvTruncLenInBytes);
     ...
     * by SecOCAuthInfoTxLength. */
    authInfoShiftBits = SECOC_MOD_FROM_EIGHT((fvTruncLenInBits));
    authInfoTruncLenInBytes =
        SECOC_BIT_TO_BYTE(txPduProcessingPtr->SecOCAuthInfoTxLength + (uint16)authInfoShiftBits);
    SecOC_ArrayShiftRight(&txRtPtr->txResultBuffer[0u], authInfoTruncLenInBytes, authInfoShiftBits);
    SecOC_ClearLowBit(
        &txRtPtr->txResultBuffer[0u],
        authInfoTruncLenInBytes,
        SECOC_MOD_FROM_EIGHT((txPduProcessingPtr->SecOCAuthInfoTxLength + fvTruncLenInBits)));
...
static FUNC(void, SECOC_CODE) SecOC_TxConstructCryptographicIPDU(uint16 index)
{
    ...
    /* Authenticator */
    authInfoShiftBits = SECOC_MOD_FROM_EIGHT((fvTruncLenInBits));
    authInfoTruncLenInBits = txPduProcessingPtr->SecOCAuthInfoTxLength;
...
static FUNC(void, SECOC_CODE) SecOC_TxBroadcastSecuredIPDU(uint16 index)
{
    ...
    PduInfoType pduInfo;
    uint16 authInfoTxLength = txPduProcessingPtr->SecOCAuthInfoTxLength;
    uint8 fvTruncLenInBits = txPduProcessingPtr->SecOCFreshnessValueTxLength;
...
static FUNC(void, SECOC_CODE) SecOC_TxChannelReset(uint16 index)
{
    ...
    (void)memset((void *)txRtPtr->txFreshnessValue, 0, SECOC_MAX_FRESHNESS_SIZE);
    txRtPtr->txPduReady = FALSE;
    txRtPtr->txAuthBuildCnt = 0u;
 ...
 *   @Sync/Async          Synchronous
 *   @Reentrancy          Non-reentrant
 *   @param[in]      i: Index for which the receive event needs to be set.
...
static FUNC(void, SECOC_CODE) SecOC_RxParseSecuredIpdu(uint16 index)
{
    ...
    uint8 fvTruncLenInBits = rxPduProcessingPtr->SecOCFreshnessValueTxLength;
            ...
            if (NULL_PTR != rxSecuredPduPtr)
            {
                ...
                if (FALSE == dropMsgFlag)
                {
                    ...
                    (void)memcpy(
                        ...
                        SECOC_BIT_TO_BYTE((uint16)rxPduProcessingPtr->SecOCFreshnessValueTxLength));
                    if (0u != SECOC_MOD_FROM_EIGHT(fvTruncLenInBits))
                    {
                    ...
                    SecOC_ArrayShiftLeft(
                        ...
                        rxPduProcessingPtr->SecOCFreshnessValueTxLength);
                    (void)memcpy(
                        rxRtPtr->rxParsedAuthenticator,
                        &rxSecuredPduBufPtr[offset],
                        (uint32)SECOC_BIT_TO_BYTE((uint16)rxPduProcessingPtr->SecOCAuthInfoTxLength));
                    SchM_Exit_SecOC_RxData();
                }
            }
            else
                ...
                if (FALSE == dropMsgFlag)
                {
                    ...
                    if (SECOC_IS_RX_EVENT(index, SECOC_RX_CRYPTOGRAPHIC_PDU))
                    {
                        ...
                        (void)memcpy(
                            rxRtPtr->rxParsedAuthenticator,
                            rxCryptographicPduBufPtr,
                            SECOC_BIT_TO_BYTE((uint16)rxPduProcessingPtr->SecOCAuthInfoTxLength));
                    }
                    SchM_Exit_SecOC_RxData();
                }
            }
        ...
        if (E_OK == ret)
        {
            ...
            case SECOC_CSMMAC:
                ret = Csm_MacVerify(
                    ...
                    dataLength,
                    rxRtPtr->rxParsedAuthenticator,
                    rxPduProcessingPtr->SecOCAuthInfoTxLength,
...
static FUNC(Std_ReturnType, SECOC_CODE) SecOC_RxConstructDataToAuthenticator(uint16 index)
{
    ...
    if (E_OK == ret)
    {
        ...
        (void)memcpy(
            &SecOC_DataToAuthenticator[SECOC_DATAID_LEN + securedIPDULength],
            (const void *)rxRtPtr->rxFreshnessValue,
            freshnessValueLen);
        SchM_Exit_SecOC_Authentication();
    }
    else if (E_BUSY == ret)
 ...
 *   @Sync/Async          Synchronous
 *   @Reentrancy          Non-reentrant
 *   @param[in]      idx: Index for which the pointer to the use message link structure is requested.
 *   @param[out]     None
 *   @param[in/out]  None
...
static FUNC(Std_ReturnType, SECOC_CODE) SecOC_RxMessageLinkCheck(uint16 index)
{
    ...
    if ((NULL_PTR != rxSecuredPduCollectionPtr))
    {
            ...
            && SECOC_IS_RX_EVENT(index, SECOC_RX_AUTHENTIC_PDU) && SECOC_IS_RX_EVENT(index, SECOC_RX_CRYPTOGRAPHIC_PDU))
        {
            ...
            offsetBits =
                rxPduProcessingPtr->SecOCAuthInfoTxLength + rxPduProcessingPtr->SecOCFreshnessValueTxLength;
...
static FUNC(void, SECOC_CODE) SecOC_RxChannelReset(uint16 index)
{
    ...
    rxRtPtr->rxAuthBuildAttempts = 0u;
    rxRtPtr->rxVerifyAttempts = 0u;
    (void)memset((void *)rxRtPtr->rxFreshnessValue, 0, sizeof(rxRtPtr->rxFreshnessValue));
    rxRtPtr->rxVeryfyResult = SECOC_VERIFICATIONFAILURE;
#if (SECOC_RX_TP_PDU_NUM > 0)
...
static FUNC(PduLengthType, SECOC_CODE) SecOC_RxGetPduLength(
    uint8 headerLength,
    P2CONST(uint8, AUTOMATIC, SECOC_CONST) headerPtr
)
{
    uint32 pduLength = 0u;
 ...
 *   @Sync/Async          Synchronous
 *   @Reentrancy          Reentrant
 *   @param[in]      dataPtr: Pointer to data to be convert
 ...
 *   @Sync/Async          Synchronous
 *   @Reentrancy          Reentrant
 *   @param[in]      numLen: Array length in Byte
 *                     shiftBits: Shift bits
 *   @param[out]     None
 *   @param[in/out]  numArr: number array for operate and also store result
 *   @return              None
 */
/******************************************************************************/
static FUNC(void, SECOC_CODE) SecOC_ArrayShiftLeft(
 ...
 *   @param[out]     None
 *   @param[in/out]  numArr1: number array for operate and also store result
 *   @return              None
 */
/******************************************************************************/
static FUNC(void, SECOC_CODE) SecOC_ArrayOrOp(
    ...
    SecOC_DataInQueue(uint16 index, PduIdType RxPduId, P2CONST(PduInfoType, AUTOMATIC, SECOC_APPL_DATA) PduInfoPtr)
{
    ...
    if (queueManagePtr->freeQueSize >= needBufSize)
    {
        ...
        else
        {
            ...
            (void)memcpy(&rxQueueBufPtr[0u], ((uint8*)&queInfo + part1Len), part2Len);
            offset = part2Len;
        }
 ...
 *   @param[out]     None
 *   @param[in/out]  PduInfoPtr: Input buffer length and buffer address,when
...
)
{
    ...
    if (queueManagePtr->queueFrameCnt > 0u)
    {
        ...
        else
        {
            ...
            (void)memcpy(((uint8*)&queInfo + part1Len), &rxQueueBufPtr[0u], part2Len);
            offset = part2Len;
        }
        *RxPduIdPtr = queInfo.pduId;
        ...
        {
            /* Buffer not enough,   @ with E_NOT_OK */
            /*   return to E_NOT_OK; */
        }
    }
    SchM_Exit_SecOC_RxData();
...
By                : cvenkata
Traceability      : DICVA-19595
Change Description: SecOC module - Initial development KW fix.
-----------------------------------------------------------------------------*/
/*---------------------------------------------------------------------------
Date              : 28/Jul/2025
By                : cvenkata
