...
#define MATHLIB_CONST_SEC_START
#include "MemMap.h"
...
sint32 MathLib_CelsToFahrRes( const sint32 Celsius, const sint8 Resolution )
{
    ...
    if ( (cMAX_RESOLUTION <= Resolution) && (1 > Resolution) )
    {
        ...
        if( ((sint64)(Celsius * cFAHR_FACTOR)) < 0x7FFFFFFF )
		{
        ...
        Fahrenh = Fahrenh + (sint32)( cFAHR_SUMMAND * (sint32)ResolutionOfTen );
    }
                                        ...
                                        const ERoundingMethods RoundMethod, uint32 * const pOutput )
{
                    ...
                    && (cMIN_RESOLUTION >= OutputRes) && (RoundMethod < eRoundingMethods_Count) )
    {
        ...
        if ( OutputRes < 0 )
        {
			if( ((uint64)(Meters * cDISTANCE_CONV_CONST_NUMERATORS[OutputUnit])) < 0xFFFFFFFF  )  
	        {
        ...
        {
			if( (((uint64)(cDISTANCE_CONV_CONST_DENOMINATORS[OutputUnit] * Resolution)) < 0xFFFFFFFF ) &&   (((uint64)(Meters * cDISTANCE_CONV_CONST_NUMERATORS[OutputUnit])) < 0xFFFFFFFF ) )
			{
	            *pOutput = cDISTANCE_CONV_FUNCS[OutputUnit](
	                Meters * cDISTANCE_CONV_CONST_NUMERATORS[OutputUnit],
	                cDISTANCE_CONV_CONST_DENOMINATORS[OutputUnit] * Resolution,
                                      ...
                                      const ERoundingMethods RoundMethod, uint32 * pOutput )
{
                    ...
                    && (cMIN_RESOLUTION >= OutputRes) && (RoundMethod < eRoundingMethods_Count) )
    {
        ...
        {
			if( ((uint64)(cVOLUME_CONV_CONST_DENOMINATORS[OutputUnit] * Resolution)) < 0xFFFFFFFF ) 
			{
				Divisor = cVOLUME_CONV_CONST_DENOMINATORS[OutputUnit] * Resolution;
			}
			else 
        ...
        if( ((uint64)((Mililiters * cVOLUME_CONV_CONST_NUMERATORS[OutputUnit]))) < 0xFFFFFFFF ) 
		{
...
Date              : 10/Feb/2021
By                : vramesh1
Traceability      : RTC #1206956
Change Description: [hondabev] CERTC New rules fix
----------------------------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------------------------
Date              : 26/Mar/2022
By                : gthanapp
Traceability      : RTC #1564070
Change Description: [J32V] CERTC Warnings Fix
----------------------------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------------------------
Date              : 29/June/2022
By                : svallur1
Traceability      : RTC #1500397
Change Description: Coverity warning analysis and fix
----------------------------------------------------------------------------------------------------------------------*/
/*--------------------------------------------------------------------------------------------------------------------
Date              : 04/April/2023
By                : gthanapp
Traceability      : RTC #1972838
Change Description: Coverity & Misra High Warning fix
----------------------------------------------------------------------------------------------------------------------*/