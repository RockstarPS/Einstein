 ...
 *****************************************************************************/
typedef void (*crypto_message_func_t)(uint8 keyId, uint8* cryptodata_addr,uint16 payload_size);
...
typedef void (*command_message_func_t)(uint8* commanddata_addr,uint16 payload_size);
...
FUNC (Std_ReturnType, CRY_CODE) vHsmCrypto_Manager_SendData(ts_vHsm_Cryptodata s_vHsm_Cryptodata)
{
    ...
    if(s_vHsm_Cryptodata.payLoadSize < ((uint16)CRYPTO_MAX_PAYLOAD_DATA_SIZE + (uint16)1))
    {
        if(s_vHsm_Cryptodata.jobId_E < CRYPTO_UNUSED_JOBID)
        {
            ...
            l_payloadsize_U16 = l_cryptodata_S.s_crytpjobheader.payloadsize + (uint16)CRYPTO_HEADER_SIZE;
            l_RetVal_E = IpcHost_MessageSend((uint8 *)&l_cryptodata_S,l_payloadsize_U16,(uint16)IPC_CRYPTO_ID);
        }
        else
        {
            l_RetVal_E = E_NOT_OK;
...
FUNC (Std_ReturnType, CRY_CODE) vHsmCrypto_Command_SendData(ts_vHsm_Commanddata s_vHsm_Commanddata)
{
    ...
    /* Packing of the data with the crypto header and sending for IPC transfer */
    if(s_vHsm_Commanddata.payLoadSize < (CRYPTO_COMMAND_MAX_PAYLOAD_DATA_SIZE + (uint16)1))
    {
        if(s_vHsm_Commanddata.jobId_E < COMMAND_UNUSED_JOBID)
        {
            ...
            l_payloadsize_U16 = l_commanddata_S.s_commandjobheader.payloadsize + (uint16)COMMAND_HEADER_SIZE;
            l_RetVal_E = IpcHost_MessageSend((uint8 *)&l_commanddata_S,l_payloadsize_U16,(uint16)IPC_COMMAND_ID);
        }
        else
        {
            l_RetVal_E = E_NOT_OK;
 ...
 **             uint16 payload_size
 *****************************************************************************/
FUNC (void, CRY_CODE) IPC_Crypto_MessageHandler(uint8 buffer_id, const uint8* buffer_add,uint16 payload_size)
{
    ...
    if(payload_size < ((uint16)CRYPTO_MAX_PAYLOAD_DATA_SIZE + (uint16)1))
    {
        ...
        if(NULL != l_cryptodata_ps)
        {
            ...
            if(l_cryptojobid_e < CRYPTO_UNUSED_JOBID)
            {
                ...
                l_keyId = l_cryptodata_ps->s_crytpjobheader.cryptokeyid;
                /* coverity[sensitive_memory_access] : FALSE */
                /* Calling the function peratining to the Job Id */
                (void)Crypto_FuncPtr_jumptable[l_cryptojobid_e](l_keyId,l_cryptodataaddr_U8P,l_cryptopayloadsize_U16);
            }
        }
    }
 ...
 **             uint16 payload_size
 *****************************************************************************/
FUNC (void, CRY_CODE) IPC_Command_MessageHandler(uint8 buffer_id, const uint8* buffer_add,uint16 payload_size)
{
    ...
    if(payload_size < ((uint16)CRYPTO_COMMAND_MAX_PAYLOAD_DATA_SIZE + (uint16)1))
    {
