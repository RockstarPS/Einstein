 ...
 *****************************************************************************/
typedef FUNC (void, CRY_CODE) (*crypto_message_func_t)(uint8_t* cryptodata_addr,uint16_t payload_size);
...
typedef FUNC (void, CRY_CODE) (*command_message_func_t)(uint8_t* commanddata_addr,uint16_t payload_size);
...
typedef struct
{
    ...
    uint8_t payloaddata[CRYPTO_MAX_PAYLOAD_DATA_SIZE];
}ts_cryptodata;
...
typedef struct
{
    uint16_t payloadsize;
    uint8_t commandjob;
    uint8_t commandjobprio;
} ts_commandjobheader;

typedef struct
{
    ts_commandjobheader s_commandjobheader;
    uint8_t payloaddata[CRYPTO_COMMAND_MAX_PAYLOAD_DATA_SIZE];
}ts_commanddata;
#pragma pack(pop)
/************************************************************************
...
FUNC (Std_ReturnType, CRY_CODE) vHsmCrypto_Manager_SendData(ts_vHsm_Cryptodata s_vHsm_Cryptodata)
{
    ...
    if(s_vHsm_Cryptodata.payLoadSize < (CRYPTO_MAX_PAYLOAD_DATA_SIZE + 1))
    {
        if(s_vHsm_Cryptodata.jobId_E < CRYPTO_UNUSED_JOBID)
        {
            ...
            (void)memcpy((uint8_t *)&l_cryptodata_S.payloaddata[0],(uint8_t *)s_vHsm_Cryptodata.payLoadPtr_U8P,s_vHsm_Cryptodata.payLoadSize);
            l_payloadsize_U16 = l_cryptodata_S.s_crytpjobheader.payloadsize + CRYPTO_HEADER_SIZE;
            l_RetVal_E = IpcHost_MessageSend((uint8_t *)&l_cryptodata_S,l_payloadsize_U16,IPC_CRYPTO_ID);
        }
        else
        {
            l_RetVal_E = E_NOT_OK;
...
FUNC (Std_ReturnType, CRY_CODE) vHsmCrypto_Command_SendData(ts_vHsm_Commanddata s_vHsm_Commanddata)
{
    ...
    if(s_vHsm_Commanddata.payLoadSize < (CRYPTO_COMMAND_MAX_PAYLOAD_DATA_SIZE + 1))
    {
        if(s_vHsm_Commanddata.jobId_E < COMMAND_UNUSED_JOBID)
        {
            ...
            (void)memcpy((uint8_t *)&l_commanddata_S.payloaddata[0],(uint8_t *)s_vHsm_Commanddata.payLoadPtr_U8P,s_vHsm_Commanddata.payLoadSize);
            l_payloadsize_U16 = l_commanddata_S.s_commandjobheader.payloadsize + COMMAND_HEADER_SIZE;
            l_RetVal_E = IpcHost_MessageSend((uint8_t *)&l_commanddata_S,l_payloadsize_U16,IPC_COMMAND_ID);
        }
        else
        {
            l_RetVal_E = E_NOT_OK;
 ...
 *****************************************************************************/
FUNC (void, CRY_CODE) IPC_Crypto_MessageHandler(uint8_t buffer_id, uint8_t* buffer_add,uint16_t payload_size)
{
    ts_cryptodata *l_cryptodata_ps = (ts_cryptodata *)NULL;
    uint8_t l_cryptojobid_e = (uint8_t)CRYPTO_UNUSED_JOBID;
    uint8_t * l_cryptodataaddr_U8P = (uint8_t *)NULL;
    uint16_t l_cryptopayloadsize_U16 = 0;
    ...
    if(payload_size < (CRYPTO_MAX_PAYLOAD_DATA_SIZE + 1))
    {
        ...
        {
            l_cryptojobid_e = (uint8_t)l_cryptodata_ps->s_crytpjobheader.cryptojob;
            /* Checking if the Job ID sent is valid */
            if(l_cryptojobid_e < (uint8_t)CRYPTO_UNUSED_JOBID)
            {
                l_cryptodataaddr_U8P = (uint8_t *)&l_cryptodata_ps->payloaddata[0];
 ...
 **             uint16_t payload_size
 *****************************************************************************/
FUNC (void, CRY_CODE) IPC_Command_MessageHandler(uint8_t buffer_id, uint8_t* buffer_add,uint16_t payload_size)
{
    ts_commanddata *l_commanddata_ps = (ts_commanddata *)NULL;
    uint8_t l_commandjobid_e = (uint8_t)COMMAND_UNUSED_JOBID;
    uint8_t * l_commanddataaddr_U8P = (uint8_t *)NULL;
    uint16_t l_commandpayloadsize_U16 = 0;
    ...
    if(payload_size < (CRYPTO_COMMAND_MAX_PAYLOAD_DATA_SIZE + 1))
    {
        ...
        if(NULL != l_commanddata_ps)
        {
            ...
            /* Checking if the Job ID sent is valid */
            if(l_commandjobid_e < (uint8_t)COMMAND_UNUSED_JOBID)
            {
                l_commanddataaddr_U8P = (uint8_t *)&l_commanddata_ps->payloaddata[0];
