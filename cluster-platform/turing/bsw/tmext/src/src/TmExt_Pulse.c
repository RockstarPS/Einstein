...
#define U32_PCHANNEL_MAX_VALUE                           0xFFFFFFFFu
#define SIZE_OF_8BIT         0xFFu
//----------------------------------------------------------------------------------------------------------------------
            ...
            if( ( 0U < pChnStatus->ClientCount ) )
            {
                ...
                if( 0U != ( cRESTART_COMMON & pChnStatus->Restart ) )
                {
                    ...
                    if( 0U == pChnStatus->Restart )
                    {
                        ...
                        {
                            TmExt_ReportDetError(TMEXT_API_MAINFUNCTION,TMEXT_E_PARAM_POINTER);
                        }
                    }
                    else
                    {
                ...
                if(pChnConfig != NULL_PTR)
                {
                    ...
                    {
						if( pChnStatus->Ix < U32_PCHANNEL_MAX_VALUE )
						{
                        ...
                        {
                            // Repeat the pattern
                            pChnStatus->Ix = (uint32)0x01u; 
                            // Patterns start at position 1
                            NewPhase = pChnConfig->StartPhase; 
                        }
                        // Start phase of the pattern
                        else
                        {
        ...
        if( ePulseChannelCount > Notification->Channel )
        {
            ...
            if( eClientCount > Client )
            {
                  ...
                  )
                {
					...
					if( pChnStatus->ClientCount < SIZE_OF_8BIT )
					{
    ...
    if(pChnStatus != NULL_PTR)
    {
        if( (ePulseChannelCount > pChnStatus->ChannelSync.Channel ) && (ePulsePhase_Any != pChnStatus->ChannelSync.Phase))
        {
            ...
            if( pChnStatus->ChannelSync.Channel < Channel )
            {
                ...
                if(pChnConfigToSync != NULL_PTR)
                {
                    if((pChnStatusToSync->Ix) > 0U)
                    {
                        if( pChnConfigToSync->pPattern[(pChnStatusToSync->Ix)-1U] == pChnStatusToSync->Counter )
                        {
                ...
                {
                    TmExt_ReportDetError(TMEXT_API_SYNCCHANNELS,TMEXT_E_PARAM_POINTER);
                }
            }
