...
#define PMICCDD_READ_WD_REG           (0x04U)
#define PMICCDD_READ_CRC_REG_BIT      ((uint8)0x05U)
...
#define PMICCDD_ERROR_I2C_TRANSMIT_CRC_FAILED           (0x06U)
...
typedef union{
    struct{
        ...
        uint8_t i2c1_spi_crc_en : 1;         // I2C1_SPI_CRC_EN
        uint8_t i2c2_crc_en : 1;             // I2C2_CRC_EN
    } bits;
    uint8_t value;
...
typedef union {
    struct {
        uint8_t recov_cnt_thr : 4;           // RECOV_CNT_THR
        uint8_t recov_cnt_clr : 1;           // RECOV_CNT_CLR
    } bits;
    uint8_t value;
...
typedef union {
    struct {
        uint8_t ss_depth : 1;                // SS_DEPTH
        uint8_t reserved : 1;                // RESERVED
        uint8_t ss_en : 1;                   // SS_EN
    } bits;
    uint8_t value;
...
/* SW Dump Log Status*/
typedef enum {
    /* SW is hung so log the Dump in the Dlt*/
    PmicCdd_SWHang_DumpLog = 0,
    /* SW hang recovered so clear the Dump log in the Dlt*/
    PmicCdd_SWHangRecovered_ClearDumpLog,
    PmicCdd_SWHang_None,
} PmicCdd_DumpLogStatusType;
