...
#if(DLTEXT_UART_PRINT == STD_ON)
extern void DltExt_Uart_TxComplete(void);
extern Std_ReturnType DltExt_Uart_Send(const uint8 *data, uint16 len);
extern Std_ReturnType DltExt_Uart_Printf(const char *fmt, ...);
extern Std_ReturnType DltExt_Uart_SendHex(ESciDrvAsyncMode mode, const uint8 *bytes, uint16 nbytes, const char *prefix);
#endif
