...
static Std_ReturnType DisplayDrvI2CSendDetect(DisplayCheckConfigType lcdConfig)
{
    Std_ReturnType retL = E_PENDING;
	I2c_StatusType I2cStatus ;

    switch (displayCheckState)
    {
        case SEND_COMMAND:
        {
            Std_ReturnType Result = DisplayDrvCdd_I2cWrite(lcdConfig.address, &lcdConfig.data, (uint8)0x01, lcdConfig.displayType);
            if((uint8)E_OK == Result)
            {
                displayCheckState = WAIT_ACKNOWLEDGE;
            }
            else
            {
                retL = E_NOT_OK;
        ...
        {
            I2cStatus = I2c_GetStatus(DISP_HW_UNIT_INDEX);/*Unit Index macro to be defined in cfg file*/
            if ( I2C_IDLE == I2cStatus )
            {
                //If acknowledge is returned for corresponding device id then display is recoginzed.
                //0x38 TrulySitronix, 0x6f Sharp and 0x52 TrulyRaydium.
                I2c_SeqResultType resAcknowledge = I2c_GetSequenceResult(lcdConfig.sequence);
                if ( I2C_SEQ_OK == resAcknowledge )
                {
                    retL = E_OK;
                }
                else
