...
Std_ReturnType DisplayDrvCdd_I2cWrite(uint8 RegAddr, uint8 *I2c_TxBufferPtr, uint8 TxLength , uint8 DrvI2cChannelId , uint8 DrvI2cSeqId , uint8 TriggeredDevice)
{
	...
	uint8 I2cSeqId, DataLength/*, I2cChannelId*/;
	uint8 index;
	...
	if((NULL != I2c_TxBufferPtr) && (TxLength > 0u))
	{
		...
		DisplayI2C_RetVal = I2c_SetupEB(DrvI2cSeqId, I2c_TxBuffer, NULL, DataLength , 0 ,I2CTrigerDeviceId );
		
		if((uint8)E_OK == DisplayI2C_RetVal)
		{
			DisplayI2C_RetVal = I2c_AsyncTransmit(I2cSeqId);
		}
    }
