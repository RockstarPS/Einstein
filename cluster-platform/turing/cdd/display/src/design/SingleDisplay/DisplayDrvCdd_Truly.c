...
static SDisplayDrvTrulyInfo DisplayDrvTrulyInfoTable[cDISPLAYCDD_MAX_DEVICE];
/*! Hold the driver power down state machine  related information  */
static EDisplayTrulyPowerUpStates DisplayDrvTruly_PowerUp_StateMachine[cDISPLAYCDD_MAX_DEVICE];
/*! Hold the driver power up state machine  related information  */
static EDisplayTrulyPowerDownStates DisplayDrvTruly_PowerDown_StateMachine[cDISPLAYCDD_MAX_DEVICE];
...
Std_ReturnType DisplayDrvCdd_Init(void)
{
	...
	{
		if(Device_No == e_DisplayTruly)
		{
			Variant_No = 0;
			DisplayDrvTrulyInfoTable[Device_No].DeviceID         	= DisplayTrulyInfoDB[Variant_No].DisplayDeviceId;
			DisplayDrvTrulyInfoTable[Device_No].DeviceSupplyPin     = DisplayTrulyInfoDB[Variant_No].DisplaySupplyPin;
			DisplayDrvTrulyInfoTable[Device_No].DeviceResetPin      = DisplayTrulyInfoDB[Variant_No].DisplayResetPin;	
			DisplayDrvTrulyInfoTable[Device_No].DeviceStandbyPin    = DisplayTrulyInfoDB[Variant_No].DisplayStandbyPin;
			DisplayDrvTrulyInfoTable[Device_No].DeviceBacklightPin  = DisplayTrulyInfoDB[Variant_No].DisplayBacklightPin;
			DisplayDrvTrulyInfoTable[Device_No].FaultStatusPin      = DisplayTrulyInfoDB[Variant_No].DisplayFaultStatusPin;	
    ...
    for(Device_No = cDISPLAYCDD_DEVICE_0; Device_No < cDISPLAYCDD_MAX_DEVICE; Device_No++)
	{
		if(Device_No == e_DisplayTruly)
		{
			switch(DisplayDrvTruly_PowerDown_StateMachine[Device_No])
			{
				...
				{
					DisplayDrvCdd_Truly_Waittime = cDISPLAYCDD_TRULY_POWEROFF_DELAY1;
					DisplayDrvTruly_PowerDown_StateMachine[Device_No] = eDisplayTruly_PowerOFF_DisableBacklight;
				}
				break;
...
static void DisplayDrvCdd_GammaCorrectionSequenceExecution(void)
{
	switch(GammaCorrectionState)
	{
		...
		case GMA_SET_STATE_DISPLAY_POWERON_WAIT:
			if((boolean)FALSE != DisplayIfCdd_GetDisplayPowerUpStatus())
			{
...
static void DisplayDrvCdd_GammaCorrectionSequence(void)
{
	...
	switch (GammaCorrectionSequenceState)
	{
		...
		case GMA_SET_STEP_OPEN_PAGE:
			...
			Ret = DisplayDrvCdd_I2cWrite(I2c_RegAdd, &I2c_TxData, 0x01);	 /* open page passward */
		...
		case GMA_SET_STEP_RED_SETTING:
			...
			if (Gamma_RGB_WriteIndex < GMA_REG_LEN_TRULY)
			{
				...
				Ret = DisplayDrvCdd_I2cWrite(I2c_RegAdd, &GammaCorrectionData[Gamma_RGB_WriteIndex], (GMA_REG_LEN_TRULY/GMA_REG_LEN_DIV_TRULY));
		...
		case GMA_SET_STEP_GREEN_SETTING:
			...
			{
				I2c_RegAdd = Gamma_RGB_WriteIndex + 1u;
				Ret = DisplayDrvCdd_I2cWrite(I2c_RegAdd, &GammaCorrectionData[GMA_REG_LEN_TRULY + Gamma_RGB_WriteIndex], (GMA_REG_LEN_TRULY/GMA_REG_LEN_DIV_TRULY));
				Gamma_RGB_WriteIndex += (GMA_REG_LEN_TRULY/GMA_REG_LEN_DIV_TRULY);
			}
			else
			{
		...
		case GMA_SET_STEP_BLUE_SETTING:
			...
			{
				I2c_RegAdd = Gamma_RGB_WriteIndex + 1u;
				Ret = DisplayDrvCdd_I2cWrite(I2c_RegAdd, &GammaCorrectionData[(GMA_REG_LEN_TRULY * 2u) + Gamma_RGB_WriteIndex], (GMA_REG_LEN_TRULY/GMA_REG_LEN_DIV_TRULY));
				Gamma_RGB_WriteIndex += (GMA_REG_LEN_TRULY/GMA_REG_LEN_DIV_TRULY);
			}
			else
			{
...
static Std_ReturnType DisplayDrvCdd_LoadGammaSettingsFromNvM(void)
{
	...
	RetVal = DisplayIfCdd_ReadGammaSettingsFromNvM(Array_Red_Gamma_Adjustment_Display1_Handle, &GammaCorrectionData, GMA_REG_LEN_TRULY);
	RetValue = DisplayIfCdd_ReadGammaSettingsFromNvM(Array_Green_Gamma_Adjustment_Display1_Handle, (GammaCorrectionData+GMA_REG_LEN_TRULY), GMA_REG_LEN_TRULY);
	RetVal = (RetVal | RetValue);
	RetValue = DisplayIfCdd_ReadGammaSettingsFromNvM(Array_Blue_Gamma_Adjustment_Display1_Handle, (GammaCorrectionData+(GMA_REG_LEN_TRULY * 2)), GMA_REG_LEN_TRULY);
	RetVal = (RetVal | RetValue);
...
static Std_ReturnType DisplayDrvCdd_I2cWrite(uint8 RegAddr, uint8 *I2c_TxBufferPtr, uint8 TxLength)
{
	...
	if((NULL != I2c_TxBufferPtr) && (TxLength > 0u))
	{
		...
		/* I2C channel details for Truly */
		I2cChannelId = TRULY_I2C_CHANNEL;
		I2cSeqId = TRULY_I2C_SEQ;
