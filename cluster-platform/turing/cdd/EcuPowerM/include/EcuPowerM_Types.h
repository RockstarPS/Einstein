...
typedef enum
{
 	...
 	/* driver api type custom. */
	ECUPOWERM_DRIVER_API_TYPE_CUSTOM,
...
typedef P2FUNC(uint8, ECUPOWERM_CODE, EcuPowerM_LowlevelDrvFct_t) ( VAR(Dio_ChannelType, AUTOMATIC) channel);

typedef P2FUNC(void, ECUPOWERM_CODE, EcuPowerM_LowlevelDrvFct1_t)( 	VAR(Dio_ChannelType, AUTOMATIC) channel,
																  	VAR(Dio_LevelType, AUTOMATIC)value);
...
typedef P2FUNC(void, ECUPOWERM_CODE, EcuPowerM_LowlevelDrvFct8_t)( 	VAR(Pwm_ChannelType, AUTOMATIC) channel,
																   	VAR(uint16, AUTOMATIC)duty);

typedef P2FUNC(void, ECUPOWERM_CODE, EcuPowerM_LowlevelDrvFct9_t)( 	VAR(Pwm_ChannelType, AUTOMATIC) channel,
																	VAR(uint32, AUTOMATIC)period,
																   	VAR(uint16, AUTOMATIC)duty);

typedef P2FUNC(Std_ReturnType, ECUPOWERM_CODE, EcuPowerM_CustomApiFct_t)(	VAR(EcuPowerM_IoHwAbsChannelType, AUTOMATIC) channel,
																   			P2VAR(uint16, AUTOMATIC,ECUPOWERM_APPL_DATA)value,
																   			P2VAR(uint8, AUTOMATIC, ECUPOWERM_APPL_DATA)error);
...
typedef struct EcuPowerM_LowlevelDrvFuncTbl_s
{
...
typedef struct EcuPowerM_ChannelRef_s
{
	...
	uint16                              value;
	EcuPowerM_LowLevelDriverApiType		apiType;
	EcuPowerM_CustomApiFct_t            apiName;
...
typedef struct EcuPowerM_CurrentState_s
{
	...
	EcuPowerM_DevicePwrStatusType   deviceStatus;
	uint8 							deviceId;
	uint8                       	sequence;
	uint8                       	jobCnt;
	uint8                       	commandCnt;
