                           ...
                           uint32_t grpChannel, uint32_t index, uint32_t intSrc, void *arg)
{
  ...
  if (flErrSrc == FLTM_ERR_ECC_1BIT)
  {
	...
	{
	   FltM_ReportError(FLTM_ERR_CBK_WKUP_DOMIAN, FLTM_ECC_ERROR, intSrc);
	}
	else
    {
      FltM_ReportError(FLTM_ERR_CBK_WKUP_DOMIAN, FLTM_ECC_1BIT_ERROR, intSrc);
    }
  }
  else if(flErrSrc == FLTM_ERR_ECC_2BIT)
  {
	...
	FltM_ReportError(FLTM_ERR_CBK_WKUP_DOMIAN, FLTM_ECC_2BIT_ERROR, intSrc);
  }
  else
  {
  ...
  if (flErrSrc == FLTM_ERR_POK)
  {
    ...
    FltM_ReportError(FLTM_ERR_CBK_WKUP_DOMIAN, FLTM_POK_ERROR, intSrc);
  }
  ...
  if (flErrSrc == FLTM_ERR_DCC)
  {
    ...
    FltM_ReportError(FLTM_ERR_CBK_WKUP_DOMIAN, FLTM_DCC_ERROR, intSrc);
  }
#endif
                           ...
                           uint32_t grpChannel, uint32_t index, uint32_t intSrc, void *arg)
{
  ...
  if (flErrSrc == FLTM_ERR_ECC_1BIT)
  {
    ...
    FltM_ReportError(FLTM_ERR_CBK_MAIN_DOMIAN, FLTM_ECC_1BIT_ERROR, localIntSrc);
  }
  else if(flErrSrc == FLTM_ERR_ECC_2BIT)
  {
	 // Handle ECC error by clearning and getting the ECC error source
	...
	FltM_ReportError(FLTM_ERR_CBK_MAIN_DOMIAN, FLTM_ECC_2BIT_ERROR, localIntSrc);
  }
  else
  {
    /* MISRA 15.7 */
  ...
  {
    // Handle POK error by clearning and getting the POK error source
    (void)FltM_POK_CallBackFunc(esmInst, esmIntrType, grpChannel, index, localIntSrc);

    FltM_ReportError(FLTM_ERR_CBK_MAIN_DOMIAN, FLTM_POK_ERROR, localIntSrc);
  }
#endif /* #if(FLTM_POK_ENABLE == STD_ON) */
  ...
  {
    // Handle DCC error
    (void)FltM_Dcc_CallBackFunc(esmInst, esmIntrType, grpChannel, index, localIntSrc);

    FltM_ReportError(FLTM_ERR_CBK_MAIN_DOMIAN, FLTM_DCC_ERROR, localIntSrc);
  }
#endif
#if (FLTM_OCOCLKDET_ENABLE == STD_ON)
