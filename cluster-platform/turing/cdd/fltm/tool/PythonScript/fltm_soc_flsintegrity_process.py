...
FLTM_SOC_FLSINTEGRITY_CONFIG = {
    "sheet_name": "FltM_Soc_FlsIntegrity",
    "templates": [
        ("FltM_Soc_FlsIntegrity_Cfg.c.j2", "FltM_Soc_FlsIntegrity_Cfg.c"),
        ("FltM_Soc_FlsIntegrity_Cfg.h.j2", "FltM_Soc_FlsIntegrity_Cfg.h")
    ]
}
...
def process_fltm_soc_flsintegrity_sheet(env: Environment, xl: pd.ExcelFile, sheet_name: str, templates: List[Tuple[str, str]]) -> bool:
    ...
    try:
        ...
        for idx in range(split):
            key = df_full.iat[idx, 0]
            val = df_full.iat[idx, 1]
