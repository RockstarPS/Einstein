...
# Configuration for FltM_Soc_Pok sheet
FLTM_SOC_POK_CONFIG = {
    "sheet_name": "FltM_Soc_Pok",
    "templates": [
        ("FltM_Soc_Pok_Cfg.c.j2", "FltM_Soc_Pok_Cfg.c"),
        ("FltM_Soc_Pok_Cfg.h.j2", "FltM_Soc_Pok_Cfg.h")
    ]
}
...
# def process_fltm_soc_pok_sheet(env: Environment, xl: pd.ExcelFile, sheet_name: str, templates: List[Tuple[str, str]]) -> bool:
#     """Process FltM_Soc_Pok sheet with blank row separator and two headers."""
#     try:
...
#         block2 = df_full.iloc[split+1:].reset_index(drop=True)
#         block2.columns = [str(c).strip() for c in block2.iloc[0]]
#         block2 = block2[1:].dropna(axis=1, how="all").to_dict(orient="records")

#         # Validate data
#         empty_cells, invalid_cells = validate_dataframe(df_full.iloc[:split], sheet_name, 2)
#         empty_cells2, invalid_cells2 = validate_dataframe(df_full.iloc[split+1:], sheet_name, split + 3)
#         if empty_cells or invalid_cells or empty_cells2 or invalid_cells2:
#             for cells, tag in [(empty_cells, "empty"), (empty_cells2, "empty"),
#                                (invalid_cells, "invalid"), (invalid_cells2, "invalid")]:
#                 for c in cells:
...
#         return False
def process_fltm_soc_pok_sheet(env: Environment, xl: pd.ExcelFile, sheet_name: str, templates: List[Tuple[str, str]]) -> None:
