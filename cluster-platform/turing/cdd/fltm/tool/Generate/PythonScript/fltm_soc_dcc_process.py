...
# Configuration for FltM_Soc_Dcc sheet
FLTM_SOC_DCC_CONFIG = {
    "sheet_name": "FltM_Soc_Dcc",
    "templates": [
        ("FltM_Soc_Dcc_Cfg.c.j2", "FltM_Soc_Dcc_Cfg.c"),
        ("FltM_Soc_Dcc_Cfg.h.j2", "FltM_Soc_Dcc_Cfg.h")
    ]
}
...
def process_fltm_soc_dcc_sheet(env: Environment, xl: pd.ExcelFile, sheet_name: str, templates: List[Tuple[str, str]]) -> bool:
    """Process FltM_Soc_Dcc sheet with blank row separator and two headers."""
    try:
        ...
        for idx in range(split):
            key = df_full.iat[idx, 3]
            val = df_full.iat[idx, 4]
