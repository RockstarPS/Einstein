...
echo options C_DEFINE_LIST MK_HWWORDLENGTH=32 OS_TOOL_INLINE_KEYWORD= mk_hwexceptioninfo_t= MK_HWMAXCORES=1 VCAST_UT=1 >> commands.tmp
...
echo environment build UPDI_HSMIOADAPTER.env >> commands.tmp
echo /E:UPDI_HSMIOADAPTER tools script run UPDI_HSMIOADAPTER.tst >> commands.tmp
echo /E:UPDI_HSMIOADAPTER execute batch >> commands.tmp
echo /E:UPDI_HSMIOADAPTER reports custom management UPDI_HSMIOADAPTER_management_report.html >> commands.tmp
