...
echo options C_DEFINE_LIST MK_HWWORDLENGTH=32 MK_HWMAXCORES=1 OS_TOOL_INLINE_KEYWORD= mk_hwexceptioninfo_t= UPDd_MAX_PACKAGE_NAME_SIZE=64 UPDd_LOAD_BUFFER_SIZE=65536 UPDd_REQ_FIFO_SIZE=8 UPDd_COMPOSE_BUFFER_SIZE=1 UPDd_HASH_BUFFER_SIZE=32 UPDd_MAX_SLOT=2 VCAST_UT=1 >> commands.tmp
...
echo environment build UPDI_SBLIOADAPTER.env >> commands.tmp
echo /E:UPDI_SBLIOADAPTER tools script run UPDI_SBLIOADAPTER.tst >> commands.tmp
echo /E:UPDI_SBLIOADAPTER execute batch >> commands.tmp
echo /E:UPDI_SBLIOADAPTER reports custom management UPDI_SBLIOADAPTER_management_report.html >> commands.tmp
