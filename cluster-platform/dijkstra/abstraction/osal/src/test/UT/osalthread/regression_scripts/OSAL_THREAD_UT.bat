...
echo options C_PREPROCESS_CMD g++ -E -C -U _WIN32 -include ..\\stubs\\DKOsalMutexLock.h >> commands.tmp
...
echo options TESTABLE_SOURCE_DIR ..\..\..\..\posix\ >> commands.tmp
echo options TESTABLE_SOURCE_DIR ..\stubs\ >> commands.tmp
echo environment build OSAL_THREAD_UT.env >> commands.tmp
echo /E:OSAL_THREAD_UT tools script run OSAL_THREAD_UT.tst >> commands.tmp
echo /E:OSAL_THREAD_UT execute batch >> commands.tmp
echo /E:OSAL_THREAD_UT tools import_coverage OSAL_THREAD_UT.cvr >> commands.tmp
echo /E:OSAL_THREAD_UT reports custom management OSAL_THREAD_UT_management_report.html >> commands.tmp
