...
echo options C_COMPILE_CMD g++ -c -g -U _WIN32 >> commands.tmp
...
echo environment build OSALTIMERMGR.env >> commands.tmp
echo /E:OSALTIMERMGR tools script run OSALTIMERMGR.tst >> commands.tmp
echo /E:OSALTIMERMGR execute batch >> commands.tmp
echo /E:OSALTIMERMGR tools import_coverage OSALTIMERMGR.cvr >> commands.tmp
echo /E:OSALTIMERMGR reports custom management OSALTIMERMGR_management_report.html >> commands.tmp
