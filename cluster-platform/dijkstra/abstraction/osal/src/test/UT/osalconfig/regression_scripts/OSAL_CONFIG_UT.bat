...
echo options C_PREPROCESS_CMD g++ -E -C -include ..\\stubs\\DKOsalMutexLock.h >> commands.tmp
...
echo environment build OSAL_CONFIG_UT.env >> commands.tmp
echo /E:OSAL_CONFIG_UT tools script run OSAL_CONFIG_UT.tst >> commands.tmp
echo /E:OSAL_CONFIG_UT execute batch >> commands.tmp
echo /E:OSAL_CONFIG_UT tools import_coverage OSAL_CONFIG_UT.cvr >> commands.tmp
echo /E:OSAL_CONFIG_UT reports custom management OSAL_CONFIG_UT_management_report.html >> commands.tmp
