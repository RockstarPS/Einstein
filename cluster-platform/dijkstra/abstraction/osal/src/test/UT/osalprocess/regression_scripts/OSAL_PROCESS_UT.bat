...
echo options C_DEFINE_LIST VCAST_NO_PTR_TO_UNCON_ARRAY_SUPPORT _POSIX_C_SOURCE >> commands.tmp
...
echo options C_PREPROCESS_CMD g++ -E -C -U _WIN32 >> commands.tmp
...
echo environment build OSAL_PROCESS_UT.env >> commands.tmp
echo /E:OSAL_PROCESS_UT tools script run OSAL_PROCESS_UT.tst >> commands.tmp
echo /E:OSAL_PROCESS_UT execute batch >> commands.tmp
echo /E:OSAL_PROCESS_UT reports custom management OSAL_PROCESS_UT_management_report.html >> commands.tmp
