...
static void *UclALPhyMuASR_Impl_TimerTask ( void *pData )
{
    ...
    switch ( pInst->muState )
    {
        ...
        case MU_SETUP_ST:
            ...
            {
                pInst->pTxShMemBuffer = ( uint8 * ) pInst->pMuRegs->RR[1];
                pInst->pRxShMemBuffer = ( uint8 * ) pInst->pMuRegs->RR[0];
...
static void UclALPhyMuASR_Impl_Transfer ( SUclALPhyMuASRInst *pInst )
{
    ...
    if ( 0u != UclALPhyMuASR_Impl_Hw_IsReceiveRegisterFull ( pInst, 1 ) )
    {
    ...
    if ( ( MU_BUFFER_EMPTY == pInst->txBufferState ) && ( 0u != UclALPhyMuASR_Impl_Hw_IsTransmitRegisterEmpty ( pInst, 1 ) ) )
    {
        if ( UCL_E_OK == UclALOs_MutexLock ( 0, pInst->txRingBufferMutexId ) )
        {
    ...
    if ( 0u != UclALPhyMuASR_Impl_Hw_IsReceiveRegisterFull ( pInst, 0 ) )
    {
    ...
    if ( ( MU_BUFFER_READY == pInst->rxBufferState ) && ( 0 != UclALPhyMuASR_Impl_Hw_IsTransmitRegisterEmpty ( pInst, 0 ) ) )
    {
        if ( 0u < RxSize )
        {
            if ( UCL_E_OK == UclALOs_MutexLock ( 0, pInst->rxRingBufferMutexId ) )
