...
//#include "Os.h"
#include <string.h>
#include "UclALOsFreeRtos_Impl.h"

static void UclALOsFreeRtos_Impl_TimerProcess ( SUclALOsFreeRtosInst *pInst, uint8 InstId );

Ucl_ReturnType UclALOsFreeRtos_Impl_IUclALOs_Initialize ( SUclALOsFreeRtosInst *pInst, uint8 InstId )
{
    SUclALOsFreeRtosCfg *pCfg = pInst->pCfg;
    uint8 i;
        ...
        uint32 Size )
{
    ...
    {
        if ( ( memcmp ( ( const uint8 * ) pDst, ( const uint8 * ) pSrc, Size ) ) == ( ( int ) 0 ) )
        {
            Ret = UCL_E_OK;
        }
    }
...
Ucl_ReturnType UclALOsFreeRtos_Impl_IUclALOs_TimerStart ( SUclALOsFreeRtosInst *pInst, uint8 InstId, uint32 TimerId, uint32 Timeout )
{
    ...
    {
        if ( ( eUclOsTimerResState_Created == pCfg->pTimerList[TimerId].state ) ||
                ( eUclOsTimerResState_Stopped == pCfg->pTimerList[TimerId].state ) )
        {
...
void UclALOsFreeRtos_Impl_IUclALOs_Tick ( SUclALOsFreeRtosInst *pInst, uint8 InstId )
{
    ...
    UclALOsFreeRtos_Impl_TimerProcess ( pInst, InstId );
}
