...
* purpose, critical section, unit, and resolution                            *
******************************************************************************/
static const uint8_t Crc_Table8bit[256] =
{
...
static const uint16_t Crc_Table16bit[256] =
{
...
static const uint8_t Crc_Table8bitH2F[256] =
{
...
static const uint32_t Crc_Table32bitP4[256] =
{
...
**
**==========================================================================*/
uint8_t UclCrc_CalculateCRC8 ( const uint8_t *p_Crc_DataPtr_U8, uint32_t p_Crc_Length_U32, uint8_t p_Crc_StartValue_U8, bool p_Crc_IsFirstCall_BOOL )
{
    uint8_t l_Crc_Value_U8;
    uint32_t l_Crc_DataCount_U32;

    if ( false != p_Crc_IsFirstCall_BOOL )
    {
        /* If first CRC call, set the initial value */
        l_Crc_Value_U8 = CRC_INITIAL_VALUE8;
    }
    else
    {
        /* If continuation of CRC calculation, then revert back the last CRC value*/
        l_Crc_Value_U8 = ( CRC_FINAL_XOR_CRC8 ^ p_Crc_StartValue_U8 );
    }

    if ( NULL != p_Crc_DataPtr_U8 )
    {
        for ( l_Crc_DataCount_U32 = 0; l_Crc_DataCount_U32 < p_Crc_Length_U32; l_Crc_DataCount_U32++ )
        {
            l_Crc_Value_U8 = Crc_Table8bit[l_Crc_Value_U8 ^ p_Crc_DataPtr_U8[l_Crc_DataCount_U32]];
        }
    }

    /* Invert the final CRC Value */
    l_Crc_Value_U8 = CRC_FINAL_XOR_CRC8 ^ l_Crc_Value_U8;

    return l_Crc_Value_U8;
}
...
**                      p_Crc_StartValue_U8: CRC Start value, if this is a continuation of
**                                       previous CRC calculation. p_Crc_IsFirstCall_BOOL shall be
**                                       false for this to be considered.
**                      p_Crc_IsFirstCall_BOOL: true if this is the first call for CRC calculation. Start Value will be 0xFF
**                                       false if this is a continuation of precious CRC calculation.
**
** Outputs          :   Calculated 8 bit CRC value
**
...
**
**==========================================================================*/
uint8_t UclCrc_CalculateCRC8H2F ( const uint8_t *p_Crc_DataPtr_U8, uint32_t p_Crc_Length_U32, uint8_t p_Crc_StartValue_U8, bool p_Crc_IsFirstCall_BOOL )
{
    uint8_t l_Crc_Value_U8;
    uint32_t l_Crc_DataCount_U32;
    ...
    if ( NULL != p_Crc_DataPtr_U8 )
    {
        for ( l_Crc_DataCount_U32 = 0; l_Crc_DataCount_U32 < p_Crc_Length_U32; l_Crc_DataCount_U32++ )
        {
            l_Crc_Value_U8 = Crc_Table8bitH2F[l_Crc_Value_U8 ^ p_Crc_DataPtr_U8[l_Crc_DataCount_U32]];
        }
    }
...
**                                       previous CRC calculation. p_Crc_IsFirstCall_BOOL shall be
**                                       false for this to be considered.
**                      p_Crc_IsFirstCall_BOOL: true if this is the first call for CRC calculation. Start Value will be 0xFFFF.
**                                       false if this is a continuation of precious CRC calculation.
**
...
**
**==========================================================================*/
uint16_t UclCrc_CalculateCRC16 ( const uint8_t *p_Crc_DataPtr_U8, uint32_t p_Crc_Length_U32, uint16_t p_Crc_StartValue_U16, bool p_Crc_IsFirstCall_BOOL )
{
    uint8_t l_Crc_TableIndex_U8;
    uint16_t l_Crc_Value_U16;
    uint32_t l_Crc_DataCount_U32;

    if ( true == p_Crc_IsFirstCall_BOOL )
    {
        l_Crc_Value_U16 = CRC_INITIAL_VALUE16;
    }
    else
    {
        l_Crc_Value_U16 = p_Crc_StartValue_U16;
    }
    ...
    if ( NULL != p_Crc_DataPtr_U8 )
    {
        for ( l_Crc_DataCount_U32 = 0; l_Crc_DataCount_U32 < p_Crc_Length_U32; l_Crc_DataCount_U32++ )
        {
            l_Crc_TableIndex_U8 = ( ( uint8_t ) ( l_Crc_Value_U16 >> ( CRC_BIT_SHIFT ) ) ) ^ p_Crc_DataPtr_U8[l_Crc_DataCount_U32];
            l_Crc_Value_U16 = ( l_Crc_Value_U16 << ( CRC_BIT_SHIFT ) ) ^ Crc_Table16bit[ l_Crc_TableIndex_U8 ];
        }
    }

    return l_Crc_Value_U16;
}
...
**                                       previous CRC calculation. p_Crc_IsFirstCall_BOOL shall be
**                                       false for this to be considered.
**                      p_Crc_IsFirstCall_BOOL: true if this is the first call for CRC calculation. Start Value will be 0xFFFFFFFF.
**                                       false if this is a continuation of precious CRC calculation.
**
...
**
**==========================================================================*/
uint32_t UclCrc_CalculateCRC32 ( const uint8_t *p_Crc_DataPtr_U8, uint32_t p_Crc_Length_U32, uint32_t p_Crc_StartValue_U32, bool p_Crc_IsFirstCall_BOOL )
{
    uint32_t l_Crc_Value_U32;
    uint32_t l_DataPtrAddr_U16 = 0;
    uint32_t l_CrcLength_U32 = p_Crc_Length_U32;

    if ( true == p_Crc_IsFirstCall_BOOL )
    {
        l_Crc_Value_U32 = ( uint32_t ) ( CRC_INITIAL_VALUE32 );
    }
    else
    {
        l_Crc_Value_U32 = ( CRC_FINAL_XOR_CRC32 ^ p_Crc_StartValue_U32 );
    }

    if ( NULL != p_Crc_DataPtr_U8 )
    {
        while ( l_CrcLength_U32 > ( uint32_t ) ( CRC_MIN_LEN ) )
        {
            l_Crc_Value_U32 ^= ( uint32_t ) 0xFFUL & ( uint32_t ) p_Crc_DataPtr_U8[l_DataPtrAddr_U16];
    ...
    l_Crc_Value_U32 = CRC_FINAL_XOR_CRC32 ^ l_Crc_Value_U32;
...
** Inputs           :   p_Crc_DataPtr_U8: Pointer to the data for which the CRC to be calculated
**                      p_Crc_Length_U32: Number of bytes to be taken for calculation.
**                      p_Crc_StartValue_U32: CRC Start value, if this is a continuation of
**                                       previous CRC calculation. p_Crc_IsFirstCall_BOOL shall be
**                                       false for this to be considered.
**                      p_Crc_IsFirstCall_BOOL: true if this is the first call for CRC calculation. Start Value will be 0xFFFFFFFF.
**                                       false if this is a continuation of precious CRC calculation.
...
**
**==========================================================================*/
uint32_t UclCrc_CalculateCRC32P4 ( const uint8_t *p_Crc_DataPtr_U8, uint32_t p_Crc_Length_U32, uint32_t p_Crc_StartValue_U32, bool p_Crc_IsFirstCall_BOOL )
{
    uint32_t l_Crc_Value_U32;
    uint32_t l_DataPtrAddr_U16 = 0;
    uint32_t l_CrcLength_U32 = p_Crc_Length_U32;
    ...
    if ( NULL != p_Crc_DataPtr_U8 )
    {
        ...
        {
            l_Crc_Value_U32 ^= ( uint32_t ) 0xFFUL & ( uint32_t ) p_Crc_DataPtr_U8[l_DataPtrAddr_U16];
            l_Crc_Value_U32 = ( l_Crc_Value_U32 >> ( CRC_BIT_SHIFT ) ) ^ ( Crc_Table32bitP4[ ( uint8_t ) ( l_Crc_Value_U32 )] );
            l_CrcLength_U32--;
            l_DataPtrAddr_U16++;
        }
    }
...
void UclCrc_GetVersionInfo ( VersionInfoType *p_Versioninfo )
{
    if ( p_Versioninfo != NULL )
    {
        p_Versioninfo->vendorID = CRC_VENDOR_ID;
        p_Versioninfo->moduleID = CRC_MODULE_ID;
        p_Versioninfo->sw_major_version = CRC_SW_MAJOR_VERSION;
        p_Versioninfo->sw_minor_version = CRC_SW_MINOR_VERSION;
        p_Versioninfo->sw_patch_version = CRC_SW_PATCH_VERSION;
    }
}
