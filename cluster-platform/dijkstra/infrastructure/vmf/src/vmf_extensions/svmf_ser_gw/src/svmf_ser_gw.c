...
int main(int argc, char *argv[])
{
    ...
    while (true)
    {
        ...
        if (read_count > 0)
        {
            ...
            if (VMF_OK != vmf_convert_svmf2vmf(&(p_tr->svmf_msg),read_count,&vmf_msg,sizeof(vmf_msg)))
            {
            ...
            switch (NW_VMF_MSG_EVENT(p_vmf_msg))
            {
                case VMF_TRACE_CTRL_REG_CLIENT:
                {
                    ...
                    if (VMF_TRACE_REGISTER(&ltrid, rtrid_name) ==VMF_OK)
                    {
                        ...
                        ltrids[rtrid] = ltrid;
                        rtrids[ltrid] = rtrid;
                        DEBUG_INFO_LO(VMF_TRACE_TASK_VMF, "register %s with remote trid %d as local trid %d", rtrid_name, rtrid,ltrid);
                    }
                    bret = false;
                }
                break;
                ...
                case VMF_TRACE_CTRL_FILTER_RESP:
                {
                    ...
                    if (0 == ltrid)
                    {
                        ...
                        if (VMF_TRACE_REGISTER(&ltrid, rtrid_name) ==VMF_OK)
                        {
                            ...
                            ltrids[rtrid] = ltrid;
                            rtrids[ltrid] = rtrid;
                            DEBUG_INFO_LO(VMF_TRACE_TASK_VMF, "VMF_TRACE_CTRL_FILTER_RESP: register %s with remote trid %d as local trid %d", rtrid_name, rtrid,ltrid);
                        }
                    }
    ...
    switch (NW_VMF_MSG_GROUP(p_vmf_msg))
    {
        case VMF_TRACE_CTRL:
        {
            ...
            switch (NW_VMF_MSG_EVENT(p_vmf_msg))
            {
                ...
                {
                    DEBUG_INFO_LO(VMF_TRACE_TASK_VMF, "VMF_TRACE_CTRL_FILTER_REQ");
                    bret = true;
                }
                break;
...
static void vmf_ser_sender (nw_vmf_mi_transfer_v2_t *tr_msg)
{
    ...
    if (tr_msg != NULL)
    {
        ...
        DEBUG_INFO_HI(VMF_TRACE_TASK_VMF,"len = %d, tr_msg->svmf_len = %d",len,tr_msg->svmf_len);
...
static bool GetOpt(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "?hHc:t:r:e:vf:l:db:a")) != -1)
    {
        switch (opt)
        {
            case '?':
...
static void usage(void)
{
    printf("\n");
    printf("************************************************************************\n");
    printf("** SVMF-serial gateway 2.0 J. Becker 2014                             **\n");
    printf("************************************************************************\n");
    printf("**                                                                    **\n");
...
static void vmf_mi_auto_start_vmf(HANDLE hdl)
{
    ...
    const   char dab_trace[] ={"dab_trace\n"};
    const   char ch_A5[] = {"A5 >"};
    const   char ch_M4[] = {"M4 >"};
 ...
 //--------------------------
    DEBUG_INFO_HI(VMF_TRACE_TASK_MAIN,"auto start vmf");
    /* send CR */
    DEBUG_INFO_HI(VMF_TRACE_TASK_MAIN,"send newline");
    WriteSlowSerial(sizeof(cr)-1, (unsigned8 *)cr);
    delay(100);

    /* start vmf */
    DEBUG_INFO_HI(VMF_TRACE_TASK_MAIN,"send vmf_start");
    ...
    /* send start command */
    DEBUG_INFO_HI(VMF_TRACE_TASK_MAIN,"send start byte");
...
//    printf("flashing... %3.1f%% done\n",fpc);
    printf("flashing... %3.1f%% done\r",fpc);
}
