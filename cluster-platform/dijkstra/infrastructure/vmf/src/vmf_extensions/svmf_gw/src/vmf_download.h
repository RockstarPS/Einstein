...
/* message format for VMF_TRACE_DOWNLOAD_TRANSFER_REQ_RESP    */
typedef struct
{
    unsigned8   version;            /* version - 0x00 for version 0 */
    unsigned8   fill;               /* fill byte */
    ...
    unsigned8   response;           /* 0x00 = NAK, 0x01=ACK, 0x02=ignore respone */
    unsigned8   resp_block;         /* response block number*/
    unsigned8   block_len_l;        /* next message length in bytes bit 0-  7 */
    unsigned8   block_len_h;        /* next message length in bytes bit 8- 15 */
}vmf_download_trans_resp_req_t;
