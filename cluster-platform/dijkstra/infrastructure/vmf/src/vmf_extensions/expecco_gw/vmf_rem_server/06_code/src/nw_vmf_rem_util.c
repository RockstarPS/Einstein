...
vmf_error_t vmf_rem_get_vmf_msg_len(unsigned16 *p_msglen, vmf_msg_t *p_vmf_msg)
{
	...
	switch (p_vmf_msg->vmf_most_msg.msg_base.base.msg_type)
	{
		...
		case MSG_TYPE_MOST_MESSAGE:
			*p_msglen = sizeof(VMF_VAR_MOST_MSG_TYP(0)) + (sizeof(p_vmf_msg->vmf_most_msg.data.pl[0])*(p_vmf_msg->vmf_most_msg.data.length))+4;
		break;

		case MSG_TYPE_BASIC_MESSAGE:
			*p_msglen = sizeof(VMF_VAR_BASIC_MSG_TYP(0)) + (sizeof(p_vmf_msg->vmf_basic_msg.data.pl[0])*(p_vmf_msg->vmf_basic_msg.data.length))+4;
			break;
