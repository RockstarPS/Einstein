...
#define VMF_SAT_MSG_EVT		        1       /* saturn message event */
#define VMF_SAT_CMD_EVT         	2       /* saturn command event */
...
bool vmf_send_io_command(unsigned16 len, char *cmd)
{
    ...
    VMF_BASIC_MSG_EVENT_ID(p_vmf_msg)       = VMF_SAT_MSG_EVT;
...
static void vmf_message_handler (vmf_basic_msg_t *vmf_basic_msg)
{
    ...
    if (VMF_SAT_MSG_EVT == NW_VMF_BASIC_MSG_EVENT(vmf_basic_msg))
    {
        len = NW_VMF_BASIC_MSG_DATA_LENGTH(vmf_basic_msg);
        if (len <= IO_MSG_DATALEN_MAX)
        {
            DEBUG_INFO_HI(VMF_TRACE_TASK_VMF, "Received VMF_SAT_MSG_EVT message");
	...
	else if (VMF_SAT_CMD_EVT == NW_VMF_BASIC_MSG_EVENT(vmf_basic_msg))
    {
	}
...
static bool GetOpt(int argc, char *argv[])
{
    ...
    while ((opt = getopt(argc, argv, "?hHc:t:r:e:vf:l:d")) != -1)
    {
        switch (opt)
        {
            case '?':
...
{
    printf("************************************************************************\n");
    printf("** NXP-Saturn VMF COM GW 3.0 by J. Becker 2014                        **\n");
    printf("************************************************************************\n");
    printf("**                                                                    **\n");
 ...
 {
    printf("************************************************************************\n");
    printf("**  NXP-Saturn VMF COM GW 3.0 by J. Becker %s %s\n",__DATE__,__TIME__);
    printf("************************************************************************\n");
    printf("** verbosity=%d                                                         \n",verbose);
