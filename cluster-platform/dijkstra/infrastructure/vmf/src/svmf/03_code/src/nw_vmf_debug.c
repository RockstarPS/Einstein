...
void nw_vmf_debug_msgs(char *prefix, pid_t pid, vmf_msg_t *p_my_msg)
{
    ...
    switch (p_my_msg->vmf_basic_msg.msg_base.base.msg_type)
    {
        ...
        case MSG_TYPE_BASIC_MESSAGE:
            p_buffer += sprintf(p_buffer, "%s pid=%d --> group=0x%x,event=0x%x,data-len=0x%x"
                                ,prefix ,pid ,p_my_msg->vmf_basic_msg.msg_base.id.group,p_my_msg->vmf_basic_msg.msg_base.id.event
