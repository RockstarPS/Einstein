...
int nw_os_sem_timedwait(sem_t *sem,unsigned16 ms_time)
{
    ...
    {
        (void) fprintf(stderr,"nw_os_sem_timedwait: clock_gettime error\n");
    }
    else
    {
        /* add timeout to timespec */
        nw_os_set_normalized_timespec (&ts_timeout, ts.tv_sec, (unsigned64)ts.tv_nsec + ((unsigned64)ms_time * 1000000L));
...
void nw_os_set_thread_name  (pthread_t th,char *thread_name)    /*!< INPUT pointer to thread name */
{
    ...
    if ( pthread_setname_np(th, thread_name) != EOK)
    {
        perror(thread_name);
    }
#endif	
 ...
 *
 *   Description:   set the tread name of current thread
 *
 *   Parameters:    thread handle
 *                  thread name         (input) pointer to thread name\n
 *
