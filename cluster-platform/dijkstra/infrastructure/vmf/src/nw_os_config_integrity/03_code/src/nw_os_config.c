...
int nw_get_app_name(char *buffer, int buffer_len)
{
    Error           ret_val;
    AddressSpace    me;
    char            address_space_name[MAX_PROGNAME_LEN] = {0};
    Address         actual_length;

    me = GetCurrentAddressSpace();
    ret_val = GetAddressSpaceName(me, &address_space_name[0], MAX_PROGNAME_LEN-1, &actual_length);  /* no 0 termination! */ 
    if (ret_val != Success)
    {
        printf("nw_get_app_name: GetAddressSpaceName() failed with ret_val=%d \n", ret_val);
    }
#if 0
    else
    {
        printf("nw_get_app_name: GetAddressSpaceName() len=%d, name=%s \n", actual_length, address_space_name);
    }
#endif

    nw_os_sstrncpy(buffer, &address_space_name[0], buffer_len);
    return 0;
}
...
bool nw_os_target_log(unsigned int severity, char *log_message)
{
    ...
    {
        my_severity = LOG_WARNING;
    }
    else
    {
        my_severity = LOG_INFO;
    }

    syslog(my_severity, "%s", log_message);
    return true;
}

#endif /* VMF_TARGET_LOGGING */
