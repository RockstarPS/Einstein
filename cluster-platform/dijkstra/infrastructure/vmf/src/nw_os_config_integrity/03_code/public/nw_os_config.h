...
/* constants                                                            */
/*****************************************************************************/
#define VMF_OS_STRING               "VMF for Integrity"
...
/* add incr and return OLD value */
static unsigned nw_atomic_add_value(volatile unsigned * loc, unsigned incr)
{
    Error ret_val;
    unsigned8 old_value;
    ret_val = AtomicModify((Address *)loc, (Address *)&old_value, 0, incr);
    if (ret_val != Success)
    {
        printf("AtomicModify failed Error=%d\n", ret_val);
    }
#if 0
    else
    {
        printf("AtomicModify - old_value=%d, new_value=%d\n", old_value, (int) *loc);
    }
#endif    
    return old_value;
}
...
#if 0
/* inc and return new value */
static inline unsigned nw_atomic_inc(volatile unsigned * loc)
{
    unsigned ret = 0;
    Address old_value;
    (void) AtomicModify((Address *)loc, &old_value, 0xFFFF, 1);
    ret = *loc;
    return ret;
}
