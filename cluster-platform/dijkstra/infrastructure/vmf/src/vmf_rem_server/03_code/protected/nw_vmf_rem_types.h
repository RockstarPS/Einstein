...
#define VMF_REM_MAX_DATA_SIZE       (MAX_VMF_DATA_LEN)      /* limit the maximum data size of a remote message */
...
typedef struct
{
    union 
    {
        unsigned16        length;                /* length if payload */
        struct
        {
            unsigned8    length_l;
            unsigned8    length_h;
...
#define NW_VMF_RPC_CONN_MAX_NAME_LEN    20
/*! rpc data for nw_vmf_connect() */
typedef struct
{
    char                process_name[NW_VMF_RPC_CONN_MAX_NAME_LEN]; /* process name */
...
typedef struct
{
    length16_t          msg_length;                    /* length of of total message (including msg_length)*/
