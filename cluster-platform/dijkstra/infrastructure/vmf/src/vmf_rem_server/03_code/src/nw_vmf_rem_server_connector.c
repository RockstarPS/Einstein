...
vmf_ret_t nw_vmf_create_rem_vmf_thread  (void)
{
    ...
    vmf_trace_us_time_offset = nw_os_cfg_get_us_time();     /* get time offset for later use */
    
    (void)pthread_attr_init(&attr);
    (void)pthread_attr_setdetachstate(&attr, PTHREAD_CREATE_DETACHED );

    #ifdef INTEGRITY
    (void)pthread_attr_setthreadname(&attr, "vmf rem server");
    #endif
    ...
    if (EOK != iRet)
    {
        NW_REM_VMF_DEBUG_ERROR("rem_vmf: nw_vmf_create_rem_vmf_thread() - pthread_attr_setstacksize() failed: %d", iRet);
    }
    #endif

    iRet = pthread_create (&th  , &attr,vmf_client_rpc_handler,NULL);
    (void)pthread_attr_destroy(&attr);
    if (EOK != iRet)
    {
        NW_REM_VMF_DEBUG_ERROR("rem_vmf: nw_vmf_create_rem_vmf_thread() - pthread_create() failed: %d", iRet);
        return VMF_ERROR;
    }

#if ((defined(__QNX__) && (_NTO_VERSION >= 632)) || defined(__LINUX__))		
    /* Name the vmf-remote-server thread */
	sprintf(Thread_name, "Rem-Svc-Connector-%d", th);
...
void *vmf_client_rpc_handler(void *pArg)
{
    ...
    VMF_TRACE_PRINTF(vmf_trace_trid,VMF_TRACE_SEVERITY_INFO_10,"vmf_rem","vmf_client_rpc_handler","connected to vmf_trace server");
...
VMF_REM_SERV_STATIC void *vmf_client_rpc_receiver(SOCKET *rpc_socket)
{
    ...
    int                 num_bytes;
    SOCKET              my_sock;
    vmf_client_id_t     all_cids[VMF_RPC_MAX_SERVER_CONNECTIONS];
    unsigned8           max_cid_ix=0;
    ...
    *rpc_socket = 0;        /* signal that this thread is running */
    ...
    /* init all_cids */
    for (max_cid_ix=0,cnt = 0;cnt < VMF_RPC_MAX_SERVER_CONNECTIONS; cnt++)
    {
        all_cids[cnt]=-1;
    }
    ...
    while (true)
    {
        ...
        NW_REM_VMF_DEBUG_INFO("rem_vmf: vmf_client_rpc_receiver() - message len=%d",rpc_buffer.msg_length.length);
        if (rpc_buffer.msg_length.length > (sizeof(rpc_buffer)-sizeof(rpc_buffer.msg_length)))
        {
            NW_REM_VMF_DEBUG_ERROR("rem_vmf: vmf_client_rpc_receiver() -invalid message len!!! =%d",rpc_buffer.msg_length.length);
            break;
        }
        ...
        if (num_bytes > 0)
        {
            ...
            if (num_bytes > 0)
            {
                ...
                NW_REM_VMF_DEBUG_INFO("rem_vmf: vmf_client_rpc_receiver() - call vmf_client_rpc_decoder(buffer addr=%p) ",&rpc_buffer);
                NW_REM_VMF_DEBUG_INFO("rem_vmf: vmf_client_rpc_receiver() - call vmf_client_rpc_decoder(rcid = %d) ",rpc_buffer.r_cid);
                vmf_ret = vmf_client_rpc_decoder(&rpc_buffer,p_rpc_response);
                ...
                if (result == SOCKET_ERROR) 
                {
                    ...
                    #if 0
                    perror("rem_vmf: vmf_client_rpc_receiver()");
                    #endif
                    vmf_ret = VMF_ERROR;
                    break;
                }

                /* store cid for each new connect */
                if (VMF_RPC_FKT_CONNECT == rpc_buffer.rpc_id)
                {
                    if (max_cid_ix < VMF_RPC_MAX_SERVER_CONNECTIONS)
                    {
                        all_cids[max_cid_ix++] = p_rpc_response->response;
                    }
                } 
#if 0 /* spy_client set but never used!? */
                ...
                else if (VMF_RPC_FKT_BLOCK_RECEIVE == rpc_buffer.rpc_id)
                {
                    ...
                    vmf_client_rpc_block_receive(&my_sock,&rpc_buffer);
                    break;
                }               
                #endif
                else
            ...
            else
            {
                ...
                /* most likely the connection was closed by the client */
                vmf_ret = VMF_ERROR;
                NW_REM_VMF_DEBUG_INFO("rem_vmf: vmf_client_rpc_receiver() recv2 failed: %d", WSAGetLastError());
                break;
            }
        ...
        else
        {
            ...
            /* most likely the connection was closed by the client */
            vmf_ret = VMF_ERROR;
            NW_REM_VMF_DEBUG_INFO("rem_vmf: vmf_client_rpc_receiver() recv1 failed: %d", WSAGetLastError());
            break;
        }
    ...
    (void) closesocket(my_sock);

    /* close all cids */
    if (max_cid_ix > 0)
    {
        for (cnt = 0;cnt < max_cid_ix; cnt++)
        {
            if (all_cids[cnt] > 0)
            {
                NW_REM_VMF_DEBUG_INFO("vmf_client_rpc_receiver() nw_vmf_disconnect(%d)",all_cids[cnt]);
                (void)nw_vmf_disconnect(all_cids[cnt]);
            }
        }
    }
    NW_REM_VMF_DEBUG_INFO("vmf_client_rpc_receiver() socket closed %d",my_sock);
    return NULL;
}
...
VMF_REM_SERV_STATIC void vmf_client_rpc_block_receive(SOCKET *rpc_socket,vmf_rpc_msg_t   *prpc_buffer)
{
    ...
    *rpc_socket = 0;        /* signal that this thread is running */        
    ...
    tx_time   =  vmf_rem_convert_clocks_to_ms(nw_os_cfg_get_ms_time());
    ...
    {
        /* decode message and call rpc function */
        vmf_ret = vmf_client_rpc_decoder(prpc_buffer,p_rpc_response_2,time_left);   /* jmerkle TBD: timeout removed, since remote client now has timeout implemented thru rpc message structure! */
        if (vmf_ret > 0)
        {
            /* we received data */
            current_time = vmf_rem_convert_clocks_to_ms(nw_os_cfg_get_ms_time());
...
VMF_REM_SERV_STATIC vmf_ret_t nw_vmf_create_rx_thread  (SOCKET *p_sock)
{
    ...
    /* set the thread stack size*/
    #ifndef __ANDROID__
    iRet = pthread_attr_setstacksize  (&attr, REM_VMF_RX_THREAD_STACK_SIZE);
    if (EOK != iRet)
    {
        VMF_TRACE_PRINTF(vmf_trace_trid,VMF_TRACE_SEVERITY_ERROR_10,"vmf_rem","nw_vmf_create_rx_thread","pthread_attr_setstacksize() failed ");
    }
    #endif
    ...
    if (EOK != iRet)
    {
        VMF_TRACE_PRINTF(vmf_trace_trid,VMF_TRACE_SEVERITY_WARNING_19,"vmf_rem","nw_vmf_create_rx_thread"," pthread_create() failed: %d",iRet);
    }

#if ((defined(__QNX__) && (_NTO_VERSION >= 632)) || defined(__LINUX__))		
    /* Name the vmf-remote-server thread */
	sprintf(Thread_name, "Rem-RPC-Rcv-%d", th);
	nw_os_set_thread_name(th, Thread_name);		
#endif
