...
/* trace macros */
    ...
    #define VMF_TRACE_PRINT(trid,severity,task,scope,msg)       vmf_trace(trid,severity,task,scope,msg)
    #define VMF_TRACE_PRINTF(trid,severity,task,scope,msg,...)  (void)vmf_trace_printf(trid,severity,task,scope,msg,##__VA_ARGS__)
    #define VMF_TRACE_HEX(trid,severity,task,scope,len,data)    vmf_trace_hex(trid,severity,task,scope,len,data)
    #define VMF_TRACE_SET_SEVERITY(trid,severity)         		vmf_trace_set_severity(trid,severity)
#else
    #ifndef WIN32
        #define VMF_TRACE_REGISTER(handle,name)                     (VMF_OK)
        #define VMF_TRACE_PRINT(trid,severity,task,scope,msg)       
        #define VMF_TRACE_PRINTF(trid,severity,task,scope,msg,...)  
        #define VMF_TRACE_HEX(trid,severity,task,scope,len,data)    
        #define VMF_TRACE_SET_SEVERITY(trid,severity)         		
    #else
        #define VMF_TRACE_REGISTER(handle,name)                     (VMF_OK)
        #define VMF_TRACE_PRINT(trid,severity,task,scope,msg)       (void)printf(scope);(void)printf(" ");(void)printf(msg);(void)printf("\n");(void)fflush(ERROR_CHANNEL);       
        #define VMF_TRACE_PRINTF(trid,severity,task,scope,msg,...)  (void)printf(args);(void)printf("\n");(void)fflush(ERROR_CHANNEL);  
        #define VMF_TRACE_HEX(trid,severity,task,scope,len,data)    
        #define VMF_TRACE_SET_SEVERITY(trid,severity)         		
    #endif
#endif
...
DLL_EXPORT  vmf_ret_t   vmf_trace_register    (unsigned8 *ptrid,const char *trid_name);
DLL_EXPORT  vmf_ret_t 	vmf_trace_register_c  (unsigned8 *ptrid,const char *trid_name,unsigned32 collector_timeout);
DLL_EXPORT  vmf_ret_t   vmf_trace             (unsigned8 trid,unsigned8 severity,const char *task,const char *scope,const char *message);
DLL_EXPORT  vmf_ret_t   vmf_trace_printf      (unsigned8 trid,unsigned8 severity,const char *task,const char *scope,const char *format, ... );
DLL_EXPORT  vmf_ret_t   vmf_trace_hex         (unsigned8 trid,unsigned8 severity,const char *task,const char *scope,unsigned8 len,const unsigned8 *data);
