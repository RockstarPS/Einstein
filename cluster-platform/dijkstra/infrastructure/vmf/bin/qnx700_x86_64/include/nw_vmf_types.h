...
/*! VMF Limits */
#define MIN_VMF_MESSAGE_LEN                 ((unsigned16)sizeof(VMF_VAR_MSG_TYP(1)))
...
typedef struct
{
    ...
    unsigned8           pl[VMF_BASIC_MSG_NEAR_DATA_LENGTH]; /* payload */
}vmf_event_data_t;
...
/* payload of time sync message */
typedef struct
{
    unsigned8   time_ll;                /* current time in ms  bit  0..7  */
    unsigned8   time_lh;                /* current time in ms  bit  8..15 */
    unsigned8   time_hl;                /* current time in ms  bit 16..23 */
    unsigned8   time_hh;                /* current time in ms  bit 24..31 */
    char    server_name[MAX_NAME_LEN+1];/* VMF time-server name  zero terminated */
}vmf_ctrl_time_sync_t;
...
/* connect ctrl structure for nw_vmf_connect() */
typedef struct
{
    vmf_handle_t            p2p_handle;                     /**< tx handle for p2p connection */
    unsigned8               unpack_vmf_spy_msgs;            /**< unpack spy messages (collected spy messages) */
}nw_vmf_connect_ctrl_t;
