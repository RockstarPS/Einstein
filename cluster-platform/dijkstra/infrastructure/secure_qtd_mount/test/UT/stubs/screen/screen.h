...
/**
* @brief Deprecated.
*        Use available feature preprocessor definitions instead.
...
/** @brief The feature preprocessor definition that indicates the definition of the
...
*          All Screen API objects, with the exception of events, are created within the scope of a
...
enum {
	...
	*                                         now connected, or that a device has disconnected and been
	...
	*/
	...
	/** A pair of integers, each in the range of [0..@c SHRT_MAX], that define the width and height,
	*   in pixels, of a clipped source rectangular viewport within the source buffers.
	*   When retrieving or setting this property type, ensure that you have sufficient storage for
	*   two integers. Specify the width and height as [@c INT_MIN, @c INT_MIN] to disable the source
	*   clip rectangle.
	*   - Configurable: No
	*   - Default privileges: @code rw-r--r-- @endcode
	*   - Change event sent: No
	...
	*              This property can be retrieved or set for an event object.
	*     - stream and window: The timestamp (in nanoseconds) to indicate the start of a frame. The
	...
	*                          time when NULL is specified for this property.
	*
	*                          For example, an application can measure the CPU time that's elapsed between when it
	...
	*/
	...
	/** A pair of integers, each in the range of [0..@c INT_MAX/256], that define the width and height,
...
typedef struct _screen_mode {
	...
	_Uint32t flags;             /**< Mutex flags of display */
...
enum {
	...
	*
	*   When you register for @c SCREEN_NOTIFY_UPDATE notifications with a NULL object, then
...
*
*   You can use this function to set the value of the following properties:
*   - @c #SCREEN_PROPERTY_BLUE_GAMMA_CURVE [1]
*   - @c #SCREEN_PROPERTY_BRIGHTNESS
*   - @c #SCREEN_PROPERTY_CBABC_MODE [1]
*   - @c #SCREEN_PROPERTY_CONTRAST
*   - @c #SCREEN_PROPERTY_GAMMA [1]
*   - @c #SCREEN_PROPERTY_GREEN_GAMMA_CURVE [1]
*   - @c #SCREEN_PROPERTY_HUE
*   - @c #SCREEN_PROPERTY_INTENSITY [1]
*   - @c #SCREEN_PROPERTY_MIRROR_MODE
*   - @c #SCREEN_PROPERTY_MODE
*   - @c #SCREEN_PROPERTY_POWER_MODE [2]
*   - @c #SCREEN_PROPERTY_PROTECTION_ENABLE
*   - @c #SCREEN_PROPERTY_RED_GAMMA_CURVE [1]
*   - @c #SCREEN_PROPERTY_ROTATION
*   - @c #SCREEN_PROPERTY_SATURATION
*   - @c #SCREEN_PROPERTY_SCALE_FACTOR
*   - @c #SCREEN_PROPERTY_VIEWPORT_POSITION
*   - @c #SCREEN_PROPERTY_VIEWPORT_SIZE
*
*   Note [1]: You don't need to be working within a privileged context of type
*             @c #SCREEN_DISPLAY_MANAGER_CONTEXT to be able to set this property.
*
*   Note [2]: You must be working within a privileged context of type
...
*   @param  count The number of rectangles supplied in the @c read_rects argument.
...
*   Note that pixmaps can have only one buffer. For more information, see the "Buffers"
...
*     the @c @c #SCREEN_PROPERTY_USAGE property to include the #SCREEN_USAGE_READ and/or
...
*   Note that streams can have multiple buffers. For more information, see the "Buffers"
*   chapter of the <em>Screen Developer's Guide</em>.
*
...
*   the "Buffers" chapter of the <em>Screen Developer's Guide</em>.
...
*                 specified by @c #SCREEN_PROPERTY_COLOR.
*
*   @return @c 0 if successful, or @c -1 if an error occurred
*           (@c errno is set; refer to @c errno.h for more details).
*/
...
*
*   Call screen_destroy_group() when you no longer need the window group.
*
...
*                 parameter are:
*                 - @c #SCREEN_WAIT_IDLE: When you use @c #SCREEN_WAIT_IDLE, this function
*                                         returns only when the contents of the display have been
*                                         updated and at least one render buffer is available.
*                 - 0: When you use 0, this function returns immediately if there is at least one
*                      render buffer available.
*
*   @return @c 0 if successful, or @c -1 if an error occurred
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/graphics/branches/release-2.1.x/lib/screen/public/screen/screen.h $ $REV$")
#endif
