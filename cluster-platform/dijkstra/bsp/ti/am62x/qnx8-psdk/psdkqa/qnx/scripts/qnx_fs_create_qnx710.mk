...
ifeq ($(SOC),$(filter $(SOC), am62a))
	mkdir -p ${QNX_FS_PATH}/usr/lib/graphics
	mkdir -p ${QNX_FS_PATH}/usr/lib/graphics/am62a
endif
ifeq ($(SOC),$(filter $(SOC), am62px))
	mkdir -p ${QNX_FS_PATH}/usr/lib/graphics
	mkdir -p ${QNX_FS_PATH}/usr/lib/graphics/am62p
endif
...
else ifeq ($(SOC),$(filter $(SOC), am62x am62a am62px))
	######## screen package start ######
	##### Graphics support - apps
ifneq ("$(wildcard $(QNX_TARGET)/$(QNX_ARCH)/bin/screencmd)","")
	cp -rfL ${QNX_TARGET}/${QNX_ARCH}/bin/screencmd          ${QNX_FS_PATH}/bin/
	cp -rfL ${QNX_TARGET}/${QNX_ARCH}/bin/screeninfo         ${QNX_FS_PATH}/bin/
	cp -rfL ${QNX_TARGET}/${QNX_ARCH}/sbin/screen            ${QNX_FS_PATH}/sbin/
	cp -rfL ${QNX_TARGET}/${QNX_ARCH}/usr/bin/events         ${QNX_FS_PATH}/usr/bin/
	...
	@echo Warning: "Screen Base Graphics" package is missing!
endif
endif
ifeq ($(SOC),$(filter $(SOC), j721e))
...
ifneq ("$(wildcard $(QNX_TARGET)/$(QNX_ARCH)/usr/lib/graphics/am62x)","")
	cp -rfL ${QNX_TARGET}/${QNX_ARCH}/usr/lib/graphics/am62x/*    ${QNX_FS_PATH}/usr/lib/graphics/am62x/
else
	@echo Warning: "Screen Board Support for TI AM62x" package is missing!
endif
else ifeq ($(SOC),$(filter $(SOC), am62a))
ifneq ("$(wildcard $(QNX_TARGET)/$(QNX_ARCH)/usr/lib/graphics/am62a)","")
	cp -rfL ${QNX_TARGET}/${QNX_ARCH}/usr/lib/graphics/am62a/*    ${QNX_FS_PATH}/usr/lib/graphics/am62a/
else
	@echo Warning: "Screen Board Support for TI AM62A" package is missing!
endif
else ifeq ($(SOC),$(filter $(SOC), am62px))
ifneq ("$(wildcard $(QNX_TARGET)/$(QNX_ARCH)/usr/lib/graphics/am62p)","")
	cp -rfL ${QNX_TARGET}/${QNX_ARCH}/usr/lib/graphics/am62p/*    ${QNX_FS_PATH}/usr/lib/graphics/am62p/
else
        @echo Warning: "Screen Board Support for TI AM62PX" package is missing!
endif
endif
	######## screen package end ######
...
ifeq ($(SOC),$(filter $(SOC), j721e))
	...
	cp -rfL ${PSDK_QNX_PATH}/qnx/bsp/images/ifs-j721s2-evm-ti.raw                                       ${QNX_BOOT_PATH}/qnx-ifs
else ifeq ($(SOC),$(filter $(SOC), am62x))
	...
	cp -rfL ${PSDK_QNX_PATH}/qnx/bsp/images/ifs-am62p-evm-ti.raw                                        ${QNX_BOOT_PATH}/qnx-ifs
else ifeq ($(SOC),$(filter $(SOC), j784s4))
...
ifeq ($(SOC),$(filter $(SOC), j721e))
	...
	cp -rfL ${PSDK_QNX_PATH}/qnx/scripts/u-boot/uEnv_am62a.txt              ${QNX_BOOT_PATH}/uEnv.txt
else ifeq ($(SOC),$(filter $(SOC), am62px))
	cp -rfL ${PSDK_QNX_PATH}/qnx/scripts/u-boot/uEnv_am62px.txt              ${QNX_BOOT_PATH}/uEnv.txt
endif
