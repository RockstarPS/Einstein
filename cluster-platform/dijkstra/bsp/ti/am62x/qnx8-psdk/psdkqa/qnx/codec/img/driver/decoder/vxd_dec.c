				       ...
				       enum vdec_vid_std vid_std)
{
	...
	int32 attributes = 0, heap_id = 0, size = 0;
	int32 i, ret = 0;
	...
	for (i = 0 ; i < MAX_SEQUENCES ; i++) {
		ret = img_mem_alloc(vxd_dev->dev, ctx->mem_ctx, heap_id,
				    ...
				    (int32 *)&fw_sequ[i].ddbuf_info.buf_id);
		if (ret) {
			OSA_DEV_ERR(dev, "Couldn't allocate sequ buffer %d\n", i);
			return -OSA_ENOMEM;
		}
		ret = img_mem_map_km(ctx->mem_ctx,
				     fw_sequ[i].ddbuf_info.buf_id);
		if (ret) {
			OSA_DEV_ERR(dev, "Couldn't map sequ buffer %d\n", i);
			return -OSA_ENOMEM;
		}
		...
		if (ret) {
			OSA_DEV_ERR(dev, "sps core_stream_map_buf failed\n");
			return ret;
		}
	}
	...
	for (i = 0 ; i < MAX_PPSS ; i++) {
		ret = img_mem_alloc(vxd_dev->dev, ctx->mem_ctx, heap_id,
				    size, (enum mem_attr)attributes,
				    (int32 *)&fw_pps[i].ddbuf_info.buf_id);
		if (ret) {
			OSA_DEV_ERR(dev, "Couldn't allocate sequ buffer %d\n", i);
			return -OSA_ENOMEM;
		}
		...
		if (ret) {
			OSA_DEV_ERR(dev, "pps core_stream_map_buf failed\n");
			return ret;
		}
	}
	return 0;
