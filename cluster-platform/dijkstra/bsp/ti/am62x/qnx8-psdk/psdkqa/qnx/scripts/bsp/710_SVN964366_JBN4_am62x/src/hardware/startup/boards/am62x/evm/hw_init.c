 ...
 */
#include "startup.h"
#include "AM62x_pinmux.h"
#include <hw/omap_i2c.h>
#include "board.h"
#include "ti_sci.h"

#define AM62X_CTRLMMR_PADCONFIG 0xF4000
#define AM62X_WKUP_CTRLMMR_PADCONFIG    (0x43000000 + 0x1C000)

#define AM62X_GPIO0_BASE        0x600000
#define AM62X_GPIO_DIR(x)       (0x10 + ((x) / 32) * 0x28)
#define AM62X_GPIO_SET_DATA(x)  (0x18 + ((x) / 32) * 0x28)
#define AM62X_GPIO_BIT(x)       (1 << ((x) % 32))
...
static int init_usb_host(int host)
{
	...
	pinmux_ctrl = AM62X_CTRLMMR_PADCONFIG + PIN_USB0_DRVVBUS;
    }
    else // host == 1
    {
	...
	pinmux_ctrl = AM62X_CTRLMMR_PADCONFIG + PIN_USB1_DRVVBUS;
    }
...
{
    // pinmux for I2C0
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_I2C0_SCL, PIN_INPUT_ENABLE | PIN_MODE(0));
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_I2C0_SDA, PIN_INPUT_ENABLE | PIN_MODE(0));

    // pinmux for I2C1
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_I2C1_SCL, PIN_INPUT_ENABLE | PIN_MODE(0));
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_I2C1_SDA, PIN_INPUT_ENABLE | PIN_MODE(0));
}

static void init_gpu(void)
{
    if (ti_sci_set_device_state(TISCI_DEV_GPU_RS_BW_LIMITER2, 0, MSG_DEVICE_SW_STATE_ON) == -1) {
        kprintf("%s: Set GPU RS BW LIMITER2 device state failed!\n", __func__);
    }
    if (ti_sci_set_device_state(TISCI_DEV_GPU_WS_BW_LIMITER3, 0, MSG_DEVICE_SW_STATE_ON) == -1) {
        kprintf("%s: Set GPU RS BW LIMITER3 device state failed!\n", __func__);
    }
...
static int init_hdmi(void)
{
    ...
    for (pin = 0; pin < sizeof(hdmi_pins) / sizeof(uint16_t); pin++) {
        ...
        out32(AM62X_CTRLMMR_PADCONFIG + hdmi_pins[pin], hdmi_pincfg);
    }
...
static void init_ospi(void)
{
    ...
    // Pinmux
    out32(AM62X_CTRLMMR_PADCONFIG + 0x0,  PIN_MODE(0) | PIN_PULL_DISABLE); //CLK
    out32(AM62X_CTRLMMR_PADCONFIG + 0x8,  PIN_INPUT_ENABLE | PIN_PULL_DISABLE | PIN_MODE(0)); //DQS
    out32(AM62X_CTRLMMR_PADCONFIG + 0xc,  PIN_INPUT_ENABLE | PIN_PULL_DISABLE | PIN_MODE(0)); //D0
    out32(AM62X_CTRLMMR_PADCONFIG + 0x10, PIN_INPUT_ENABLE | PIN_PULL_DISABLE | PIN_MODE(0)); //D1
    out32(AM62X_CTRLMMR_PADCONFIG + 0x14, PIN_INPUT_ENABLE | PIN_PULL_DISABLE | PIN_MODE(0)); //D2
    out32(AM62X_CTRLMMR_PADCONFIG + 0x18, PIN_INPUT_ENABLE | PIN_PULL_DISABLE | PIN_MODE(0)); //D3
    out32(AM62X_CTRLMMR_PADCONFIG + 0x1C, PIN_INPUT_ENABLE | PIN_PULL_DISABLE | PIN_MODE(0)); //D4
    out32(AM62X_CTRLMMR_PADCONFIG + 0x20, PIN_INPUT_ENABLE | PIN_PULL_DISABLE | PIN_MODE(0)); //D5
    out32(AM62X_CTRLMMR_PADCONFIG + 0x24, PIN_INPUT_ENABLE | PIN_PULL_DISABLE | PIN_MODE(0)); //D6
    out32(AM62X_CTRLMMR_PADCONFIG + 0x28, PIN_INPUT_ENABLE | PIN_PULL_DISABLE | PIN_MODE(0)); //D7
    out32(AM62X_CTRLMMR_PADCONFIG + 0x2c, PIN_MODE(0) | PIN_PULL_DISABLE); //CSN0
...
static void init_sdmmc(void)
{
    ...
    // pinmux for SD(MMC1)
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_MMC1_DAT3, PIN_PULL_DIRECTION | PIN_INPUT_ENABLE | PIN_MODE(0));
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_MMC1_DAT2, PIN_PULL_DIRECTION | PIN_INPUT_ENABLE | PIN_MODE(0));
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_MMC1_DAT1, PIN_PULL_DIRECTION | PIN_INPUT_ENABLE | PIN_MODE(0));
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_MMC1_DAT0, PIN_PULL_DIRECTION | PIN_INPUT_ENABLE | PIN_MODE(0));
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_MMC1_CMD, PIN_PULL_DIRECTION | PIN_INPUT_ENABLE | PIN_MODE(0));
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_MMC1_SDCD, PIN_PULL_DIRECTION | PIN_INPUT_ENABLE | PIN_MODE(0));
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_MMC1_CLK, PIN_INPUT_ENABLE | PIN_MODE(0)); // Pull Down
    // GPIO0_31 for signal voltage switch
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_GPMC0_CLK, PIN_PULL_DISABLE | PIN_MODE(7));
    // GPIO0_31 as output
    out32(AM62X_GPIO0_BASE + AM62X_GPIO_DIR(EVM_SDIO_1V8_EN_PIN_NUM),
        in32(AM62X_GPIO0_BASE + AM62X_GPIO_DIR(EVM_SDIO_1V8_EN_PIN_NUM)) & ~(AM62X_GPIO_BIT(EVM_SDIO_1V8_EN_PIN_NUM)));
    // Default signal voltage 3.3v, output high
    out32(AM62X_GPIO0_BASE + AM62X_GPIO_SET_DATA(EVM_SDIO_1V8_EN_PIN_NUM), (unsigned int)(AM62X_GPIO_BIT(EVM_SDIO_1V8_EN_PIN_NUM)));

    // pinmux for eMMC(MMC0)
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_MMC0_DAT7, PIN_PULL_DIRECTION | PIN_INPUT_ENABLE | PIN_MODE(0));
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_MMC0_DAT6, PIN_PULL_DIRECTION | PIN_INPUT_ENABLE | PIN_MODE(0));
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_MMC0_DAT5, PIN_PULL_DIRECTION | PIN_INPUT_ENABLE | PIN_MODE(0));
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_MMC0_DAT4, PIN_PULL_DIRECTION | PIN_INPUT_ENABLE | PIN_MODE(0));
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_MMC0_DAT3, PIN_PULL_DIRECTION | PIN_INPUT_ENABLE | PIN_MODE(0));
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_MMC0_DAT2, PIN_PULL_DIRECTION | PIN_INPUT_ENABLE | PIN_MODE(0));
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_MMC0_DAT1, PIN_PULL_DIRECTION | PIN_INPUT_ENABLE | PIN_MODE(0));
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_MMC0_DAT0, PIN_PULL_DIRECTION | PIN_INPUT_ENABLE | PIN_MODE(0));
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_MMC0_CMD, PIN_PULL_DIRECTION | PIN_INPUT_ENABLE | PIN_MODE(0));
    out32(AM62X_CTRLMMR_PADCONFIG + PIN_MMC0_CLK, PIN_INPUT_ENABLE | PIN_MODE(0)); // Pull Down
...
/*
* init_pwm()
*    Board specific PWM pin/clk enable
*/
static void init_pwm(void)
{
    out32(0x000104044, 0x01); /* EPWM_TB_CLKEN_epwm0_tb_clken */
    out32(0x000106130, 0x01); /* EPWM_TB_CLKEN_epwm0_tb_clken_proxy */

    out32(AM62X_CTRLMMR_PADCONFIG + PIN_SPI0_CS0, ((PIN_PULL_DISABLE | PIN_INPUT_ENABLE) & (~PIN_PULL_DIRECTION)) | PIN_MODE(2)); /* A13 - A */
}
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/branches/7.1.0/trunk/hardware/startup/boards/am62x/evm/hw_init.c $ $Rev: 963235 $")
#endif
