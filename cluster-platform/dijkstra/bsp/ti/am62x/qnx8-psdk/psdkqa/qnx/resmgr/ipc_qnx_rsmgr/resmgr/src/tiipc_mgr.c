...
static int g_log_level = _SLOG_NOTICE;

static void IpcPrint(const char *str)
{
    slogf(_SLOG_SETCODE(_SLOGC_PRIVATE_START, 57), _SLOG_INFO, str);
}
...
void qnx_logger(int level, const char *fmt, ...)
{
    ...
    if (level <= g_log_level) {
        vslogf(_SLOG_SETCODE(_SLOGC_PRIVATE_START, 57), level, fmt, arglist);
    }
    va_end(arglist);
}
...
{
    QNX_PR_NOTICE("TI IPC resmgr for %s (version=%s, date=%s)", buildSoc, buildVersion, buildDate);
}
