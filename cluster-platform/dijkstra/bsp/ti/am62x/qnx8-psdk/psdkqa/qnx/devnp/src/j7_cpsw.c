    ...
    switch (opt) {
        ...
        case CPSWOPT_RUN_MASK_CPU:
            if (cpsw != NULL) {
                ...
                if (cpsw->run_mask_core > 2) {
                    // Bad run mask, so defaulting to both cores
                    cpsw->run_mask_core = 2;
                }
            }
            break;
...
#elif defined (CPSW9G) || defined (CPSW5G) || defined (CPSW2G_MAIN)
...
cpsw_attach(struct device *parent, struct device *self, void *aux)
{
    ...
    if (nw_pthread_create(&cpsw->tid, NULL,
        ...
        slogf(_SLOGC_NETWORK, _SLOG_ERROR, "%s:%d Error: nw_pthread_create failed: %s",
                __FUNCTION__, __LINE__, strerror(errno));
        return ENOMEM;
    }
