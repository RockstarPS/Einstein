...
static int32_t s28hx_set_device_config(struct _snor_chip_t* const chip, const uint32_t proto)
{
    ...
    if ((proto & SNOR_BUSPROTO_BUS_MASK) == SNOR_BUSPROTO_8_8_8) {
        ...
        /*
         * S28Hx Spec Table 51. Register latency code (cycles) versus frequency says
         * read register dummy cycle should be 6 for Octal mode and 2 for single mode.
         * Table 78 says register reading needs 4 bytes address in Octal mode,
         * so here we add 4 dummy cycles for Octal mode.
         */
        chip->rdr_dc = 10;
        if (proto & SNOR_BUSPROTO_DTR_MODE) {
