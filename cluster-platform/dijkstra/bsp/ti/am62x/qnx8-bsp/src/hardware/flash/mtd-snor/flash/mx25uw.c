...
#define MX25UW_MEM_DENSITY_16MB   0x38            /* Memory density */
...
int32_t f3s_mx25uw_ident(f3s_dbase_t *dbase, f3s_access_t *access, uint32_t flags, uint32_t cs)
{
    ...
    if ((ids[0] == MX25UW_MANID) &&
        ...
        ((ids[2] == MX25UW_MEM_DENSITY_16MB) || (ids[2] == MX25UW_MEM_DENSITY_32MB) || (ids[2] == MX25UW_MEM_DENSITY_64MB))) {
    ...
    if (dbase != NULL) {
        ...
        /* mx25uw51245g is a SPI NOR that supports the SFDPRD command but some samples return an empty SFDP page.
        ...
        */
        if (ret != EOK) {
            ...
            uint32_t chip_size = (ids[2] == MX25UW_MEM_DENSITY_32MB) ? 0x2000000U : 0x4000000U;
			if (ids[2]==MX25UW_MEM_DENSITY_16MB) chip_size = 0x1000000U;
    ...
    snor_slogf(_SLOG_INFO, ctrl->verbosity, 0,
            "%s: Supported ID[%02x:%02x:%02x] size[0x%08x]", __func__, ids[0], ids[1], ids[2], chip->chipsz);
...
static int _mx25uw_set_protocol(struct _snor_chip_t* const chip, const uint32_t proto)
{
    ...
    if ((proto & SNOR_BUSPROTO_BUS_MASK) == SNOR_BUSPROTO_8_8_8) {
        ...
        if (proto & SNOR_BUSPROTO_DTR_MODE) {
			...
			chip->rdcfg.cflgs |= SNOR_CFGFLGS_SWAPB;
			chip->wrcfg.cflgs |= SNOR_CFGFLGS_SWAPB;
			chip->cfg.cflgs |= SNOR_CFGFLGS_SWAPB;
        }

        for (int i = 0; i < SNOR_MAX_ERSCFG; i++) {
            chip->blkers[i].opcode = SNOR_CMD_SE;
