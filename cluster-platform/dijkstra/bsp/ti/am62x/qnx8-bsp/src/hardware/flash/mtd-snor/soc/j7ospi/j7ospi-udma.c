...
int j7ospi_udma_xfer(j7ospi_dev_t *ospi, paddr_t src, paddr_t dst, size_t len)
{
    ...
    if (Udma_ringQueueRaw(Udma_chGetFqRingHandle(&gUdmaChObj), pUdmaTrpdMem) != UDMA_SOK) {
        snor_slogf(_SLOG_ERROR, 0, 0, "%s: [Error] Channel queue failed!!\n", __func__);
        return (EIO);
    }
    ...
    if (gUdmaResult != UDMA_SOK) {
        snor_slogf(_SLOG_ERROR, 0, 0, "%s: [Error] No descriptor after callback!!\n", __func__);
        return (ETIMEDOUT);
    }
...
__SRCVERSION("$URL: http://svn.ott.qnx.com/product/hardware/branches/release/hardware/flash/mtd-snor/soc/j7ospi/j7ospi-udma.c $ $Rev: 998562 $")
#endif
