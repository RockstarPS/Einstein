...
static void* memset(void* dest, char c, size_t count)
{
...
static void check_for_special_prop(dtb_node* node, dtb_prop* prop)
{
    ...
    {
        size_t handle;
        dtb_read_prop_values(prop, 1, (uintmax_t *)&handle);
...
{
    if(str_len < prop->length)
    {
        memset((uint8_t*)prop->first_cell, 0, prop->length);
        memcpy((uint8_t*)prop->first_cell, str, str_len);
        return true;
    }
    return false;
}
