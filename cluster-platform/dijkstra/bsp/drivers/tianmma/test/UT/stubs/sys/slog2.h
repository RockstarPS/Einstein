 ...
 *
 * Copyright 2012-2019, QNX Software Systems. All Rights Reserved.
 *
...
/**
  ...
  */
#define SLOG2_INVALID_VERBOSITY ( (uint8_t) -1 )
...
#define SLOG2_ALLOC_TYPE_SHMEM      (1<<0)   /* use an OS-managed shared memory object for the buffer set   */
                                             /* this flag only has effect if slogger2 is invoked with the   */
                                             /* -d or -D options.                                           */
...
#define SLOG2_HINT_SKIP_BUFFER_0    (1<<3)   /* hint to skip buffer 0 when parsing                          */
#define SLOG2_HINT_SKIP_BUFFER_1    (1<<4)   /* hint to skip buffer 1 when parsing                          */
#define SLOG2_HINT_SKIP_BUFFER_2    (1<<5)   /* hint to skip buffer 2 when parsing                          */
#define SLOG2_HINT_SKIP_BUFFER_3    (1<<6)   /* hint to skip buffer 3 when parsing                          */
#define SLOG2_ALLOC_TYPE_PHYSICAL   (1<<7)   /* attempt to have the buffer set backed to physical memory,   */
                                             /* this flag is ignored if SLOG2_ALLOC_TYPE_SHMEM is set.      */
                                             /* This flag only has effect if slogger2 is invoked with the   */
                                             /* -d or -D options                                            */
#define SLOG2_LIMIT_RETRIES         (1<<8)   /* when calling the logging functions, this limits the amount  */
                                             /* of retries to write the write and read indexes: these       */
                                             /* functions will fail if the amount of retries is reached     */
#define SLOG2_QUIET                 (1<<9)   /* APIs will not write to stderr on failure for this instance  */
...
#define SLOG2_FA_VAR_FLOATING 0x00020000
/* <STAN_MACRO1> */
#define SLOG2_FA_NUMERIC( __x, __type ) ( SLOG2_FA_VAR_VAL | (__type) | (sizeof(__x) & SLOG2_FA_SIZE_MASK) ), (__x)
#define SLOG2_FA_FLOATING( __x ) ( SLOG2_FA_VAR_VAL | SLOG2_FA_VAR_FLOATING | (sizeof(__x) & SLOG2_FA_SIZE_MASK) ), (__x)
...
#define SLOG2_FA_FLOAT( __x )     SLOG2_FA_FLOATING( (__x) )
#define SLOG2_FA_DOUBLE( __x )    SLOG2_FA_FLOATING( (__x) )
#define SLOG2_FA_STRING( __x )    (size_t)( SLOG2_FA_STRING | ((strlen_isr(__x)+1) & SLOG2_FA_SIZE_MASK ) ), ( __x )
#define SLOG2_FA_STAR( __x )      ( SLOG2_FA_VAR_VAL | SLOG2_FA_STAR | sizeof(int) ), (int)( __x )
#define SLOG2_FA_END              (size_t)0, 0
/* </STAN_MACRO1> */
 ...
 */
uint8_t slog2_get_verbosity( slog2_buffer_t __buffer );

/*--------------------------------------------------------------------------*/
/**
 * Register a new instance of a slog2 buffer set.  Each buffer set contains one
 * or more buffers.  The size of each buffer is specified as a multiple of 4k
 * pages.  The maximum number of buffers is specified by SLOG2_MAX_BUFFERS.
 *
 * A maximum number of retries when writing to the buffer can be specified via
 * slog2_register() by passing the SLOG2_LIMIT_RETRIES flag and setting the
 * number of retries desired in the slog2_buffer_set_config_t data structure.
 * By default, there is no limit. Setting the value to UINT32_MAX or
 * (uint32_t)-1 is equivalent to an infinite number of retries as well.
 *
 ...
 * @return -1 on error, 0 on success
 */
int slog2_dump_logs_to_file( FILE *__file, uint32_t __flags, ... );
 ...
 * @return -1 on error, 0 on success
 */
int slog2_hash( slog2_hash_expiry_t __expiry,
                const char *__input,
                size_t __output_size,
                char *__output_hash );


typedef enum {
  SLOG2_OBF_FILEPATH,   /* input is a file path, which could    */
                        /* include directory name(s), file name */
                        /* and file extension                   */
  SLOG2_OBF_NUM         /* Number of items in this enum         */
} slog2_obf_t;

/**
 * Slog2 Obfuscate
 *
 * @param obf_type  type of input that needs to be obfuscated
 * @param obf_char  the character to use for obfuscation. A null character
 *                  (i.e.,'\0') indicates that slog2_hash() will be used
 *                  to fill in the obfuscated characters.
 *                  typical values used here are 'x' or '.'.
 * @param flags     Reserved for future use. Set to 0.
 * @param input     null-terminated input string (UTF-8) to be obfuscated (e.g. personally
 *                  identifiable information such as a file name)
 * @param size      size of the output buffer (i.e., size includes null character)
 * @param output    resulting obfuscated identifier string buffer with a maximum
 *                  length of size - 1. The string will be null terminated.
 *
 *
 * The obfuscated output string length will be truncated to size - 1 characters
 * if it is smaller than the size required to store input.
 *
 * When obf_char is not null the resulting obfuscated output string will always
 * produce the same output.
 * When obf_char is null the obfuscated output string value will be identical
 * within a similar time frame across all processes.
 *
 * @return -1 on error, 0 on success
 */
int slog2_obfuscate( slog2_obf_t __obf_type,
                     char __obf_char,
                     uint32_t __flags,
                     const char *__input,
                     size_t __size,
...
/**
 * Performs a reverse lookup of the buffer name to try and find the
 * corresponding buffer handle.
 *
 * @param buffer_name  Buffer name of a previously registered buffer
 * @param owner_name   Optional name of a previously registered
 *                     buffer set.
 *
 * @return NULL on not found, non-NULL value is the matching buffer handle
 */
slog2_buffer_t slog2_find_buffer( const char *__buffer_name,
                                  const char *__buffer_set_name );
...
__SRCVERSION( "$URL: http://svn.ott.qnx.com/product/branches/7.0.0/BC700_6762_sdp704/lib/slog2/public/sys/slog2.h $ $Rev: 879432 $" )
#endif
