...
class Logger
{
    public:
        ...
        static Logger *createInstance ( const char *const pAppId,
                const char *const pDescription,
                const bool debugEnabled = true,
                const eLogDestination_t logDest = eLogDestination_DLT,
                const bool silent = false,
                const std::string filename = "",
                const bool logOnlyError = false );

        static Logger *getInstance();
        ~Logger();
        bool init ( const DltLogLevelType loglevel, DltContext *pContext = NULL );
        void deInit();
        bool getEnabled() const;
        void registerContext ( DltContext &handle, const char *const pContextId, const char *const pDescription );
        void registerContext ( DltContext &handle, const char *const pContextId, const char *const pDescription, const DltLogLevelType level, const DltTraceStatusType status );
        void unregisterContext ( DltContext &handle ) const;
        void send();
        ...
        void append ( const char *const value )
        {
            ...
            {
                (void)dlt_user_log_write_string ( &mDltContextData, value );
            }
            else
            {
        ...
        template <class T>
        void appendNoDLT ( const T value )
        {
        ...
        std::string getFileName ( const std::string filePath, const bool withExtension = true, const char seperator = '/' );
        std::string now() const;

    private:
        Logger ( const char *const pAppId,
                const char *const pDescription,
                const bool debugEnabled = true,
                const eLogDestination_t logDest = eLogDestination_DLT,
                const bool silent = false,
                const std::string filename = "",
                const bool logOnlyError = false ); ///< Private because of singleton pattern

        bool initNoDlt ( const DltLogLevelType loglevel, DltContext *const pContext );
