...
int main ( int argc, char *argv[] )
{
    ...
    if(2 == argc )
    {
        ...
        if( option < 7 || option > 0 )
        {
            ...
            LOG_REGISTER_APP_DLT ( "TC", "Test C Application" );
            
            LOG_REGISTER_CONTEXT ( mLogContext1, "MOD2", "Module 2 Context", option );
            ...
            LOGW ( &mLogContext1, "Warning message %s %s", "Welcome", "INDIA" );
            LOGI ( &mLogContext1, "Info message %d and %d", 500, 700  ); 
            LOGD ( &mLogContext1, "Debug message %d and %d", 100, 200 );
            LOGV ( &mLogContext1, "Verbose message %s %s", " WELCOME ", " Banagalore " );
