 ...
 *
 * \file boot_kpi_startup.h
 *
 * \brief
 * Visteon KPI header file for QNX startup
 *
 ******************************************************************************/
 
#ifndef BOOT_KPI_QSTARTUP_H
#define BOOT_KPI_QSTARTUP_H
    ...
    do { \
        ...
        if ((Id >= 0) && (Id < KPI_ENTRY_LAST_INV)) { \
            if (pKPI_Records->record[Id].Sig_u16 != 0) { \
                ...
                pKPI_Records->record[Id].Time_u32 = (uint32_t)(ClockCycles()/200000u)+KPI_POWERUP_OFFSET; \
                ...
                pKPI_Records->record[Id].Sig_u16 = 1 + (Id) + bitCount; \
            } \
        } \
    } while (0)
