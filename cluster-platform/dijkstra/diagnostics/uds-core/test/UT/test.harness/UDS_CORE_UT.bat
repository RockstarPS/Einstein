...
echo options C_COMPILE_CMD g++ -c -g -fpermissive >> commands.tmp
...
echo options C_DEFINE_LIST VC_UNIT_TEST VCAST_NO_PTR_TO_UNCON_ARRAY_SUPPORT >> commands.tmp
...
echo options C_LINK_OPTIONS -static-libgcc -static-libstdc++ >> commands.tmp
...
echo options VCAST_ENHANCED_FOR_LOOP_COVERAGE TRUE >> commands.tmp
...
echo options TESTABLE_SOURCE_DIR ..\..\..\..\uds-core\src\ >> commands.tmp
echo options TESTABLE_SOURCE_DIR ..\stubs\ >> commands.tmp
echo environment build UDS_CORE_UT.env >> commands.tmp
echo /E:UDS_CORE_UT tools script run UDS_CORE_UT.tst >> commands.tmp
echo /E:UDS_CORE_UT execute batch >> commands.tmp
echo /E:UDS_CORE_UT reports custom management UDS_CORE_UT_management_report.html >> commands.tmp
