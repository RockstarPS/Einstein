...
# Configuration
SERVICE_TO_KILL="test_10.service"
SERVICE_TO_MONITOR="wdg_proxy_test_10.service"
LOG_FILTER="wdg"

echo "------------------------------------------------------------------"
echo "----- Test Case 17 - Restart type: on-failure --------------------"
...
# Function to trigger watchdog-like failure (if supported by app)
send_signal() {
    local TEST_NAME=$1
    local SIGNAL=$2
    local DESC=$3
...
# 3–4. Execute tests
send_signal "Test with watchdog timeout (1st attempt)" 35 "SIGRTMIN+1"
sleep 1
send_signal "Test with watchdog timeout (2nd attempt)" 35 "SIGRTMIN+1"
