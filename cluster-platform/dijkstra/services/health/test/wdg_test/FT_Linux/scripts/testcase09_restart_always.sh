#!/bin/sh

# Configuration
SERVICE_TO_KILL="test_04.service"
SERVICE_TO_MONITOR="wdg_proxy_test_04.service"
LOG_FILTER="wdg_proxy"

echo "------------------------------------------------------------------"
echo "----- Test Case 9 - Restart type: always --------------------------"
echo "  Cause of termination:"
echo "        (1) Clean exit (code 0)"
echo "        (2) Clean signals: SIGHUP, SIGINT, SIGTERM, SIGPIPE"
echo "        (3) Unclean exit (code 1)"
echo "        (4) Unclean signals: SIGKILL, SIGSEGV"
echo ""
echo "  Expected behavior:"
echo "        The process shall always be restarted regardless of"
echo "        exit code or signal."
echo ""
echo "  Error Notification Fatal (Yes/No): No"
echo "  Process Restart (Yes/No): Yes"
echo "  [*] Configuration:"
echo "        Restart=always"
echo "------------------------------------------------------------------"

# 1. Record start time
TEST_START_TIME=$(date "+%Y-%m-%d %H:%M:%S")

# 2. Start services
for SERVICE in "$SERVICE_TO_KILL" "$SERVICE_TO_MONITOR"; do
    echo "[>] Starting $SERVICE..."
    systemctl start "$SERVICE"

    if ! systemctl is-active --quiet "$SERVICE"; then
        echo "Error: Failed to start $SERVICE."
        continue
    fi

    echo "[+] $SERVICE started successfully."
    sleep 2
done

# Helper function
send_kill() {
    local sig=$1
    local desc=$2

    echo "......................................................."
    echo "       Test: $desc"
    echo "......................................................."

    PID_01=$(systemctl show --property MainPID --value "$SERVICE_TO_KILL")

    if [ "$PID_01" -ne 0 ]; then
        echo "[!] Sending signal $sig to $SERVICE_TO_KILL (PID: $PID_01)..."
        kill -$sig "$PID_01"
    else
        echo "Error: Could not find PID."
    fi

    sleep 2
}

# 3–9 Tests
send_kill 35 "signal-triggered clean exit (code 0)"
send_kill 1  "clean signal termination (SIGHUP)"
send_kill 36 "signal-triggered exit (code 255)"
send_kill 6  "unclean signal termination (SIGABRT)"
send_kill 37 "signal-triggered exit (code 1)"
send_kill 9  "unclean signal termination (SIGKILL)"
send_kill 15 "clean signal termination (SIGTERM)"

# 10. Review results
echo "-----------------------------------------------"
echo "    Recent journal entries"
echo "-----------------------------------------------"
journalctl -u "$SERVICE_TO_KILL" --since "$TEST_START_TIME" --no-pager

# 11. Capture DLT logs
TIMEOUT_SEC=5
echo "-----------------------------------------------"
echo "    Capturing DLT logs for $TIMEOUT_SEC seconds..."
echo "-----------------------------------------------"
timeout "$TIMEOUT_SEC"s /bin/dlt-receive -a localhost | grep "$LOG_FILTER"

# 12. Stop services
for SERVICE in "$SERVICE_TO_KILL" "$SERVICE_TO_MONITOR"; do
    echo "-----------------------------------------------"
    echo "[*] Checking status: $SERVICE"

    if systemctl is-active --quiet "$SERVICE"; then
        echo "[!] $SERVICE is running. Stopping for a clean state..."
        systemctl stop "$SERVICE"
        sleep 1
    fi
done

echo "--- Test complete. DLT logging stopped. ---"
echo "******************************************************"