...
echo "------------------------------------------------------------------"
echo "----- Test Case 12 - Restart type: Always with limit (count) -----"
...
echo "    The process should restart only up to the limit configured in StartLimitBurst"
echo "    within the StartLimitInterval."
echo ""
...
send_kill_signal() {
    ...
    if [ "$PID" -ne 0 ]; then
        echo "[!] Sending signal 35 (SIGRTMIN+1) to $SERVICE_TO_KILL (PID: $PID)..."
        kill -35 "$PID"
    else
...
# 3–5. Execute restart tests
send_kill_signal "First test with clean exit (code 0)"
send_kill_signal "Second test with clean exit (code 0)"
send_kill_signal "Third test with clean exit (code 0)"
