...
echo options C_COMPILE_CMD gcc -c -g -std=c99 >> commands.tmp
...
echo options C_PREPROCESS_CMD gcc -E -C -std=c99 >> commands.tmp
...
echo options TESTABLE_SOURCE_DIR ..\..\..\..\src\res_mon\ >> commands.tmp
echo environment build UT_RES_MON.env >> commands.tmp
echo /E:UT_RES_MON tools script run UT_RES_MON.tst >> commands.tmp
echo /E:UT_RES_MON execute batch >> commands.tmp
echo /E:UT_RES_MON reports custom full UT_RES_MON_management_report.html >> commands.tmp
